...
{
    public class AltaProveedorDAO : GenericDAO<tblCom_sp_proveedores>, IAltaProveedorDAO
    {
        ...
        public Dictionary<string, object> getProveedores()
        {
            try
            {
                ...
                List<tblCom_sp_proveedores> listaProveedores = _context.tblCom_sp_proveedores.Where(x => x.registroActivo).ToList();
        ...
        public Dictionary<string, object> obtenerDatosProveedores(int id,int numpro)
        {
            try
            {
                ...
                // var ProvEkontrol = _context.tblCom_sp_proveedores.Where(e => e.registroActivo && e.numpro == numpro).ToList();

                // var ProvCuentasEkontrol = from cuentas in _context.tblCom_sp_proveedores_cta_dep
                //     join mon in _context.tblC_TipoMoneda on cuentas.moneda equals mon.id
                //     join ban in _context.tblC_sb_banco on cuentas.banco equals ban.banco
                //     where cuentas.numpro == numpro
                //     select new
                //     {
                //         cuentas.numpro,
                //         cuentas.id_cta_dep,
                //         banco = ban.banco,
                //         descBanco = ban.descripcion,
                //         moneda = mon.id,
                //         descMoneda = mon.nombreCorto,
                //         cuentas.cuenta,
                //         cuentas.sucursal,
                //         cuentas.tipo_cta,
                //         cuentas.plaza,
                //         cuentas.clabe,
                //         descCuenta = cuentas.descCuenta,
                //         cuentas.plastico
                //     };
                ...
                //     InformacionCuentas.moneda = item.moneda.ToString();
        ...
        public Dictionary<string, object> GuardarProveedor(tblCom_sp_proveedoresDTO objProveedor, HttpPostedFileBase objFile)
        {
            using (var dbContextTransaction = _context.Database.BeginTransaction())
            {
                try
                {
                    ...
                    if (objProveedor.moneda <= 0) { throw new Exception("Moneda requerida"); }
                    ...
                    {
                        var Existe = _context.tblCom_sp_proveedores.Where(e => e.registroActivo && (e.numpro == objProveedor.numpro || e.rfc == objProveedor.rfc) && e.moneda == objProveedor.moneda).FirstOrDefault();
                        ...
                        if (Existe == null)
                        {
                            ...
                            using (var ctxARR = new MainContext(EmpresaEnum.Arrendadora))
                            {
...
//                                 lstCorreos.Add("sangeldc@gmail.com");
// #endif
...
//                                 #endregion
            ...
            using (var dbContextTransaction = _context.Database.BeginTransaction())
            {
                using (var connEk = ConexionEnkontrol())
                {
                    using (var transactionEk = connEk.BeginTransaction())
                    {
                        using(var ctxArr = new MainContext(EmpresaEnum.Arrendadora))
	                    {
                            using (var dbTransacArr = ctxArr.Database.BeginTransaction())
                            {
                                using (var dbEKArrenadora = new Conexion().Connect(2))
                                {
                                    using (var dbTransacEKArrendadora = dbEKArrenadora.BeginTransaction())
                                    {
                                        try
                                        {
                                            ...
                                            #region AGREGAR REGISTRO SP ARRENDADORA

                                            var objProvARR = ctxArr.tblCom_sp_proveedores.FirstOrDefault(e => e.registroActivo && e.numpro == objProveedor.numpro);
                                            ...
                                            if (objProvARR == null)
                                            {
                                                ...
                                                ctxArr.tblCom_sp_proveedores.Add(objNewProv);
                                                ctxArr.SaveChanges();
                                                ...
                                                if (infoProveedorCuentas != null && infoProveedorCuentas.Count() > 0)
                                                {
                                                    foreach (var item in infoProveedorCuentas)
                                                    {
                                                        ...
                                                        ctxArr.tblCom_sp_proveedores_cta_dep.Add(infoCuentas);
                                                        ctxArr.SaveChanges();
                                                    }
                                                }
                                                #endregion
                                                ...
                                                if (objEvidenciaProv != null)
                                                {
                                                    ...
                                                    ctxArr.tblCom_ArchivosAdjuntosProveedores.Add(objNewEvidenciaProv);
                                                    ctxArr.SaveChanges();
                                                }
                                            }
                                            ...
                                            string body = string.Format(@"
                                                Buen dia ,<br>Se informa que el usuario {0} {1} {2} Autorizó el alta del siguiente proveedor {3} {4} en la empresa Construplan. <br>
                                                {5}
                                                <br>
                                                <H2>
                                                {6}</H2>
                                            ...
                                            var insertProvArr = string.Format(@"
                                                            ...
                                                            sp_proveedores
                                                                ...
                                                                tipo_tercero, tipo_operacion, curp, bit_factoraje, id_fiscal, nacionalidad, persona_fisica, a_paterno, a_materno, a_nombre,
                                                                tipo_pago_transferencia, bit_estrategico, swift_code, account_number, routing, numpro_factoraje, num_empleado, obliga_cfd, clave_id, numregidtrib,
                                                                fecha_mod, pp, ekuserweb, st_aut, st_vobo, bit_complemento_pago
                                                                )
                                                            VALUES
                                                                (
                                                                        ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,
                                            ...
                                            using (var cmd = new OdbcCommand(insertProvArr))
                                            {
                                                ...
                                                cmd.Parameters.Add("@st_aut", OdbcType.Bit).Value = false;
                                                cmd.Parameters.Add("@st_vobo", OdbcType.Bit).Value = false;
                                                cmd.Parameters.Add("@bit_complemento_pago", OdbcType.Char).Value = "N";
                                                cmd.Connection = dbTransacEKArrendadora.Connection;
                                                cmd.Transaction = dbTransacEKArrendadora;
                                                cmd.ExecuteNonQuery();
                                            }
                                        ...
                                        catch (Exception e)
                                        {
                                            ...
                                            dbContextTransaction.Rollback();
                                            dbTransacArr.Rollback();
                                            dbTransacEKArrendadora.Rollback();
            ...
            using (var dbContextTransactionNotificacion = _context.Database.BeginTransaction())
            {
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        if (objVobo != null)
                        {
                            var alertasRelacionadasAlVoBo = _context.tblP_Alerta
                                .Where(x => x.tipoAlerta == 2 && x.sistemaID == 4 && x.objID == (int)objProveedor.numpro && (x.obj == "AutorizacionProveedor" || x.obj == "AltaProveedor") && 
                                    x.msj.Contains("Proveedor") && !x.visto).ToList();
                            ...
                            foreach (var item in objAutorizador)
                            {
                                ...
                                string body = string.Format(@"
                                    Buen dia ,<br>Se informa que el usuario {0} {1} {2} confirmó el alta en la empresa Construplan del siguiente proveedor {3} {4} y se encuentra listo para ser autorizado <br>
                                    {5}
                                    <br>
                                    <H2>
                                    {6}</H2>
        ...
        public Dictionary<string, object> VisualizarArchivoAdjunto(int idArchivo)
        {
            ...
            try
            {
                tblCom_ArchivosAdjuntosProveedores objArchivoAdjunto = _context.tblCom_ArchivosAdjuntosProveedores.Where(w => w.id == idArchivo && w.registroActivo).FirstOrDefault();
        ...
        public int GetLastProveedor(int tipoProveedor)
        {
            ...
            try
            {
                ...
                int? numProvEKCP = 0;
                List<int> lstObjsProveedores = new List<int>();
                ...
                {
                    lstProvsSPCP = ctx.tblCom_sp_proveedores
                    ...
                    int numProvSP = findMissing(lstProvsSPCP);

                    if (lstProvsSPCP.Count() > 0)
                    {
                        valSPCP = numProvSP == -1 ? (lstProvsSPCP.Max() + 1) : numProvSP;
                    }
                }
                #endregion
                ...
                //sigNumProveedor = lstIds.Min().Value;
            }
            catch (Exception e)
            {
                throw e;
        ...
        public List<Core.DTO.Principal.Generales.ComboDTO> FillComboCiudad()
        {
            try
            {
                ...
                var lst = _context.tblCom_sp_ciudades.Where(w => w.registroActivo).Select(x => new Core.DTO.Principal.Generales.ComboDTO
                {
                    Value = x.id.ToString(),
                    Text = x.concepto,
        ...
        public List<Core.DTO.Principal.Generales.ComboDTO> FillComboTipoProveedor()
        {
            try
            {
                ...
                var lst = _context.tblCom_sp_tipo_prov.Where(w => w.registroActivo).Select(x => new Core.DTO.Principal.Generales.ComboDTO
                {
                    Value = x.id.ToString(),
                    Text = x.concepto,
                    Prefijo = string.Empty
        ...
        public List<Core.DTO.Principal.Generales.ComboDTO> FillComboTipoTercero()
        {
            try
            {
                ...
                var lst = _context.tblCom_sp_tercero.Where(w => w.registroActivo).Select(x => new Core.DTO.Principal.Generales.ComboDTO
                {
                    Value = x.id.ToString(),
                    Text = x.concepto,
                    Prefijo = string.Empty
        ...
        public List<Core.DTO.Principal.Generales.ComboDTO> FillComboTipoOperacion()
        {
            try
            {
                ...
                var lst = _context.tblCom_sp_operacion.Where(w => w.registroActivo).Select(x => new Core.DTO.Principal.Generales.ComboDTO
                {
                    Value = x.id.ToString(),
                    Text = x.concepto,
                    Prefijo = string.Empty
        ...
        public List<Core.DTO.Principal.Generales.ComboDTO> FillComboTipoPagoTerceroTrans()
        {
            try
            {
                ...
                var lst = _context.tblCom_sp_pago_tercero.Where(w => w.registroActivo).Select(x => new Core.DTO.Principal.Generales.ComboDTO
                {
                    Value = x.id.ToString(),
                    Text = x.concepto,
                    Prefijo = string.Empty
        ...
        public List<Core.DTO.Principal.Generales.ComboDTO> FillComboTipoMovBase()
        {
            try
            {
                ...
                var lst = _context.tblCom_sp_tm.Where(w => w.registroActivo).Select(x => new Core.DTO.Principal.Generales.ComboDTO
                {
                    Value = x.id.ToString(),
                    Text = x.concepto,
                    Prefijo = string.Empty
        ...
        public List<Core.DTO.Principal.Generales.ComboDTO> FillComboTipoMoneda()
        {
            try
            {
                var lst = _context.tblC_TipoMoneda.Where(w => w.registroActivo).Select(x => new Core.DTO.Principal.Generales.ComboDTO
                {
                    Value = x.id.ToString(),
                    Text = x.nombreCorto,
                    Prefijo = string.Empty
        ...
        public List<Core.DTO.Principal.Generales.ComboDTO> FillComboBancos()
        {
            try
            {
                ...
                var lst = _context.tblCom_sb_bancos.Where(w => w.registroActivo).Select(x => new Core.DTO.Principal.Generales.ComboDTO
                {
                    Value = x.id.ToString(),
                    Text = x.concepto,
                    Prefijo = string.Empty
        ...
        public List<Core.DTO.Principal.Generales.ComboDTO> FillComboTipoRegimen()
        {
            try
            {
                ...
                var lst = _context.tblCom_sr_regimen.Where(w => w.registroActivo).Select(x => new Core.DTO.Principal.Generales.ComboDTO
                {
                    Value = x.id.ToString(),
                    Text = x.concepto,
                    Prefijo = string.Empty
