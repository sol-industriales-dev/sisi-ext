using Core.DAO.RecursosHumanos;
using Core.DAO.RecursosHumanos.ActoCondicion;
using Core.DTO.RecursosHumanos;
using Core.DTO.RecursosHumanos.ActoCondicion;
using Core.DTO.RecursosHumanos.ActoCondicion.Acta;
using Core.DTO.RecursosHumanos.ActoCondicion.CargaZipActoCondicion;
using Core.DTO.RecursosHumanos.ActoCondicion.Graficas;
using Core.DTO.RecursosHumanos.ActoCondicion.Historial;
...
namespace Data.DAO.RecursosHumanos.ActoCondicion
{
    public class ActoCondicionCHDAO : GenericDAO<tblRH_AC_Acto>, IActoCondicionCHDAO
    {
        #region VARIABLES PRIVADAS
        private readonly string RutaBase = @"\\REPOSITORIO\Proyecto\SIGOPLAN\CAPITAL_HUMANO\ACTO_CONDICIONES\ACTOS";
        private const string RutaLocal = @"C:\Proyecto\SIGOPLAN\CAPITAL_HUMANO\ACTOS_CONDICIONES\ACTOS";
        ...
        private const string _NOMBRE_CONTROLADOR = "ActoCondicionCHController";
        ...
        private readonly List<int> empresas = new List<int> { 1, 2, 3, 6, 8 };
        #endregion

        #region CONSTRUCTOR
        public ActoCondicionCHDAO()
        {
            resultado.Clear();
#if DEBUG
            RutaBase = RutaLocal;
        ...
        public Dictionary<string, object> ObtenerCentrosCostos()
        {
            ...
            {
                LogError(0, 0, _NOMBRE_CONTROLADOR, "ObtenerCentrosCostos", e, AccionEnum.CONSULTA, 0, null);
                resultado.Clear();
        ...
        public Dictionary<string, object> ObtenerSupervisores()
        {
            try
            {
                ...
                var listaSupervisoresActos = _context.tblRH_AC_Acto
                    .Where(x => x.activo)
                ...
                var listaSupervisoresCondiciones = _context.tblRH_AC_Condicion
                    .Where(x => x.activo)
                    .Select(x => new ComboDTO
                    {
                        Value = x.claveSupervisor.ToString(),
            ...
            {
                LogError(0, 0, _NOMBRE_CONTROLADOR, "ObtenerSupervisores", e, AccionEnum.CONSULTA, 0, null);
                resultado.Clear();
                resultado.Add(SUCCESS, false);
            }
        ...
        public Dictionary<string, object> ObtenerDepartamentos()
        {
            try
            {
                #region FILL COMBO DEPARTAMENTOS NUEVO
                List<ComboDTO> lstCboDepartamentos = _context.Select<ComboDTO>(new DapperDTO
                {
                    ...
                    consulta = @"SELECT id AS Value, UPPER(descripcion) AS text FROM tblRH_AC_Departamentos WHERE estatus = @estatus ORDER BY descripcion",
                    parametros = new { estatus = true }
                });
                resultado.Add(ITEMS, lstCboDepartamentos);
                #endregion

                #region FILL COMBO DEPARTAMENTOS ANTERIOR
                //if (vSesiones.sesionEmpresaActual == (int)EmpresaEnum.GCPLAN || vSesiones.sesionEmpresaActual == (int)EmpresaEnum.Arrendadora)
                //{
                //    List<ComboDTO> lstCboDepartamentos = _context.Select<ComboDTO>(new DapperDTO
                //    {
                //        baseDatos = (MainContextEnum)vSesiones.sesionEmpresaActual,
                ...
                //    resultado.Add(ITEMS, lstCboDepartamentos);
                //}
                //else if (vSesiones.sesionEmpresaActual == (int)EmpresaEnum.Colombia || vSesiones.sesionEmpresaActual == (int)EmpresaEnum.Peru)
                //{
                //    List<ComboDTO> lstCboDepartamentos = _context.Select<ComboDTO>(new DapperDTO
                //    {
                //        baseDatos = (MainContextEnum)vSesiones.sesionEmpresaActual,
            ...
            {
                LogError(0, 0, _NOMBRE_CONTROLADOR, "ObtenerDepartamentos", e, AccionEnum.CONSULTA, 0, null);
                resultado.Clear();
                resultado.Add(SUCCESS, false);
            }
        ...
        public Dictionary<string, object> FillCboSubclasificacionesDepartamentos(int idDepartamento)
        {
            ...
            try
            {
                #region FILL COMBO SUBCLASIFICACIONES DEPARTAMENTOS NUEVO
                List<ComboDTO> lstSubclasificacionesDep = _context.Select<ComboDTO>(new DapperDTO
                {
                    baseDatos = (MainContextEnum)vSesiones.sesionEmpresaActual,
                    consulta = @"SELECT id AS Value, subclasificacionDep AS text FROM tblRH_AC_SubclasificacionDepartamentos WHERE registroActivo = @registroActivo AND idDepartamento = @idDepartamento ORDER BY subclasificacionDep",
                    parametros = new { registroActivo = true, idDepartamento = idDepartamento }
                });
                resultado.Add(ITEMS, lstSubclasificacionesDep);
                #endregion

                #region FILL COMBO SUBCLASIFICACIONES DEPARTAMENTOS ANTERIOR
                //if (vSesiones.sesionEmpresaActual == (int)EmpresaEnum.GCPLAN || vSesiones.sesionEmpresaActual == (int)EmpresaEnum.Arrendadora)
                //{
                //    List<ComboDTO> lstSubclasificacionesDep = _context.Select<ComboDTO>(new DapperDTO
                //    {
        ...
        public Dictionary<string, object> FillCboSubclasificaciones()
        {
            ...
            try
            {
                var items = _context.tblRH_AC_SubclasificacionDepartamentos.Where(x => x.registroActivo).OrderBy(x => x.subclasificacionDep).Select(x => new ComboDTO
                {
                    Value = x.id.ToString(),
        ...
        public Dictionary<string, object> ObtenerAcciones()
        {
            try
            {
                var acciones = _context.tblRH_AC_Accion
                    .ToList()
                    .Select(x => new ComboDTO
                    {
                        Value = x.id.ToString(),
            ...
            {
                LogError(0, 0, _NOMBRE_CONTROLADOR, "ObtenerAcciones", e, AccionEnum.CONSULTA, 0, null);
                resultado.Clear();
                resultado.Add(SUCCESS, false);
            }
        ...
        public Dictionary<string, object> ObtenerClasificaciones()
        {
            try
            {
                var clasificaciones = _context.tblRH_AC_Clasificacion.ToList();

                var clasificacionesActo = clasificaciones
                    .Where(x => x.tipoRiesgo == TipoRiesgoCH.Acto)
                    .Select(x => new ComboDTO
                    {
                        Value = x.id.ToString(),
                        Text = x.descripcion.ToUpper()
                ...
                var clasificacionesCondicion = clasificaciones
                    .Where(x => x.tipoRiesgo == TipoRiesgoCH.Condicion)
                    .Select(x => new ComboDTO
                    {
                        Value = x.id.ToString(),
                        Text = x.descripcion.ToUpper()
            ...
            {
                LogError(0, 0, _NOMBRE_CONTROLADOR, "ObtenerClasificaciones", e, AccionEnum.CONSULTA, 0, null);
                resultado.Clear();
                resultado.Add(SUCCESS, false);
            }
        ...
        public Dictionary<string, object> CargarActosCondiciones(FiltroActoCondicionDTO filtro)
        {
            try
            {
                ...
                if (vSesiones.sesionEmpresaActual == (int)EmpresaEnum.GCPLAN || vSesiones.sesionEmpresaActual == (int)EmpresaEnum.Arrendadora || vSesiones.sesionEmpresaActual == (int)EmpresaEnum.Construplan)
                {
                    #region CONSTRUPLAN Y ARRENDADORA
                    #region SE OBTIENE LISTADO DE SUBCLASIFICACIONES DEPARTAMENTOS
                    List<tblRH_AC_Departamentos> lstDepartamentos = _context.Select<tblRH_AC_Departamentos>(new DapperDTO
                    {
                        baseDatos = (MainContextEnum)vSesiones.sesionEmpresaActual,
                        consulta = @"SELECT id, descripcion FROM tblRH_AC_Departamentos WHERE estatus = @estatus",
                        parametros = new { estatus = true }
                    }).ToList();
                    #endregion

                    #region SE OBTIENE LISTADO DE SUBCLASIFICACIONES DEPARTAMENTOS
                    List<tblRH_AC_SubclasificacionDepartamentos> lstSubclasificacionesDep = _context.Select<tblRH_AC_SubclasificacionDepartamentos>(new DapperDTO
                    {
                        baseDatos = (MainContextEnum)vSesiones.sesionEmpresaActual,
                        consulta = @"SELECT id, subclasificacionDep, idDepartamento FROM tblRH_AC_SubclasificacionDepartamentos WHERE registroActivo = @registroActivo",
                        parametros = new { registroActivo = true }
                    }).ToList();
                    #endregion

                    string actoDesc = TipoRiesgoCH.Acto.GetDescription();
                    string condicionDesc = TipoRiesgoCH.Condicion.GetDescription();
                    string estatusCompletoDesc = EstatusActoCondicionCH.Completo.GetDescription();

                    filtro.fechaFinal = filtro.fechaFinal.AddHours(23).AddMinutes(59);

                    bool estatusVencido = filtro.estatus == (int)EstatusActoCondicionCH.Vencido;

                    #region Actos
                    if (filtro.estatus == (int)EstatusActoCondicionCH.Vencido)
                        filtro.estatus = (int)EstatusActoCondicionCH.EnProceso;

                    bool puedeEliminar = new UsuarioDAO().getViewAction(7320, "eliminarEvidencia");

                    if (vSesiones.sesionUsuarioDTO.id == 79807 || vSesiones.sesionUsuarioDTO.idPerfil == 1) // RIGOBERTO NARCISO CORONADO ASTIAZARAN (SOLAMENTE ÉL PUEDE ELIMINAR)
                        puedeEliminar = true;

                    var actos = _context.tblRH_AC_Acto.Where(w => w.activo && w.cc == filtro.cc)
                    ...
                    {
                        string CC_Descripcion = GetDescripcionCC(item.cc);
                        item.proyecto = CC_Descripcion;
                    }

                    if (estatusVencido)
                        filtro.estatus = (int)EstatusActoCondicionCH.Vencido;
                ...
                else if (vSesiones.sesionEmpresaActual == (int)EmpresaEnum.Colombia || vSesiones.sesionEmpresaActual == (int)EmpresaEnum.Peru)
                {
                    ...
                    var actos = _context.tblRH_AC_Acto
                        .Where(x =>
                            ...
                            (filtro.estatus == -1 ? true : x.estatus == (EstatusActoCondicionCH)filtro.estatus) &&
                    ...
                    foreach (var item in actos)
                    {
                        EstatusActoCondicionCH estatus = 0;
                        string estatusDesc = null;
                        if (item.estatus == EstatusActoCondicionCH.Completo)
                        {
                            estatus = item.estatus;
                            estatusDesc = EstatusActoCondicionCH.Completo.GetDescription();
                        }
                        else if (item.fechaFirmado.HasValue && item.fechaFirmado.Value.AddDays(14) < DateTime.Now.Date)
                        {
                            estatus = EstatusActoCondicionCH.Vencido;
                            estatusDesc = EstatusActoCondicionCH.Vencido.GetDescription();
                        }
                        else
                        {
                            estatus = EstatusActoCondicionCH.EnProceso;
                            estatusDesc = EstatusActoCondicionCH.EnProceso.GetDescription();
                        }
                        ...
                        if (filtro.estatus == -1 || (int)estatus == filtro.estatus)
                        {
                            ...
                            {
                                //proyecto = listaContratistas.First(x => x.id == (int)item.idAgrupacion).nombreEmpresa; // OMAR
                            }
                            else if (item.idEmpresa < 1000)
                            {
                                //proyecto = item.agrupacion.nomAgrupacion; // OMAR
                            }
                            else
                            {
                                //proyecto = listaAgrupacionContratistas.First(x => x.id == item.idAgrupacion).nomAgrupacion; // OMAR
                            }
                    ...
                    var condiciones = _context.tblRH_AC_Condicion
                        .Where(x =>
                            x.fechaSuceso >= filtro.fechaInicial &&
                            x.fechaSuceso <= filtro.fechaFinal &&
                            x.idAgrupacion == filtro.idAgrupacion &&
                        ...
                        .Select(x => new ActoCondicionDTO
                        {
                            ...
                            //        listaAgrupacionContratistas.Where(y => y.id == x.idAgrupacion).Select(z => z.nomAgrupacion).FirstOrDefault()
                            //    : x.agrupacion.nomAgrupacion,
                            tipoRiesgo = TipoRiesgoCH.Condicion,
            ...
            {
                resultado.Add(SUCCESS, false);
                LogError(0, 0, _NOMBRE_CONTROLADOR, "CargarActosCondiciones", e, AccionEnum.CONSULTA, 0, null);
        ...
        public Dictionary<string, object> GuardarActo(ActoDTO acto)
        {
            using (var dbContextTransaction = _context.Database.BeginTransaction())
            {
                try
                {
                    ...
                    tblRH_AC_Acto objActo = _context.tblRH_AC_Acto.Where(x => x.id == acto.id).FirstOrDefault();
                    if (objActo == null)
                    {
                        #region NUEVO REGISTRO
                        tblRH_AC_Acto nuevoActo = InicializarActo(acto);
                        nuevoActo.cc = acto.cc;

                        if (nuevoActo.tipoActo == TipoActoCH.Inseguro)
                        {
                            ...
                            if (nuevoActo.accionID != (int)AccionInfraccionEnum.AMONESTACION)
                                nuevoActo.estatus = EstatusActoCondicionCH.EnProceso;
                        }

                        _context.tblRH_AC_Acto.Add(nuevoActo);
                        _context.SaveChanges();

                        #region COMENTADO
                        //if (acto.tipoActo == TipoActoCH.Inseguro && acto.accionID == (int)AccionInfraccionEnum.AMONESTACION)
                        //{
                        //    var causasAcciones = new List<tblRH_AC_ActoAccionReaccion>();
                        //    foreach (var item in acto.causas)
                        //    {
                        //        var causaAccion = new tblRH_AC_ActoAccionReaccion();
                        ...
                        //    _context.tblRH_AC_ActoAccionReaccion.AddRange(causasAcciones);
                        //    _context.SaveChanges();
                        //}
                        #endregion

                        #region SE ENVIA CORREO AL EMPLEADO CAPTURADO, AL RESPONSABLE DEL CC Y A RIGOBERTO CORONADO
                        List<string> lstClaveEmpleados = new List<string>();
                        lstClaveEmpleados.Add(acto.claveEmpleado.ToString());
                        lstClaveEmpleados.Add(acto.FK_ClaveEmpleadoResponsableCC.ToString());
                        lstClaveEmpleados.Add("30607"); // RIGOBERTO CORONADO

                        List<string> lstCorreos = _context.tblP_Usuario.Where(w => lstClaveEmpleados.Contains(w.cveEmpleado) && w.estatus).Select(s => s.correo).ToList();
#if DEBUG
                        //lstCorreos = new List<string>();
                        //lstCorreos.Add("omar.nunez@construplan.com.mx");
#endif

                        GlobalUtils.sendEmail(string.Format("{0}: Se ha registrado un acto", PersonalUtilities.GetNombreEmpresa()), string.Format("Se ha registrado un acto al empleado: [{0}] {1}.", acto.claveEmpleado, acto.nombre), lstCorreos);
                        #endregion
                        #endregion
                    }
                    else if (objActo != null)
                    {
                        #region ACTUALIZACIÓN
                        objActo.accionID = acto.accionID;
                        objActo.descripcion = acto.descripcion;
                        objActo.clasificacionID = acto.clasificacionID;
                        objActo.procedimientoID = acto.procedimientoID;
                        _context.SaveChanges();
                        #endregion
                    }
                    ...
                    SaveBitacora(0, acto.id <= 0 ? (int)AccionEnum.AGREGAR : (int)AccionEnum.ACTUALIZAR, acto.id, JsonUtils.convertNetObjectToJson(acto));
                }
                catch (Exception e)
                {
                    dbContextTransaction.Rollback();
                    LogError(0, 0, _NOMBRE_CONTROLADOR, "GuardarActo", e, AccionEnum.AGREGAR, 0, new { acto = acto });
                    resultado.Add(SUCCESS, false);
                    resultado.Add(MESSAGE, e.Message);
                }
            }
        ...
        public Dictionary<string, object> GuardarCondicion(CondicionDTO condicion)
        {
            using (var dbContextTransaction = _context.Database.BeginTransaction())
            {
                try
                {
                    ...
                    tblRH_AC_Condicion nuevaCondicion = null;
                    ...
                    if (condicion.id == 0)
                    {
                        ...
                        nuevaCondicion.estatus = EstatusActoCondicionCH.EnProceso;

                        _context.tblRH_AC_Condicion.Add(nuevaCondicion);
                        _context.SaveChanges();
                        ...
                        nuevaCondicion.departamento = _context.tblRH_AC_Departamentos.First(x => x.id == nuevaCondicion.departamentoID);
                        nuevaCondicion.prioridad = _context.tblRH_AC_ClasificacionPrioridad.First(x => x.id == nuevaCondicion.nivelPrioridad);
                    ...
                    else
                    {
                        ...
                        var condicionExistente = _context.tblRH_AC_Condicion.First(x => x.id == condicion.id);
                        ...
                        if (condicion.imagenDespues != null)
                        {
                            ...
                            condicionExistente.estatus = EstatusActoCondicionCH.EnProceso;

                            if (condicionExistente.fechaFirmado.HasValue)
                            {
                                condicionExistente.estatus = EstatusActoCondicionCH.Completo;
                            }
                ...
                {
                    dbContextTransaction.Rollback();
                    LogError(0, 0, _NOMBRE_CONTROLADOR, "GuardarCondicion", e, AccionEnum.AGREGAR, 0, new { condicion = condicion });
                    resultado.Clear();
                    resultado.Add(SUCCESS, false);
                    resultado.Add(MESSAGE, e.Message);
                }
        ...
        public Dictionary<string, object> ObtenerActoCondicion(TipoRiesgoCH tipoRiesgo, int id)
        {
            try
            {
                switch (tipoRiesgo)
                {
                    case TipoRiesgoCH.Acto:

                        var tblActo = _context.tblRH_AC_Acto.First(x => x.id == id);
                        ...
                        acto.tipoRiesgo = TipoRiesgoCH.Acto;
                        acto.numeroInfraccion = tblActo.numeroInfraccion.HasValue ? tblActo.numeroInfraccion.Value : 0;
                        acto.descripcionInfraccion = tblActo.numeroInfraccion.HasValue && tblActo.numeroInfraccion.Value != 0 ? _context.tblRH_AC_MatrizAccionesDisciplinarias.First(y => y.numero == tblActo.numeroInfraccion.Value).tipoInfraccion : "";
                        ...
                        acto.numeroFalta = tblActo.tipoActo == TipoActoCH.Seguro ? 0 : tblActo.numeroFalta ?? 1;
                        ...
                        {
                            var causas = _context.tblRH_AC_ActoAccionReaccion.Where(x => x.actoID == acto.id).ToList();
                    ...
                    case TipoRiesgoCH.Condicion:

                        var condicion = _context.tblRH_AC_Condicion
                            .Where(x => x.id == id)
                            .ToList()
            ...
            {
                LogError(0, 0, _NOMBRE_CONTROLADOR, "ObtenerActoCondicion", e, AccionEnum.CONSULTA, id, null);
                resultado.Add(SUCCESS, false);
        ...
        public Dictionary<string, object> EliminarActoCondicion(TipoRiesgoCH tipoRiesgo, int id)
        {
            try
            {
                switch (tipoRiesgo)
                {
                    case TipoRiesgoCH.Acto:
                        ...
                        var acto = _context.tblRH_AC_Acto
                            .First(x => x.id == id);
            ...
            {
                LogError(0, 0, _NOMBRE_CONTROLADOR, "EliminarActoCondicion", e, AccionEnum.ELIMINAR, id, null);
                resultado.Add(SUCCESS, false);
        ...
        public Tuple<Stream, string> DescargarArchivo(int sucesoID, TipoRiesgoCH tipoRiesgo, TipoArchivo tipoArchivo)
        {
            ...
            {
                LogError(0, 0, _NOMBRE_CONTROLADOR, "DescargarArchivo", e, AccionEnum.DESCARGAR, sucesoID, 0);
                return null;
            }
        }
        ...
        public Dictionary<string, object> CargarDatosArchivo(int sucesoID, TipoRiesgoCH tipoRiesgo, TipoArchivo tipoArchivo)
        {
            ...
            {
                LogError(0, 0, _NOMBRE_CONTROLADOR, "CargarDatosArchivo", e, AccionEnum.CONSULTA, sucesoID, null);
                resultado.Add(SUCCESS, false);
            }
            return resultado;
        }
        ...
        public Dictionary<string, object> ObtenerInformacionInfraccion(int numeroInfraccion, int claveEmpleado, DateTime fechaSuceso)
        {
            try
            {
                InfoInfraccionDTO objInfraccion = CalcularNivelFaltaAccion(numeroInfraccion, claveEmpleado, fechaSuceso);

                // SE OBTIENE LA CANTIDAD DE INFRACCIÓN 

                if (objInfraccion.estatus)
                {
                    resultado.Add(SUCCESS, true);
                    resultado.Add(ITEMS, objInfraccion);
                }
                else
                {
                    resultado.Add(SUCCESS, false);
                    resultado.Add(MESSAGE, objInfraccion.mensaje);
                }
            }
            catch (Exception e)
            {
                LogError(0, 0, _NOMBRE_CONTROLADOR, "ObtenerInformacionInfraccion", e, AccionEnum.CONSULTA, 0, null);
                resultado.Clear();
                resultado.Add(SUCCESS, false);
            }
        ...
        public Dictionary<string, object> ObtenerInformacionInfraccionContratista(int numeroInfraccion, int claveEmpleado, DateTime fechaSuceso)
        {
            try
            {
                ...
                var infoMatriz = _context.tblRH_AC_MatrizAccionesDisciplinarias.FirstOrDefault(x => x.estatus && x.numero == numeroInfraccion);
                ...
                var infraccionesAcumuladas = _context.tblRH_AC_Acto.Where(x => x.claveEmpleado == claveEmpleado && x.tipoActo == TipoActoCH.Inseguro && x.activo && x.numeroInfraccion != null).ToList();
                ...
                {
                    var tiempoDeSancion = _context.tblRH_AC_MatrizAccionesDisciplinarias.FirstOrDefault(x => x.numero == existeInfraccionRescision.numeroInfraccion).sancion;
            ...
            {
                LogError(0, 0, _NOMBRE_CONTROLADOR, "ObtenerInformacionInfraccionExterno", e, AccionEnum.CONSULTA, 0, new { numeroInfraccion = numeroInfraccion, fechaSuceso = fechaSuceso });
                resultado.Clear();
                resultado.Add(SUCCESS, false);
            }
        ...
        {
            var matriz = _context.tblRH_AC_MatrizAccionesDisciplinarias.Where(x => x.estatus).ToList();
            ...
            var infraccionesAcumuladas = _context.tblRH_AC_Acto.Where(x => x.claveEmpleado == claveEmpleado && x.tipoActo == 0 && x.activo && x.numeroInfraccion != null).ToList();
            var fecha30DiasAntes = fechaSuceso.AddDays(-30);
            var fecha30DiasDespues = fechaSuceso.AddDays(30);
        ...
        public Dictionary<string, object> ObtenerAccionReaccion(int tipo)
        {
            try
            {
                var opciones = _context.tblRH_AC_AccionReaccionContactoPersonal.Where(x => x.tipo == tipo && x.estatus).ToList();
            ...
            {
                LogError(0, 0, _NOMBRE_CONTROLADOR, "ObtenerAccionReaccion", e, AccionEnum.CONSULTA, 0, null);
                resultado.Clear();
                resultado.Add(SUCCESS, false);
            }
        ...
        public Dictionary<string, object> ObtenerPrioridades()
        {
            try
            {
                var listaPrioridades = _context.tblRH_AC_ClasificacionPrioridad.Where(x => x.estatus).Select(x => new
                {
                    Value = x.id,
                    Text = x.descripcion,
                    Prefijo = ""
            ...
            {
                LogError(0, 0, _NOMBRE_CONTROLADOR, "ObtenerPrioridades", e, AccionEnum.CONSULTA, 0, null);
                resultado.Clear();
                resultado.Add(SUCCESS, false);
            }
        ...
        public reporteActoCondicionDTO ObtenerReporteActoCondicion(int id, int tipo)
        {
            ...
            try
            {
                if (tipo == (int)TipoRiesgoCH.Acto)
                {
                    var acto = _context.tblRH_AC_Acto.First(x => x.id == id);
                    ...
                    objActoCondicion.esActoSeguro = acto.tipoActo == TipoActoCH.Seguro;
                    objActoCondicion.esActoInseguro = acto.tipoActo == TipoActoCH.Inseguro;
                    ...
                    {
                        var causasAcciones = _context.tblRH_AC_ActoAccionReaccion.Where(x => x.actoID == acto.id && x.estatus).ToList();
                ...
                {
                    var condicion = _context.tblRH_AC_Condicion.First(x => x.id == id);
                    objActoCondicion.codigoControl = "";
            ...
            {
                resultado = new Dictionary<string, object>();
                LogError(0, 0, _NOMBRE_CONTROLADOR, "ObtenerReporteActoCondicion", e, AccionEnum.CONSULTA, 0, new { id, tipo });
                resultado.Add(SUCCESS, false);
            }
        ...
        public Dictionary<string, object> GuardarFirma(GuardarFirmaDTO data)
        {
            using (var transaccion = _context.Database.BeginTransaction())
            {
                try
                {
                    ...
                    {
                        case TipoRiesgoCH.Acto:
                            var acto = _context.tblRH_AC_Acto.First(x => x.id == data.idActoCondicion);
                            switch (data.tipoFirma)
                            {
                                case Core.Enum.RecursosHumanos.ActoCondicion.TipoFirmaEnum.EMPLEADO:
                                    acto.firmaEmpleado = firmaBase64SinTransparencia(data.imagen);
                                    break;
                                case Core.Enum.RecursosHumanos.ActoCondicion.TipoFirmaEnum.SUPERVISOR:
                                    acto.firmaSupervisor = firmaBase64SinTransparencia(data.imagen);
                                    break;
                                case Core.Enum.RecursosHumanos.ActoCondicion.TipoFirmaEnum.SST:
                            ...
                            break;
                        case TipoRiesgoCH.Condicion:
                            var condicion = _context.tblRH_AC_Condicion.First(x => x.id == data.idActoCondicion);
                            switch (data.tipoFirma)
                            {
                                case Core.Enum.RecursosHumanos.ActoCondicion.TipoFirmaEnum.SUPERVISOR:
                            ...
                            if (firmado)
                            {
                                ...
                                {
                                    condicion.estatus = EstatusActoCondicionCH.Completo;
                                }
                ...
                catch (Exception e)
                {
                    ...
                    resultado = new Dictionary<string, object>();
                    LogError(0, 0, _NOMBRE_CONTROLADOR, "GuardarFirma", e, AccionEnum.AGREGAR, 0, data);
                    resultado.Add(SUCCESS, false);
                    resultado.Add(MESSAGE, e.Message);
                }
            }
        ...
        public Dictionary<string, object> VistoAlerta(int id)
        {
            ...
            catch (Exception e)
            {
                ...
                LogError(0, 0, _NOMBRE_CONTROLADOR, "VisorAlerta", e, AccionEnum.ACTUALIZAR, 0, id);
        ...
        //        var relacionEmpleadoAreaCC = ObtenerRelacionEmpleadoAreaCC(TipoRiesgoCH.Acto, acto.idAgrupacion.Value, acto.idEmpresa, (AreaEnum)acto.departamentoID);
        //        if (relacionEmpleadoAreaCC.Count > 0)
        //        {
        //            var responsableCapitalHumanoDelCC = relacionEmpleadoAreaCC.FirstOrDefault(x => x.area == AreaEnum.CapitalHumano);
        ...
        //                var infraccion = _context.tblRH_AC_MatrizAccionesDisciplinarias.First(x => x.numero == acto.numeroInfraccion);
        ...
        //    catch (Exception e)
        //    {
        //        LogError(0, 0, _NOMBRE_CONTROLADOR, "DescargarActa", e, AccionEnum.CONSULTA, 0, id);
        //        resultado.Clear();
        //        resultado.Add(SUCCESS, false);
        //        resultado.Add(MESSAGE, e.Message);
        //    }
        ...
        {
            sn_empleadosDTO resultado = null;

            var empleados = new List<sn_empleadosDTO>();
            foreach (var empresa in empresas)
            {
                var empleado = _context.Select<sn_empleadosDTO>(new DapperDTO
                {
                    ...
                    consulta = @"SELECT
                                ...
                                clave_depto,
                                puesto,
                                estatus_empleado as estatus_empleados
                            FROM
            ...
            {
                resultado = new sn_empleadosDTO();
                resultado.sexo = "";
            }

            resultado = empleados.OrderBy(x => x.estatus_empleados).FirstOrDefault();
...
//            if (resultadoConstruplan == null && resultadoArrendadora == null)
//            {
...
//            return resultadoArrendadora;
        ...
        public Dictionary<string, object> CargarActa(HttpPostedFileBase acta, int id)
        {
            using (var transaccion = _context.Database.BeginTransaction())
            {
                try
                {
                    var acto = _context.tblRH_AC_Acto.First(x => x.id == id && x.activo && x.estatus == EstatusActoCondicionCH.EnProceso);
                    ...
                    {
                        acto.estatus = EstatusActoCondicionCH.Completo;
                ...
                {
                    transaccion.Rollback();
                    LogError(0, 0, _NOMBRE_CONTROLADOR, "CargaActa", e, AccionEnum.ACTUALIZAR, 0, id);
                    resultado.Clear();
                    resultado.Add(SUCCESS, false);
                    resultado.Add(MESSAGE, e.Message);
                }
        ...
        private List<tblS_Req_EmpleadoAreaCC> ObtenerRelacionEmpleadoAreaCC(TipoRiesgoCH riesgo, int idAgrupacion, int idEmpresa, AreaEnum area)
        {
            ...
            //}else
            //{
            //   public int empresa = vSesiones.sesionEmpresaActual;
            //}
                ...
                case TipoRiesgoCH.Acto:
                    {
                        ...
                        {
                            relacionEmpleadoGrupo = _context.tblS_Req_EmpleadoAreaCC.Where(x => x.idAgrupacion == idAgrupacion && x.idEmpresa == vSesiones.sesionEmpresaActual && areasActo.Contains(x.area) && x.division == 1 && x.estatus).ToList();
                        }
                ...
                case TipoRiesgoCH.Condicion:
                    {
                        ...
                        if (vSesiones.sesionEmpresaActual == 1 || vSesiones.sesionEmpresaActual == 2)
                        {
                            relacionEmpleadoGrupo = _context.tblS_Req_EmpleadoAreaCC.Where(x => x.idAgrupacion == idAgrupacion && x.idEmpresa == idEmpresa && areasCondicion.Contains(x.area) && x.division == 1).ToList();
                        }
                        else
                        {
                            relacionEmpleadoGrupo = _context.tblS_Req_EmpleadoAreaCC.Where(x => x.idAgrupacion == idAgrupacion && x.idEmpresa == vSesiones.sesionEmpresaActual && areasCondicion.Contains(x.area) && x.division == 1).ToList();
                        }
        ...
        private void NotificarActo(tblRH_AC_Acto acto)
        {
            ...
            var relacionEmpleadoGrupo = ObtenerRelacionEmpleadoAreaCC(TipoRiesgoCH.Acto, acto.idAgrupacion.Value, acto.idEmpresa, (AreaEnum)acto.departamentoID);
        ...
        private void NotificarCondicion(tblRH_AC_Condicion condicion)
        {
            List<MultiplesDTO> reportes = null;
            if (HttpContext.Current.Session["rptNotificacionActoCondicion"] != null)
            {
            ...
            var relacionEmpleadoGrupo = ObtenerRelacionEmpleadoAreaCC(TipoRiesgoCH.Condicion, condicion.idAgrupacion.Value, condicion.idEmpresa, (AreaEnum)condicion.departamentoID);
        ...
        public Dictionary<string, object> CargarComprimido(HttpPostedFileBase archivoComprimido)
        {
            using (var transaccion = _context.Database.BeginTransaction())
            {
                try
                {
                    ...
                    using (var zip = new ZipArchive(archivoComprimido.InputStream, ZipArchiveMode.Read))
                    {
                                    ...
                                    if (hoja.Dimension.Rows >= 3)
                                    {
                                        for (int i = 3; i <= hoja.Dimension.Rows; i++)
                                        {
                                            ...
                                            switch (tipoRiesgo)
                                            {
                                                ...
                                                case "ACTO":
                                                    {
                                                        var acto = new tblRH_AC_Acto();

                                                        var tipoActoTexto = hoja.Cells["C" + i].GetValue<string>().Trim().ToUpper();

                                                        #region infoEmpleado
                                                        //empleadoIncidenteDTO infoEmpleado = null; // OMAR
                                                        var claveEmpleado = hoja.Cells["N" + i].GetValue<int>();
                                                        /*var resultadoInfoEmpleado = incidenteFS.getInfoEmpleado(claveEmpleado, esContratista, idEmpresa); // OMAR
                                                        ...
                                                        //acto.nombre = infoEmpleado.nombreEmpleado; // OMAR
                                                        //acto.puesto = infoEmpleado.puestoEmpleado; // OMAR
                                                        //acto.fechaIngreso = infoEmpleado.antiguedadEmpleado; // OMAR
                                                        acto.esExterno = esExterno;
                                                        acto.claveContratista = claveContratista;
                                                        //acto.claveInformo = claveInformo; // OMAR
                                                        //acto.nombreInformo = infoEmpleadoInformo.nombreEmpleado; // OMAR
                                                        ...
                                                        //acto.claveSupervisor = claveSupervisor; // OMAR
                                                        //acto.nombreSupervisor = infoEmpleadoSupervisor.nombreEmpleado; // OMAR
                                                        ...
                                                        switch (tipoActoTexto)
                                                        {
                                                            #region SEGURO
                                                            case "SEGURO":
                                                                acto.tipoActo = TipoActoCH.Seguro;
                                                                acto.estatus = EstatusActoCondicionCH.Completo;
                                                                acto.accionID = (int)AccionInfraccionEnum.AMONESTACION;

                                                                _context.tblRH_AC_Acto.Add(acto);
                                                                _context.SaveChanges();
                                                                break;
                                                            #endregion
                                                            #region INSEGURO
                                                            case "INSEGURO":
                                                                ...
                                                                {
                                                                    estatus = EstatusActoCondicionCH.Completo;
                                                                }
                                                                #endregion

                                                                acto.accionID = falta.nivelInfraccionAcumulado;
                                                                acto.tipoActo = TipoActoCH.Inseguro;
                                                                ...
                                                                if (acto.accionID == (int)AccionInfraccionEnum.AMONESTACION)
                                                                {
                                                                    ...
                                                                    {
                                                                        var ca = new tblRH_AC_ActoAccionReaccion();
                                                                ...
                                                                if (acto.accionID != (int)AccionInfraccionEnum.AMONESTACION)
                                                                {
                                                                    acto.departamento = _context.tblRH_AC_Departamentos.First(x => x.id == acto.departamentoID);
                                                                    acto.accion = _context.tblRH_AC_Accion.First(x => x.id == acto.accionID);
                                                ...
                                                case "CONDICION":
                                                    {
                                                        ...
                                                        {
                                                            var condicion = new tblRH_AC_Condicion();

                                                            //condicion.claveInformo = claveInformo; // OMAR
                                                            //condicion.nombreInformo = infoEmpleadoInformo.nombreEmpleado; // OMAR
                                                            ...
                                                            //condicion.claveSupervisor = claveSupervisor; // OMAR
                                                            //condicion.nombreSupervisor = infoEmpleadoSupervisor.nombreEmpleado; // OMAR
                                                            condicion.departamentoID = hoja.Cells["F" + i].GetValue<int>();
                                                            condicion.subclasificacionDepID = hoja.Cells["AL" + i].GetValue<int>();
                                                            condicion.estatus = EstatusActoCondicionCH.EnProceso;
                                                            ...
                                                            _context.tblRH_AC_Condicion.Add(condicion);
                                                            _context.SaveChanges();
                                                            ...
                                                            condicion.departamento = _context.tblRH_AC_Departamentos.First(x => x.id == condicion.departamentoID);
                                                            condicion.prioridad = _context.tblRH_AC_ClasificacionPrioridad.First(x => x.id == condicion.nivelPrioridad);
                        ...
                        else
                        {
                            ...
                            LogError(0, 0, _NOMBRE_CONTROLADOR, "CargarComprimido", null, AccionEnum.AGREGAR, 0, mensajeError + " Nombre archivo: " + nombreArchivo);
                        }
                    }
                    #endregion
                }
                catch (Exception e)
                {
                    ...
                    resultado.Add(SUCCESS, false);
                    resultado.Add(MESSAGE, e.Message);
                    LogError(0, 0, _NOMBRE_CONTROLADOR, "CargarComprimido", e, AccionEnum.AGREGAR, 0, 0);
                }
            }
                            ...
                            if (objActo == null)
                            {
                                ...
                                if (!string.IsNullOrEmpty(tipoRiesgo))
                                {
                                    if (tipoRiesgo.Trim().ToUpper() == "ACTO")
                                        objActoDTO.tipoRiesgo = TipoRiesgoCH.Acto;
                                    else if (tipoRiesgo.Trim().ToUpper() == "CONDICION" || tipoRiesgo.Trim().ToUpper() == "CONDICIÓN")
                                        objActoDTO.tipoRiesgo = TipoRiesgoCH.Condicion;
                                }
                                #endregion
                                ...
                                if (!string.IsNullOrEmpty(tipoActo))
                                {
                                    if (tipoActo.Trim().ToUpper() == "SEGURO")
                                        objActoDTO.tipoActo = TipoActoCH.Seguro;
                                    else if (tipoActo.Trim().ToUpper() == "INSEGURO")
                                        objActoDTO.tipoActo = TipoActoCH.Inseguro;
                                }
                                #endregion
                                ...
                                {
                                    List<tblRH_AC_Clasificacion> lstClasificaciones = _context.tblRH_AC_Clasificacion.ToList();
                                ...
                                if (objActoDTO.tipoRiesgo == TipoRiesgoCH.Acto)
                                {
                                ...
                                else if (objActoDTO.tipoRiesgo == TipoRiesgoCH.Condicion)
                                {
        ...
        public Dictionary<string, object> DescargarFormato()
        {
            ...
            {
                LogError(0, 0, _NOMBRE_CONTROLADOR, "DescargarFormato", e, AccionEnum.DESCARGAR, 0, 0);
                return null;
            }
        }
        ...
        public List<CapturaActoCondicion_AutocompleteEmpleadoDTO> GetInfoEmpleado(string term)
        {
            try
            {
                var resultado = new List<CapturaActoCondicion_AutocompleteEmpleadoDTO>();
                foreach (var empresa in empresas)
                {
                    var nombreEmpresa = "NOMBRE EMPRESA";
                    switch ((EmpresaEnum)empresa)
                    {
                        case EmpresaEnum.Construplan:
                            nombreEmpresa = "CONSTRUPLAN";
                            break;
                        case EmpresaEnum.Arrendadora:
                            nombreEmpresa = "ARRENDADORA";
                            break;
                        ...
                        case EmpresaEnum.Integradora:
                            nombreEmpresa = "INTEGRADORA";
                            break;
                        case EmpresaEnum.Peru:
                            nombreEmpresa = "PERÚ";
                            break;
                        case EmpresaEnum.GCPLAN:
                            nombreEmpresa = "GCPLAN";
                            break;
                    }

                    var empleados = _context.Select<CapturaActoCondicion_InfoEmpleadoDTO>(new DapperDTO
                    {
                        baseDatos = (MainContextEnum)empresa,
                    ...
                    foreach (var item in empleados)
                    {
                        ...
                        resultado.Add(empleadoAutocomplete);
                    }
                }
...
//                return empleados;
            ...
            {
                LogError(0, 0, _NOMBRE_CONTROLADOR, "GetInfoEmpleado", e, AccionEnum.CONSULTA, 0, 0);
            }
        ...
        public List<CapturaActoCondicion_AutocompleteEmpleadoDTO> GetInfoEmpleadoInternoContratista(string term, bool esContratista, int idEmpresaContratista)
        {
            ...
            if (!esContratista)
            {
                ...
                foreach (var empresa in empresas)
                {
                    ...
                    var resultados = _context.Select<CapturaActoCondicion_AutocompleteEmpleadoDTO>(new DapperDTO
                    {
                        baseDatos = (MainContextEnum)empresa,
                        consulta = string.Format(@"
                                        SELECT
                                            e.clave_empleado AS id, (e.nombre + ' ' + e.ape_paterno + ' ' + e.ape_materno + ' [{0}]') AS label
                                        FROM tblRH_EK_Empleados as e
                                        WHERE UPPER(TRIM(e.nombre + e.ape_paterno + e.ape_materno)) LIKE @term
                                        ORDER BY e.ape_paterno DESC", nombreEmpresa),
                        parametros = new { term = termFormateado }
                    });
                    resultado.AddRange(resultados);
                }

                #region SE OBTIENE INFORMACIÓN DE PERSONAL INTERNO ANTERIOR
...
//                                consulta = @"
//                                        SELECT
//                                            e.clave_empleado AS id, (e.nombre + ' ' + e.ape_paterno + ' ' + e.ape_materno + ' [COLOMBIA]') AS label
//                                        FROM tblRH_EK_Empleados as e
//                                        WHERE UPPER(TRIM(e.nombre + e.ape_paterno + e.ape_materno)) LIKE @term
...
//                                    consulta = @"
//                                        SELECT
//                                            e.clave_empleado AS id, (e.nombre + ' ' + e.ape_paterno + ' ' + e.ape_materno + ' [PERÚ]') AS label
//                                        FROM tblRH_EK_Empleados as e
//                                        WHERE UPPER(TRIM(e.nombre + e.ape_paterno + e.ape_materno)) LIKE @term
//                                        ORDER BY e.ape_paterno DESC",
//                                    parametros = new { term = termFormateado }
...
//                }
        ...
        public Dictionary<string, object> DescargarReporteExcel(FiltroActoCondicionDTO filtro)
        {
            try
            {
                ...
                var matrizAccionesDisciplinarias = _context.tblRH_AC_MatrizAccionesDisciplinarias.Where(x => x.estatus);
                ...
                var actos = _context.tblRH_AC_Acto
                    .Where(x =>
                        ...
                        (filtro.claveSupervisor == 0 ? true : x.claveSupervisor == filtro.claveSupervisor) &&
                        (filtro.departamentoID == 0 ? true : x.departamentoID == filtro.departamentoID) &&
                        (filtro.estatus == -1 ? true : filtro.estatus == (int)EstatusActoCondicionCH.Vencido ? x.estatus == EstatusActoCondicionCH.EnProceso : x.estatus == (EstatusActoCondicionCH)filtro.estatus) &&
                        x.activo)
                    .Select(x => new ReporteActoCondicionExcelDTO
                    {
                        ...
                        tipoActo = x.tipoActo == TipoActoCH.Seguro ? "Seguro" : "Inseguro"
                    }).ToList();
            ...
            catch (Exception e)
            {
                resultado.Add(SUCCESS, false);
                resultado.Add(MESSAGE, e.Message);
                LogError(0, 0, _NOMBRE_CONTROLADOR, "DescargarReporteExcel", e, AccionEnum.CONSULTA, 0, filtro);
            }
        ...
        public Dictionary<string, object> ObtenerClasificacionesGenerales()
        {
            try
            {
                var listaClasificaciones = _context.tblRH_AC_ClasificacionGeneral.Where(x => x.estatus).Select(x => new
                {
                    Value = x.id,
                    Text = x.descripcion,
                    Prefijo = ""
            ...
            {
                LogError(0, 0, _NOMBRE_CONTROLADOR, "ObtenerClasificacionesGenerales", e, AccionEnum.CONSULTA, 0, null);
                resultado.Clear();
                resultado.Add(SUCCESS, false);
            }
        ...
        public Dictionary<string, object> FillCboProcedimientos(int FK_Clasificacion)
        {
            ...
            {
                #region VALIDACIONES
                if (FK_Clasificacion <= 0) { throw new Exception("Ocurrió un error al obtener el listado de procedimientos."); }
                #endregion

                #region FILL COMBO PROCEDIMIENTOS
                List<tblRH_AC_MatrizAccionesDisciplinarias> lstProcedimientos = _context.Select<tblRH_AC_MatrizAccionesDisciplinarias>(new DapperDTO
                {
                    baseDatos = (MainContextEnum)vSesiones.sesionEmpresaActual,
                    consulta = @"SELECT id, tipoInfraccion FROM tblRH_AC_MatrizAccionesDisciplinarias WHERE FK_Clasificacion = @FK_Clasificacion AND estatus = @estatus",
                    parametros = new { FK_Clasificacion = FK_Clasificacion, estatus = true }
                }).ToList();

                List<ComboDTO> lstComboDTO = new List<ComboDTO>();
                ComboDTO objComboDTO = new ComboDTO();
                foreach (var item in lstProcedimientos)
                {
                    if (item.id > 0 && !string.IsNullOrEmpty(item.tipoInfraccion))
                    {
                        objComboDTO = new ComboDTO();
                        objComboDTO.Value = item.id.ToString();
                        objComboDTO.Text = item.tipoInfraccion.Trim();
            ...
            {
                LogError(0, 0, _NOMBRE_CONTROLADOR, "FillCboProcedimientos", e, AccionEnum.FILLCOMBO, 0, 0);
                resultado.Add(MESSAGE, e.Message);
                resultado.Add(SUCCESS, false);
            }
            return resultado;
        ...
        public Dictionary<string, object> GuardarArchivoAdjunto(List<HttpPostedFileBase> lstArchivos, int idActo)
        {
            using (var dbContextTransaction = _context.Database.BeginTransaction())
            {
                ...
                {
                    #region VALIDACIONES
                    if (idActo <= 0) { throw new Exception("Ocurrió un error al registar el archivo."); }
                    #endregion

                    #region SE REGISTRA EL ARCHIVO ADJUNTO
                    var listaRutaArchivos = new List<Tuple<HttpPostedFileBase, string>>();
#if DEBUG
                    var CarpetaNueva = Path.Combine(RutaLocal, idActo.ToString());
#else
                    var CarpetaNueva = Path.Combine(RutaBase, idActo.ToString());
#endif
                    ...
                    foreach (var objArchivo in lstArchivos)
                    {
                        ...
                        tblRH_AC_ActoArchivos objEvidencia = new tblRH_AC_ActoArchivos()
                        {
                            FK_Acto = idActo,
                        ...
                        _context.tblRH_AC_ActoArchivos.Add(objEvidencia);
                        _context.SaveChanges();
                    }
                ...
                catch (Exception e)
                {
                    ...
                    dbContextTransaction.Rollback();
                    resultado.Add(MESSAGE, e.Message);
                    LogError(0, 0, "ActoCondicionCHController", "GuardarArchivoAdjunto", e, AccionEnum.AGREGAR, idActo, new { lstArchivos = lstArchivos, idActo = idActo });
                }
            }
            return resultado;
        }
        ...
        public Dictionary<string, object> GetArchivosAdjuntos(int idActo)
        {
            ...
            {
                #region VALIDACIONES
                if (idActo <= 0) { throw new Exception("Ocurrió un error al obtener el listado de archivos adjuntos."); }
                #endregion

                #region SE OBTIENE LISTADO DE ARCHIVOS ADJUNTOS EN BASE AL ACTO SELECCIONADO
                List<tblRH_AC_ActoArchivos> lstArchivos = _context.tblRH_AC_ActoArchivos.Where(w => w.FK_Acto == idActo && w.registroActivo).ToList();
            ...
            {
                LogError(0, 0, _NOMBRE_CONTROLADOR, "GetArchivosAdjuntos", e, AccionEnum.CONSULTA, 0, new { idActo = idActo });
                resultado.Add(MESSAGE, e.Message);
                resultado.Add(SUCCESS, false);
            }
            return resultado;
        ...
        public Dictionary<string, object> VisualizarArchivoAdjunto(int idArchivo)
        {
            ...
            try
            {
                tblRH_AC_ActoArchivos objArchivoAdjunto = _context.tblRH_AC_ActoArchivos.Where(w => w.id == idArchivo && w.registroActivo).FirstOrDefault();
        ...
        public Dictionary<string, object> EliminarArchivoAdjunto(int idArchivo)
        {
            ...
            try
            {
                ...
                tblRH_AC_ActoArchivos objEliminar = _context.tblRH_AC_ActoArchivos.Where(w => w.id == idArchivo && w.registroActivo).FirstOrDefault();
                if (objEliminar == null)
            ...
            {
                LogError(0, 0, _NOMBRE_CONTROLADOR, "EliminarArchivoAdjunto", e, AccionEnum.ELIMINAR, idArchivo, 0);
                resultado.Add(MESSAGE, e.Message);
                resultado.Add(SUCCESS, false);
            }
            return resultado;
        ...
        public Dictionary<string, object> GetSiguienteInfraccion(int claveEmpleado, int procedimientoID)
        {
            ...
            {
                #region VALIDACIONES
                if (claveEmpleado <= 0) { throw new Exception("Es necesario indicar la clave del empleado."); }
                if (procedimientoID <= 0) { throw new Exception("Es necesario indicar el procedimiento."); }
                #endregion

                #region SE VERIFICA SI EL EMPLEADO YA CUENTA CON UNA AMONESTACIÓN
                int CantActos_ClaveEmpleado = _context.Select<int>(new DapperDTO
                {
                    baseDatos = (MainContextEnum)vSesiones.sesionEmpresaActual,
                    consulta = @"SELECT claveEmpleado
	                                    FROM tblRH_AC_Acto AS t1
	                                    INNER JOIN tblRH_AC_MatrizAccionesDisciplinarias AS t2 ON t2.id = t1.procedimientoID
		                                    WHERE t1.claveEmpleado = @claveEmpleado AND t1.procedimientoID = @procedimientoID AND t1.estatus = @estatus AND t2.estatus = @estatus",
                    parametros = new { claveEmpleado = claveEmpleado, procedimientoID = procedimientoID, estatus = true }
                }).Count();

                string infraccion = string.Empty;
                int idInfraccion = 0;
                switch (CantActos_ClaveEmpleado)
                {
                    case 0:
                        infraccion = "Amonestación";
                        idInfraccion = 1;
                        break;
                    case 1:
                        infraccion = "Suspensión";
                        idInfraccion = 2;
                        break;
                    case 2:
                        infraccion = "Rescisión";
                        idInfraccion = 3;
                        break;
                    case 3:
                        infraccion = "Sanción";
                ...
                resultado.Add(SUCCESS, true);
                resultado.Add("infraccion", infraccion);
                resultado.Add("idInfraccion", idInfraccion);
                #endregion
            }
            catch (Exception e)
            {
                LogError(0, 0, _NOMBRE_CONTROLADOR, "GetSiguienteInfraccion", e, AccionEnum.CONSULTA, 0, 0);
                resultado.Add(MESSAGE, e.Message);
                resultado.Add(SUCCESS, false);
            }
            return resultado;
        ...
        public Dictionary<string, object> GetPermisos()
        {
            ...
            try
            {
                bool puedeCambiarAccion = false;
                 
                if (vSesiones.sesionUsuarioDTO.id == 79807 || vSesiones.sesionUsuarioDTO.idPerfil == 1)
                    puedeCambiarAccion = true;

                resultado.Add(SUCCESS, true);
                resultado.Add("puedeCambiarAccion", puedeCambiarAccion);
            }
            catch (Exception e)
            {
                LogError(0, 0, _NOMBRE_CONTROLADOR, "GetPermisos", e, AccionEnum.CONSULTA, 0, null);
                resultado.Add(SUCCESS, false);
                resultado.Add(MESSAGE, e.Message);
            }
            return resultado;
        ...
        public Dictionary<string, object> CargarDatosDashboard(FiltroDashboardDTO filtro)
        {
            try
            {
                ...
                var actosActivos = _context.tblRH_AC_Acto
                    //.Where(x =>
                ...
                #region COMENTADO
                List<GfxActoCondicionDTO> condicionesActivas = new List<GfxActoCondicionDTO>();
                //var condicionesActivas = _context.tblRH_AC_Condicion
                //    .Where(x =>
                //        (grupos.Count > 0 ? grupos.Contains(x.idEmpresa + "-" + x.idAgrupacion) : true) &&
                //        (filtro.claveSupervisor == 0 ? true : filtro.claveSupervisor == x.claveSupervisor) &&
                //        (filtro.departamentoID == 0 ? true : filtro.departamentoID == x.departamentoID) &&
            ...
            {
                resultado.Add(SUCCESS, false);
                LogError(0, 0, _NOMBRE_CONTROLADOR, "CargarDatosDashboard", e, AccionEnum.CONSULTA, 0, null);
        ...
        public Dictionary<string, object> obtenerGraficaTotalDep(FiltroDashboardDTO filtro)
        {
            ...
            try
            {
                ...
                var departamentos = _context.tblRH_AC_Departamentos.ToList();

                #region Condicion
                var condiciones = _context.tblRH_AC_Condicion.Where(x =>
                        (grupos.Count > 0 ? grupos.Contains(x.idEmpresa + "-" + x.idAgrupacion) : true) &&
                        (filtro.claveSupervisor == 0 ? true : filtro.claveSupervisor == x.claveSupervisor) &&
                        (filtro.departamentoID == 0 ? true : filtro.departamentoID == x.departamentoID) &&
                ...
                foreach (var item in departamentos)
                {
                    data.Add(new
                    {
                        ...
                        y = retornarPorcentaje(condiciones.Where(x => x.departamentoID == item.id && x.estatus == EstatusActoCondicionCH.Completo).Count(), condiciones.Where(x => x.departamentoID == item.id).Count())
                    });
                }
                ...
                #region Acto
                var actos = _context.tblRH_AC_Acto.Where(x =>
                        (grupos.Count > 0 ? grupos.Contains(x.idEmpresa + "-" + x.idAgrupacion) : true) &&
                        (filtro.claveSupervisor == 0 ? true : filtro.claveSupervisor == x.claveSupervisor) &&
                        (filtro.departamentoID == 0 ? true : filtro.departamentoID == x.departamentoID) &&
                        (x.fechaSuceso >= filtro.fechaInicial && x.fechaSuceso <= filtro.fechaFinal) &&
                        x.activo &&
                        x.tipoActo == TipoActoCH.Inseguro)
                    .Select(x => new GfxActoCondicionDTO
                    {
                        fechaSuceso = x.fechaSuceso,
                        departamentoID = x.departamentoID,
                ...
                decimal incompletos = actos.Where(x => x.estatus != EstatusActoCondicionCH.Completo).Count();
                decimal porcentajeIncompleto = (incompletos / (decimal)actos.Count) * 100.00M;
                dataActo.Add(new
                {
                    name = "En proceso",
                ...
                #region vacunación
                var actosVacunacion = _context.tblRH_AC_Acto.Where(x =>
                        (grupos.Count > 0 ? grupos.Contains(x.idEmpresa + "-" + x.idAgrupacion) : true) &&
                        (filtro.claveSupervisor == 0 ? true : filtro.claveSupervisor == x.claveSupervisor) &&
                        (filtro.departamentoID == 0 ? true : filtro.departamentoID == x.departamentoID) &&
                        (x.fechaSuceso >= filtro.fechaInicial && x.fechaSuceso <= filtro.fechaFinal) &&
                    ...
                    .Select(x => new GfxActoCondicionDTO
                    {
                        ...
                        //nombreAgrupacion = x.idEmpresa == 0 ? x.agrupacion.nomAgrupacion : "" // OMAR
                    }).ToList();
                ...
                #region vacunacion de todos los empleados
                {
                    ...
                    foreach (var item in gbAgrupacion)
                    {
                        ...
                        //var empleadosAgrupacion = ObtenerTodosLosEmpleadosDeLaAgrupacion(item.Key, listaAgrupacion); // OMAR

                        //empleadosAgrupacion.RemoveAll(x => item.Select(y => y.clave_empleado).Contains(x.clave_empleado)); // OMAR

                        //var total = empleadosAgrupacion.Count + item.Count(); // OMAR
                        var total = 0;
            ...
            {
                LogError(0, 0, _NOMBRE_CONTROLADOR, "CargarDatosArchivo", e, AccionEnum.CONSULTA, 0, null);
                resultado.Add(SUCCESS, false);
            }
            return resultado;
        }
        ...
        private List<EmpleadoAgrupacionDTO> ObtenerTodosLosEmpleadosDeLaAgrupacion(int idAgrupacion, List<tblS_IncidentesAgrupacionCC> catAgrupacion)
        {
            ...
            var resultados = new List<EmpleadoAgrupacionDTO>();
            foreach (var empresa in empresas)
            {
                var resultado = _context.Select<EmpleadoAgrupacionDTO>(new DapperDTO
                {
                    baseDatos = (MainContextEnum)empresa,
                    consulta = @"SELECT
                                dep.cc,
...
//            return resultadoConstruplan;
        ...
        public Dictionary<string, object> GetDashboard(int anio)
        {
            ...
            {
                // LISTADO ACTOS
                List<tblRH_AC_Acto> lstActos = _context.tblRH_AC_Acto.Where(w =>  w.activo).ToList();
                ...
                if (vSesiones.sesionUsuarioDTO.idPerfil == (int)PerfilUsuarioEnum.USUARIO)
                {
                    ...
                    lstActos = _context.tblRH_AC_Acto.Where(w => w.activo && usuarioCCs.Contains(w.cc)).ToList();
                }
                else
                {
                    lstActos = _context.tblRH_AC_Acto.Where(w => w.activo).ToList();
                }   

                #region SE OBTIENE TODAS LAS ACTAS (DASHBOARD GLOBAL)
                List<GraficaDTO> lstGraficaTotalActasDTO = new List<GraficaDTO>();
                ...
                if (lstActos.Count() > 0)
                {
                    ...
                    #region ENERO
                    contadorMes++;
                    DateTime fechaInicio = new DateTime(anio > 0 ? anio : DateTime.Now.Year, contadorMes, 01);
                    DateTime fechaFin = new DateTime(fechaInicio.Year, contadorMes, DateTime.DaysInMonth(fechaInicio.Year, contadorMes));
                    objGraficaDTO = new GraficaDTO();
                    objGraficaDTO.name = "Enero";
                    objGraficaDTO.y = lstActos.Where(w => w.fechaSuceso >= fechaInicio && w.fechaSuceso <= fechaFin).Count();
                    objGraficaDTO.drilldown = "Enero";
                    lstGraficaTotalActasDTO.Add(objGraficaDTO);
                    #endregion

                    #region FEBRERO
                    contadorMes++;
                    fechaInicio = new DateTime(anio > 0 ? anio : DateTime.Now.Year, contadorMes, 01);
                    fechaFin = new DateTime(fechaInicio.Year, contadorMes, DateTime.DaysInMonth(fechaInicio.Year, contadorMes));
                    objGraficaDTO = new GraficaDTO();
                    objGraficaDTO.name = "Febrero";
                    objGraficaDTO.y = lstActos.Where(w => w.fechaSuceso >= fechaInicio && w.fechaSuceso <= fechaFin).Count();
                    objGraficaDTO.drilldown = "Febrero";
                    lstGraficaTotalActasDTO.Add(objGraficaDTO);
                    #endregion
                    ...
                    objGraficaDTO.y = lstActos.Where(w => w.fechaSuceso >= fechaInicio && w.fechaSuceso <= fechaFin).Count();
                    objGraficaDTO.drilldown = "Agosto";
                    lstGraficaTotalActasDTO.Add(objGraficaDTO);
                    #endregion
                    ...
                    objGraficaDTO.y = lstActos.Where(w => w.fechaSuceso >= fechaInicio && w.fechaSuceso <= fechaFin).Count();
                    objGraficaDTO.drilldown = "Septiembre";
                    lstGraficaTotalActasDTO.Add(objGraficaDTO);
                    #endregion
                    ...
                    objGraficaDTO.y = lstActos.Where(w => w.fechaSuceso >= fechaInicio && w.fechaSuceso <= fechaFin).Count();
                    objGraficaDTO.drilldown = "Octubre";
                    lstGraficaTotalActasDTO.Add(objGraficaDTO);
                    #endregion
                    ...
                    objGraficaDTO.y = lstActos.Where(w => w.fechaSuceso >= fechaInicio && w.fechaSuceso <= fechaFin).Count();
                    objGraficaDTO.drilldown = "Noviembre";
                    lstGraficaTotalActasDTO.Add(objGraficaDTO);
                    #endregion
                    ...
                    objGraficaDTO.y = lstActos.Where(w => w.fechaSuceso >= fechaInicio && w.fechaSuceso <= fechaFin).Count();
                    objGraficaDTO.drilldown = "Diciembre";
                    lstGraficaTotalActasDTO.Add(objGraficaDTO);
                    #endregion
                }
                #endregion

                #region SE OBTIENE LOS CC Y SU CANTIDAD DE ACTAS POR CC
                List<GraficaDTO> lstGraficaActasPorCC = new List<GraficaDTO>();
                objGraficaDTO = new GraficaDTO();
                foreach (var item in lstActos.Select(s => s.cc).ToList())
                {
                    objGraficaDTO = new GraficaDTO();
                    objGraficaDTO.name = item;
                    objGraficaDTO.y = lstActos.Where(w => w.cc == item).Count();
                    objGraficaDTO.drilldown = item;
                    lstGraficaActasPorCC.Add(objGraficaDTO);
                }
                #endregion

                #region SE OBTIENE LA CANTIDAD DE CLASIFICACIONES REGISTRADAS
                List<GraficaDTO> lstGraficaCantActasPorClasificaciones = new List<GraficaDTO>();
                objGraficaDTO = new GraficaDTO();
                foreach (var item in lstActos.Select(s => s.accionID).ToList())
                {
                    objGraficaDTO = new GraficaDTO();
                    objGraficaDTO.name = GetClasificacion(item);
                    objGraficaDTO.y = lstActos.Where(w => w.accionID == item).Count();
                    objGraficaDTO.drilldown = objGraficaDTO.name;
                    lstGraficaCantActasPorClasificaciones.Add(objGraficaDTO);
                }
                #endregion

                #region SE OBTIENE EL TIPO DE INFRACCIÓN EN BASE A LAS CLASIFICACIONES REGISTRADAS
                List<MatrizDTO> lstInfraccionesClasificacion = _context.Select<MatrizDTO>(new DapperDTO
                {
                    baseDatos = (MainContextEnum)vSesiones.sesionEmpresaActual,
                    consulta = @"SELECT t1.cc, t3.descripcion AS clasificacion, t2.tipoInfraccion AS infraccion
	                                        FROM tblRH_AC_Acto AS t1
	                                        INNER JOIN tblRH_AC_MatrizAccionesDisciplinarias AS t2 ON t2.id = t1.accionID
	                                        INNER JOIN tblRH_AC_Clasificacion AS t3 ON t3.id = t2.FK_Clasificacion
		                                        WHERE t1.estatus = @estatus AND t2.estatus = @estatus",
                    parametros = new { estatus = true }
                }).ToList();
                #endregion

                resultado.Add(SUCCESS, true);
                resultado.Add("lstGraficaTotalActasDTO", lstGraficaTotalActasDTO);
                resultado.Add("lstGraficaActasPorCC", lstGraficaActasPorCC);
                resultado.Add("lstGraficaCantActasPorClasificaciones", lstGraficaCantActasPorClasificaciones);
                resultado.Add("lstInfraccionesClasificacion", lstInfraccionesClasificacion);
            }
            catch (Exception e)
            {
                LogError(0, 0, _NOMBRE_CONTROLADOR, "GetDashboard", e, AccionEnum.CONSULTA, 0, 0);
                resultado.Add(MESSAGE, e.Message);
                resultado.Add(SUCCESS, false);
            }
            return resultado;
        ...
        public Dictionary<string, object> ObtenerHistorialEmpleado(int claveEmpleado)
        {
            try
            {
                List<tblRH_AC_Acto> objActo = new List<tblRH_AC_Acto>();

                if (vSesiones.sesionUsuarioDTO.idPerfil == (int)PerfilUsuarioEnum.USUARIO)
                {
                    var usuarioCCs = _context.tblRH_BN_Usuario_CC.Where(x => x.usuarioID == vSesiones.sesionUsuarioDTO.id).Select(x => x.cc).ToList();
                    objActo = _context.tblRH_AC_Acto.Where(x => x.claveEmpleado == claveEmpleado && x.activo && usuarioCCs.Contains(x.cc)).ToList();
                }
                else 
                {
                    objActo = _context.tblRH_AC_Acto.Where(x => x.claveEmpleado == claveEmpleado && x.activo).ToList();
                }      

                List<HistorialEmpleadoDTO> lstHistorial = new List<HistorialEmpleadoDTO>();
                HistorialEmpleadoDTO objHistorial = new HistorialEmpleadoDTO();
                foreach (var item in objActo)
                {
                    objHistorial = new HistorialEmpleadoDTO();
                    objHistorial.claveEmpleado = item.claveEmpleado;
                    objHistorial.proyecto = GetDescripcionCC(item.cc);
                    objHistorial.departamentoDesc = GetDepartamento(item.departamentoID);
                    objHistorial.procedimientoDesc = GetProcedimiento(item.procedimientoID);
                    objHistorial.id = item.id;
                    objHistorial.folio = item.folio;
                    objHistorial.tipoActo = item.tipoActo;
                    objHistorial.accionDesc = item.accion.descripcion;
                    objHistorial.clasificacionDesc = item.clasificacion.descripcion;
                    objHistorial.fechaSucesoDT = item.fechaSuceso;
                    objHistorial.tieneEvidencia = !string.IsNullOrEmpty(item.rutaEvidencia);
                    objHistorial.tipoActoDesc = item.tipoActo.GetDescription();
                    objHistorial.fechaSuceso = objHistorial.fechaSucesoDT.ToString("yyyy/MM/dd");
                    lstHistorial.Add(objHistorial);
                }
                ...
                resultado.Add(SUCCESS, true);
                resultado.Add(ITEMS, lstHistorial);
            }
            catch (Exception e)
            {
                resultado.Add(SUCCESS, false);
                LogError(0, 0, _NOMBRE_CONTROLADOR, "ObtenerHistorialEmpleado", e, AccionEnum.CONSULTA, 0, null);
        ...
        public Tuple<Stream, string> DescargarActo(int actoID)
        {
            try
            {
                string rutaArchivo = ObtenerRutaFisicaArchivo(actoID, TipoRiesgoCH.Acto, TipoArchivo.Evidencia);
            ...
            {
                LogError(0, 0, _NOMBRE_CONTROLADOR, "DescargarActo", e, AccionEnum.DESCARGAR, actoID, 0);
                return null;
            }
        }
        ...
        public Dictionary<string, object> ObtenerMatrices(FiltroDashboardDTO filtro)
        {
            try
            {
                ...
                var clasificaciones = _context.tblRH_AC_Clasificacion.ToList();
                var subclasificaciones = _context.tblRH_AC_SubclasificacionDepartamentos.Where(x => x.registroActivo).ToList();
                ...
                if (filtro.ccs != null)
                    lstCC = filtro.ccs.Select(s => s).ToList();

                List<tblRH_AC_Acto> lstActos = _context.Select<tblRH_AC_Acto>(new DapperDTO
                {
                    baseDatos = (MainContextEnum)vSesiones.sesionEmpresaActual,
                    consulta = @"SELECT * FROM tblRH_AC_Acto WHERE activo = @activo",
                    parametros = new { activo = true }
                }).ToList();

                List<ActoExcelDTO> lstExcelDTO = new List<ActoExcelDTO>();
                ActoExcelDTO objExcelDTO = new ActoExcelDTO();
                foreach (var item in lstActos)
                {
                    objExcelDTO = new ActoExcelDTO();
                    objExcelDTO.claveEmpleado = item.claveEmpleado;
                    objExcelDTO.nombreEmpleado = item.nombre;
                    objExcelDTO.folio = item.folio.ToString();
                    objExcelDTO.fechaSucesoDT = item.fechaSuceso;
                    objExcelDTO.claveEmpleado = item.claveEmpleado;
                    objExcelDTO.nombreEmpleado = item.nombre;
                    objExcelDTO.puestoEmpleado = item.puesto;
                    objExcelDTO.proyecto = GetDescripcionCC(item.cc);
                    objExcelDTO.departamento = GetDepartamento(item.departamentoID);
                    objExcelDTO.tipoActoEnum = item.tipoActo;
                    objExcelDTO.accionDesc = GetAccion(item.accionID);
                    objExcelDTO.descripcion = item.descripcion;
                    objExcelDTO.clasificacion = GetClasificacion(item.clasificacionID);
                    objExcelDTO.subClasificacionDepID = item.subclasificacionDepID;
                    objExcelDTO.empleadoInformo = item.nombreInformo;
                    lstExcelDTO.Add(objExcelDTO);
                }
                lstExcelDTO.OrderByDescending(o => o.fechaSucesoDT).ToList();

                //var actos = _context.tblRH_AC_Acto.Where(x =>
                //    x.fechaSuceso >= filtro.fechaInicial &&
                //    x.fechaSuceso <= filtro.fechaFinal &&
                //    (lstCC.Count() > 0 ? lstCC.Contains(x.cc) : true) &&
                //    (filtro.claveSupervisor == 0 ? true : filtro.claveSupervisor == x.claveSupervisor) &&
                ...
                //    fechaSucesoDT = x.fechaSuceso,
                //    //proyecto = x.cc + " - " + x.agrupacion.nomAgrupacion.ToUpper(), // OMAR
                //    claveEmpleado = x.claveEmpleado,
                ...
                //    //procedimiento = x.procedimientoViolado.Procedimineto, // OMAR
                //    subClasificacionDepID = x.subclasificacionDepID,
                ...
                List<tblRH_AC_Condicion> condiciones = new List<tblRH_AC_Condicion>();
                //var condiciones = _context.tblRH_AC_Condicion.Where(x =>
                //    x.fechaSuceso >= filtro.fechaInicial &&
                //    x.fechaSuceso <= filtro.fechaFinal &&
                //    (grupos.Count > 0 ? grupos.Contains(x.idEmpresa + "-" + x.idAgrupacion) : true) &&
                ...
                //    folio = x.folio.ToString(),
                //    fechaSucesoDT = x.fechaSuceso,
                //    //proyecto = x.cc + " - " + x.agrupacion.nomAgrupacion, // OMAR
                //    departamento = x.departamento.descripcion,
                //    descripcion = x.descripcion,
                //    clasificacion = x.clasificacion.descripcion,
                //    //procedimiento = x.procedimientoViolado.Procedimineto, // OMAR
                ...
                HttpContext.Current.Session["actos"] = lstExcelDTO;
                HttpContext.Current.Session["condiciones"] = condiciones;

                resultado.Add("actos", lstExcelDTO);
            ...
            {
                resultado.Add(SUCCESS, false);
                LogError(0, 0, _NOMBRE_CONTROLADOR, "ObtenerMatrices", e, AccionEnum.CONSULTA, 0, null);
                resultado.Add(MESSAGE, "Ocurrió un error al consultar la información.");
            }
        ...
        public Tuple<MemoryStream, string> DescargarExcelMatrizActos()
        {
            ...
            {
                LogError(0, 0, _NOMBRE_CONTROLADOR, "DescargarExcelMatrizActos", e, AccionEnum.DESCARGAR, 0, 0);
                return null;
            }
        }
        ...
        public Tuple<MemoryStream, string> DescargarExcelMatrizCondiciones()
        {
            ...
            {
                LogError(0, 0, _NOMBRE_CONTROLADOR, "DescargarExcelMatrizCondiciones", e, AccionEnum.DESCARGAR, 0, 0);
                return null;
            }
        }
        ...
        public Dictionary<string, object> GetListadoArchivosAdjuntos(int FK_Acto)
        {
            ...
            {
                #region VALIDACIONES
                if (FK_Acto <= 0) { throw new Exception("Ocurrió un error al obtener el listado de archivos adjuntos del acto seleccionado."); }
                #endregion

                #region SE OBTIENE LISTADO DE ARCHIVOS ADJUNTOS DEL ACTO SELECCIONADO
                List<tblRH_AC_ActoArchivos> lstArchivos = _context.tblRH_AC_ActoArchivos.Where(w => w.FK_Acto == FK_Acto && w.registroActivo).ToList();
            ...
            {
                LogError(0, 0, _NOMBRE_CONTROLADOR, "GetListadoArchivosAdjuntos", e, AccionEnum.CONSULTA, 0, new { FK_Acto = FK_Acto });
                resultado.Add(MESSAGE, e.Message);
                resultado.Add(SUCCESS, false);
            }
            return resultado;
        ...
        public Dictionary<string, object> GetMatriz()
        {
            ...
            try
            {
                #region SE OBTIENE LISTADO DE LA MATRIZ
                List<MatrizDTO> lstMatriz = _context.Select<MatrizDTO>(new DapperDTO
                {
                    baseDatos = (MainContextEnum)vSesiones.sesionEmpresaActual,
                    consulta = @"SELECT * FROM tblRH_AC_MatrizAccionesDisciplinarias WHERE estatus = @estatus",
                    parametros = new { estatus = true }
                }).ToList();

                foreach (var item in lstMatriz)
                {
                    #region NO MATRIZ
                    item.noMatriz = item.id;
                    #endregion

                    #region AMONESTACIÓN
                    if (item.amonestacion == 1)
                        item.amonestacionDescripcion = "1ra falta";
                    #endregion

                    #region SUSPENCIÓN
                    if (item.suspension == 2)
                        item.suspensionDescripcion = "2da falta";
                    #endregion

                    #region RECISIÓN
                    if (item.rescision == 3)
                        item.recisionDescripcion = "3ra falta";
                    #endregion

                    #region SANCIÓN
                    if (item.sancion > 0)
                        item.sancionDescripcion = string.Format("{0} {1}", item.sancion, item.sancion > 1 ? "meses" : "mes");
                    else
                        item.sancionDescripcion = "Lista negra";
                    #endregion
                }

                resultado.Add(SUCCESS, true);
                resultado.Add("lstMatriz", lstMatriz);
                #endregion
            }
            catch (Exception e)
            {
                LogError(0, 0, _NOMBRE_CONTROLADOR, "GetMatriz", e, AccionEnum.CONSULTA, 0, 0);
                resultado.Add(SUCCESS, false);
                resultado.Add(MESSAGE, e.Message);
            }
            return resultado;
        ...
        public Dictionary<string, object> GenerarReporte(ReporteActoCondicionCH objParametros)
        {
            ...
            try
            {
                ...
                if (objParametros.idTipoReporte <= 0) { throw new Exception(mensajeError); }
                if (objParametros.idActo <= 0) { throw new Exception(mensajeError); }
                if (objParametros.idTipoReporte != (int)TipoReporteEnum.ACTA_ADMINISTRATIVA && string.IsNullOrEmpty(objParametros.fechaReporte)) { throw new Exception(mensajeError); }
                if (string.IsNullOrEmpty(objParametros.parrafoUno)) { throw new Exception(mensajeError); }
                #endregion

                // SE OBTIENE LA INFORMACIÓN DEL ACTO
                tblRH_AC_Acto objActo = _context.tblRH_AC_Acto.Where(w => w.id == objParametros.idActo && w.activo).FirstOrDefault();
                if (objActo == null)
                    throw new Exception(mensajeError);

                switch (objParametros.idTipoReporte)
                {
                    case (int)TipoReporteEnum.AMONESTACION:
                        #region REPORTE AMONESTACION
                        resultado.Add("fechaReporte", SetReporteFormatoConCiudad(objParametros.fechaReporte));
                        resultado.Add("nombreTrabajador", GetNombreUsuario(objActo.claveEmpleado));
                        resultado.Add("razonSocialEmpresa", PersonalUtilities.GetNombreEmpresa());
                        resultado.Add("parrafoUno", !string.IsNullOrEmpty(objParametros.parrafoUno) ? objParametros.parrafoUno.Trim() : string.Empty);
                        resultado.Add("nombreQuienAplica", GetNombreUsuario(objActo.claveInformo));
                        #endregion
                        break;
                    case (int)TipoReporteEnum.SUSPENSION:
                        #region REPORTE SUSPENSIÓN
                        resultado.Add("fechaReporte", SetReporteFormatoConCiudad(objParametros.fechaReporte));
                        resultado.Add("nombreTrabajador", GetNombreUsuario(objActo.claveEmpleado));
                        resultado.Add("parrafoUno", objParametros.parrafoUno.Trim());
                        resultado.Add("razonSocialEmpresa", PersonalUtilities.GetNombreEmpresa());
                        #endregion
                        break;
                    case (int)TipoReporteEnum.CARTA_RESPONSABILIDAD:
                        #region REPORTE CARTA RESPONSABILIDAD
                        resultado.Add("fechaReporte", SetReporteFormatoConCiudad(objParametros.fechaReporte));
                        resultado.Add("nombreTrabajador", GetNombreUsuario(objActo.claveEmpleado));
                        resultado.Add("razonSocialEmpresa", PersonalUtilities.GetNombreEmpresa());
                        resultado.Add("parrafoUno", !string.IsNullOrEmpty(objParametros.parrafoUno) ? objParametros.parrafoUno : string.Empty);
                        resultado.Add("nombreQuienAplica", GetNombreUsuario(objActo.claveInformo));
                        #endregion
                        break;
                    case (int)TipoReporteEnum.ACTA_ADMINISTRATIVA:
            ...
            {
                LogError(0, 0, _NOMBRE_CONTROLADOR, "GenerarReporte", e, AccionEnum.CONSULTA, 0, objParametros);
                resultado.Add(MESSAGE, e.Message);
                resultado.Add(SUCCESS, false);
            }
            return resultado;
        ...
        public Dictionary<string, object> DescargarActa(int idActo, string ciudad, string articulos)
        {
            ...
            try
            {
                #region ACTO Y CONDICION SEGURIDAD
                DateTime fecha = DateTime.Now;
                tblRH_AC_Acto objActo = _context.tblRH_AC_Acto.Where(w => w.id == idActo).FirstOrDefault();

                string claveEmpleado = objActo.claveInformo.ToString();
                var usuarioCH = _context.tblP_Usuario.Where(w => w.cveEmpleado == claveEmpleado).FirstOrDefault();
                var sexoCH = InfoEmpleado(objActo.claveEmpleado).sexo;

                int claveEmpleadoINT = Convert.ToInt32(claveEmpleado);
                string nombreCH = string.Format("{0} {1}", sexoCH == "M" ? "el C." : "la C.", GetNombreUsuario(claveEmpleadoINT));
                //var nombreCH = (sexoCH == "M" ? "el C. " : "la C. ") +
                //    usuarioCH.nombre + (!string.IsNullOrEmpty(usuarioCH.apellidoPaterno) ? " " : "") +
                //    (usuarioCH.apellidoPaterno ?? "") +
                //    (!string.IsNullOrEmpty(usuarioCH.apellidoMaterno) ? " " : "") +
                ...
                var sexoEmpleado = InfoEmpleado(objActo.claveEmpleado).sexo;
                var nombreEmpleado = (sexoEmpleado == "M" ? "el C. " : "la C. ") + objActo.nombre;

                var infraccion = _context.tblRH_AC_MatrizAccionesDisciplinarias.First(x => x.numero == objActo.numeroInfraccion);

                var hora = fecha.ToString("t");
                var dia = fecha.ToString("D");
                var empresa = _context.tblP_Encabezado.First(x => x.id == vSesiones.sesionEmpresaActual).nombreEmpresa;
                var puesto = objActo.puesto;

                var infoActa = new Core.DTO.RecursosHumanos.ActoCondicion.Acta.ActaAdministrativaDTO();
                infoActa.parrafo1 = string.Format("" +
                    "<div align=\"justify\">En {7}, siendo las {0} del día {1}, se reunieron en las instalaciones de <b>{2}</b>, " +
                    "<b>{3}</b>, en calidad de Coordinador de Relaciones Laborales de la empresa {4}, así como <b>{5}</b> que ostenta el cargo de <b>" +
                    "{6}</b> de la empresa ya mencionada, a efecto de hacer constar los siguientes:</div>",
                    hora, dia, empresa, nombreCH, empresa, nombreEmpleado, puesto, ciudad);

                infoActa.compromiso = string.Format("" +
                    "<div align=\"justify\"><p>Yo {0}, {1}.</p><p>Por otro lado y no habiendo nada más que hacer constar en la presente acta circunstanciada, " +
                    "se da por concluida la cual se levanta por duplicado, siendo las {2} horas del día {3}, " +
                    "firman al calce y al margen los que en ella intervinieron, para constancia, damos fe.</p></div>",
                    nombreEmpleado, objActo.compromiso, hora, dia);

                tblRH_AC_Acto objActoCH = _context.tblRH_AC_Acto.Where(w => w.id == idActo).FirstOrDefault();
                if (objActoCH == null)
                    throw new Exception("Ocurrió un error al generar el reporte.");

                // SE OBTIENE LA INFRACCIÓN DEL ACTO
                int Infraccion_ID = _context.tblRH_AC_Acto.Where(w => w.id == idActo).Select(s => s.accionID).FirstOrDefault();
                if (Infraccion_ID <= 0)
                    throw new Exception("Ocurrió un error al obtener los articulos.");

                // SE OBTIENE FK_Infraccion
                List<int> lstInfraccionID = _context.Select<int>(new DapperDTO
                {
                    baseDatos = (MainContextEnum)vSesiones.sesionEmpresaActual,
                    consulta = @"SELECT FK_Infraccion FROM tblRH_AC_MatrizRelArticuloDet WHERE FK_Matriz = @FK_Matriz AND registroActivo = @registroActivo",
                    parametros = new { FK_Matriz = Infraccion_ID, registroActivo = true }
                }).ToList();
                string strInfraccionesID = string.Empty;
                foreach (var item in lstInfraccionID)
                {
                    if (string.IsNullOrEmpty(strInfraccionesID))
                        strInfraccionesID = item.ToString();
                    else
                        strInfraccionesID += string.Format(", {0}", item.ToString());
                }

                // SE OBTIENE LAS INFRACCIONES
                string strQuery = string.Format("SELECT FK_Articulo, infraccion FROM tblRH_AC_CatInfracciones WHERE id IN ({0}) AND registroActivo = {1} ORDER BY FK_Articulo", strInfraccionesID, 1);
                List<InfraccionDTO> lstInfracciones = _context.Select<InfraccionDTO>(new DapperDTO
                {
                    baseDatos = (MainContextEnum)vSesiones.sesionEmpresaActual,
                    consulta = strQuery
                }).ToList();
                ...
                bool articulo47 = false;
                bool articulo134 = false;
                bool articulo135 = false;
                if (vSesiones.sesionEmpresaActual == (int)EmpresaEnum.GCPLAN || vSesiones.sesionEmpresaActual == (int)EmpresaEnum.Construplan)
                {
                    foreach (var item in lstInfracciones)
                    {
                        switch (item.FK_Articulo)
                        {
                            case 1:
                                if (string.IsNullOrEmpty(Articulos))
                                {
                                    Articulos = string.Format("{0} 47 Fracción {1}", lstInfracciones.Where(w => w.FK_Articulo == 1).Count() > 1 ? "artículos" : "artículo", item.infraccion);
                                    articulo47 = true;
                                }
                                else
                                    Articulos += string.Format(", {0}", item.infraccion);
                                break;
                            case 2:
                                if (string.IsNullOrEmpty(Articulos))
                                    Articulos = string.Format("{0} 134 Fracción {1}", lstInfracciones.Where(w => w.FK_Articulo == 2).Count() > 1 ? "artículos" : "artículo", item.infraccion);
                                else
                                {
                                    if (articulo47 && !articulo134)
                                    {
                                        Articulos += string.Format(" && {0} 134 Fracción {1}", lstInfracciones.Where(w => w.FK_Articulo == 2).Count() > 1 ? "artículos" : "artículo", item.infraccion);
                                        articulo134 = true;
                                    }
                            ...
                            case 3:
                                if (string.IsNullOrEmpty(Articulos))
                                    Articulos = string.Format("{0} 135 Fracción {1}", lstInfracciones.Where(w => w.FK_Articulo == 3).Count() > 1 ? "artículos" : "artículo", item.infraccion);
                                else
                                {
                                    if (articulo134 && !articulo135)
                                    {
                                        Articulos += string.Format(" && {0} 135 Fracción {1}", lstInfracciones.Where(w => w.FK_Articulo == 3).Count() > 1 ? "artículos" : "artículo", item.infraccion);
                                        articulo135 = true;
                                    }
                ...
                infoActa.declaraciones = string.Format("" +
                    ...
                    "quien manifestó llamarse como ha quedado escrito y en el puesto mencionado declara: Yo {4}, (falta la declaración del trabajador) que si incurrí " +
                    "en no realizar mi trabajo con esmero y el no cumplir con los compromisos establecidos.</p></div>",
                    nombreCH, nombreEmpleado, Articulos, nombreEmpleado, nombreEmpleado);
                ...
                infoActa.hechos = string.Format("" +
                    "<div align=\"justify\">El día {0}, en el área de trabajo de la empresa {1}, se presentó una situación donde {2}, {3}</div>",
                    objActo.fechaSuceso.ToString("D"), empresa, nombreEmpleado, objActo.descripcion);
                ...
                resultado.Add(SUCCESS, true);
                resultado.Add(ITEMS, infoActa);
                #endregion ACTO Y CONDICION SEGURIDAD
            }
            catch (Exception e)
            {
        ...
        public Dictionary<string, object> GetContenidoActaAdministrativa(int idActo)
        {
            ...
            {
                #region VALIDACIONES
                string mensajeError = "Ocurrió un error al pre cargar la información del reporte.";
                if (idActo <= 0) { throw new Exception(mensajeError); }
                #endregion
                ...
                resultado.Add(SUCCESS, true);
                resultado.Add("objArticulos", Articulos);
                #endregion
            }
            catch (Exception e)
            {
                LogError(0, 0, _NOMBRE_CONTROLADOR, "GetPuestoEmpleadoSIGOPLAN", e, AccionEnum.CONSULTA, 0, 0);
                resultado.Add(MESSAGE, e.Message);
                resultado.Add(SUCCESS, false);
            }
            return resultado;
        ...
        public Dictionary<string, object> GetPuestoEmpleado(int idActo)
        {
            ...
            {
                #region VALIDACIONES
                string mensajeError = "Ocurrió un error al obtener el puesto del empleado.";
                if (idActo <= 0) { throw new Exception(mensajeError); }
                #endregion

                #region SE OBTIENE EL PUESTO DEL EMPLEADO
                string puesto = _context.Select<string>(new DapperDTO
                {
                    baseDatos = (MainContextEnum)vSesiones.sesionEmpresaActual,
                    consulta = @"SELECT t3.descripcion
	                                    FROM tblRH_AC_Acto AS t1
	                                    INNER JOIN tblRH_EK_Empleados AS t2 ON t2.clave_empleado = t1.claveEmpleado
	                                    INNER JOIN tblRH_EK_Puestos AS t3 ON t3.puesto = t2.puesto
		                                    WHERE t1.id = @idActo",
                    parametros = new { idActo = idActo }
                }).FirstOrDefault();
                if (string.IsNullOrEmpty(puesto))
                    throw new Exception(mensajeError);

                string parrafoUno = string.Format("En mi carácter de trabajador de esa empresa, desempeñando actualmente el puesto de {0}, por este conducto, es mi voluntad aclarar y precisar lo siguiente:", puesto);

                resultado.Add(SUCCESS, true);
                resultado.Add("parrafoUno", parrafoUno);
                #endregion
            }
            catch (Exception e)
            {
                LogError(0, 0, _NOMBRE_CONTROLADOR, "GetPuestoEmpleado", e, AccionEnum.CONSULTA, 0, 0);
                resultado.Add(MESSAGE, e.Message);
                resultado.Add(SUCCESS, false);
            }
            return resultado;
        ...
        private string GetPuestoEmpleadoSIGOPLAN(int idActo)
        {
        ...
        private int GetUsuarioID(string claveEmpleado)
        {
            ...
            {
                #region VALIDACIONES
                string mensajeError = "Ocurrió un error al obtener la clave del empleado.";
                if (string.IsNullOrEmpty(claveEmpleado)) { throw new Exception(mensajeError); }
                #endregion

                #region SE OBTIENE ID DEL USUARIO EN BASE A SU CLAVE EMPLEADO
            ...
            {
                LogError(0, 0, _NOMBRE_CONTROLADOR, "GetUsuarioID", e, AccionEnum.CONSULTA, 0, new { claveEmpleado = claveEmpleado });
                return idUsuario;
            }
            return idUsuario;
        }

        private string GetNombreUsuario(int claveEmpleado)
        {
            string nombreUsuario = string.Empty;
            try
            {
                #region VALIDACIONES
                string mensajeError = "Ocurrió un error al obtener el nombre del usuario.";
                if (claveEmpleado <= 0) { throw new Exception(mensajeError); }
                #endregion

                #region SE OBTIENE EL NOMBRE DEL USUARIO
                ...
                if (!string.IsNullOrEmpty(objEmpleado.nombre))
                    nombreUsuario = objEmpleado.nombre.Trim();
                if (!string.IsNullOrEmpty(objEmpleado.ape_paterno))
                    nombreUsuario += string.Format(" {0}", objEmpleado.ape_paterno.Trim());
                if (!string.IsNullOrEmpty(objEmpleado.ape_materno))
                    nombreUsuario += string.Format(" {0}", objEmpleado.ape_materno.Trim());
                #endregion
            }
            catch (Exception e)
            {
                LogError(0, 0, _NOMBRE_CONTROLADOR, "GetNombreUsuario", e, AccionEnum.CONSULTA, 0, new { claveEmpleado = claveEmpleado });
                return nombreUsuario;
            }
            return nombreUsuario;
        }

        private string SetReporteFormatoConCiudad(string ciudadEstado)
        {
            string fechaReporte = string.Empty;
            try
            {
                #region VALIDACIONES
                if (string.IsNullOrEmpty(ciudadEstado)) { throw new Exception("Es necesario indicar la ciudad."); }
                #endregion

                #region SE GENERA LA FECHA DEL REPORTE CONCATENANDO LA CIUDAD.

                fechaReporte = string.Format("{0}, a {1} de {2} de {3}", ciudadEstado, DateTime.Now.Day, SetPrimeraLetraMayuscula(DateTime.Now.ToString("MMMM")), DateTime.Now.Year);
                #endregion
            }
            catch (Exception e)
            {
                LogError(0, 0, _NOMBRE_CONTROLADOR, "SetReporteFormatoConCiudad", e, AccionEnum.CONSULTA, 0, new { ciudadEstado = ciudadEstado });
                return fechaReporte;
            }
            return fechaReporte;
        }
        ...
        private tblRH_AC_Condicion InicializarCondicion(CondicionDTO condicion)
        {
            var fechaSuceso = Convert.ToDateTime(condicion.fechaSuceso);
            return new tblRH_AC_Condicion
            {
        ...
        private tblRH_AC_Acto InicializarActo(ActoDTO acto)
        {
            ...
            return new tblRH_AC_Acto
            {
                ...
                hallazgo = acto.hallazgo
            };
        }

        private int ObtenerNuevoFolioActoCondicion(int idEmpresa, int idAgrupacion)
        {
            var ultimoFolioActo = _context.tblRH_AC_Acto.Where(x => x.idEmpresa == idEmpresa && x.idAgrupacion == idAgrupacion).Select(x => x.folio).DefaultIfEmpty().Max();
            var ultimoFolioCondicion = _context.tblRH_AC_Condicion.Where(x => x.idEmpresa == idEmpresa && x.idAgrupacion == idAgrupacion).Select(x => x.folio).DefaultIfEmpty().Max();
        ...
        private string ObtenerRutaFisicaArchivo(int sucesoID, TipoRiesgoCH tipoRiesgo, TipoArchivo tipoArchivo)
        {
            switch (tipoRiesgo)
            {
                case TipoRiesgoCH.Acto:

                    var acto = _context.tblRH_AC_Acto.First(x => x.id == sucesoID);

                    return acto.rutaEvidencia;

                case TipoRiesgoCH.Condicion:

                    var condicion = _context.tblRH_AC_Condicion.First(x => x.id == sucesoID);
        ...
        private void CargarGraficasAño(ref Dictionary<string, object> resultado, FiltroDashboardDTO filtro, List<GfxActoCondicionDTO> actos, List<GfxActoCondicionDTO> condiciones)
        {
            ...
            var condicionesAño = condiciones.ToList();

            List<tblS_IncidentesInformePreliminar> accidentesAño = new List<tblS_IncidentesInformePreliminar>();
        ...
        private ChartDataDTO ObtenerChartSucesosPorMes(IEnumerable<Tuple<string, int, int>> months, List<GfxActoCondicionDTO> actosAño, List<GfxActoCondicionDTO> condicionesAño)
        {
            ...
            var datasetActosSeguros = new DatasetDTO
            {
                ...
                borderWidth = 2,
                fill = true,
                data = months.Select(x => (decimal)actosAño.Where(y => y.fechaSuceso.Year == x.Item2 && y.fechaSuceso.Month == x.Item3 && y.tipoActo == TipoActoCH.Seguro).Count()).ToList()
            };
            ...
            var datasetActosInseguros = new DatasetDTO
            {
                ...
                borderWidth = 2,
                fill = true,
                data = months.Select(x => (decimal)actosAño.Where(y => y.fechaSuceso.Year == x.Item2 && y.fechaSuceso.Month == x.Item3 && y.tipoActo == TipoActoCH.Inseguro).Count()).ToList()
            };
        ...
        private ChartDataDTO ObtenerChartComportamiento(IEnumerable<Tuple<string, int, int>> months, List<GfxActoCondicionDTO> actosAño, List<GfxActoCondicionDTO> condicionesAño, List<tblS_IncidentesInformePreliminar> accidentesAño)
        {
            ...
            var datasetCondicionesSinCorreccion = new DatasetStackedDTO
            {
                ...
                borderWidth = 2,
                fill = true,
                data = months.Select(x => (decimal)condicionesAño.Where(y => y.fechaSuceso.Year == x.Item2 && y.fechaSuceso.Month == x.Item3 && y.estatus == EstatusActoCondicionCH.EnProceso).Count()).ToList(),
                stack = stackComportamientos
            };
            ...
            var datasetCondicionesCorregidas = new DatasetStackedDTO
            {
                ...
                borderWidth = 2,
                fill = true,
                data = months.Select(x => (decimal)condicionesAño.Where(y => y.fechaSuceso.Year == x.Item2 && y.fechaSuceso.Month == x.Item3 && y.estatus == EstatusActoCondicionCH.Completo).Count()).ToList(),
                stack = stackComportamientos
            };
            ...
            var acciones = _context.tblRH_AC_Accion.ToList();
            ...
            var datasetContactoPersonalPositivo = new DatasetStackedDTO
            {
                ...
                borderWidth = 2,
                fill = true,
                data = months.Select(x => (decimal)actosAño.Where(y => y.fechaSuceso.Year == x.Item2 && y.fechaSuceso.Month == x.Item3 && y.tipoActo == TipoActoCH.Seguro && y.accionID == 1).Count()).ToList(),
                stack = stackAcciones
            };
            ...
            foreach (var accion in acciones)
            {
                ...
                var datasetActoAccion = new DatasetStackedDTO
                {
                    ...
                    borderWidth = 2,
                    fill = true,
                    data = months.Select(x => (decimal)actosAño.Where(y => y.fechaSuceso.Year == x.Item2 && y.fechaSuceso.Month == x.Item3 && y.tipoActo == TipoActoCH.Inseguro && y.accionID == accion.id).Count()).ToList(),
                    stack = stackAcciones
                };
        ...
        {
            var departamentos = _context.tblRH_AC_Departamentos.ToList().Select(x => new KeyValuePair<int, string>(x.id, x.descripcion)).ToList();
            ...
            foreach (var departamento in departamentos)
            {
                ...
                var cantidadActosInseguros = actos.Where(x => x.departamentoID == departamento.Key && x.tipoActo == TipoActoCH.Inseguro).ToList();
        ...
        {
            var clasificacionesActo = _context.tblRH_AC_Clasificacion.Where(x => x.tipoRiesgo == TipoRiesgoCH.Acto && x.id != 31 && x.id != 32).OrderBy(x => x.descripcion).ToList();
            ...
            var datasetActosClasificacion = new DatasetDTO
            {
                ...
                borderWidth = 2,
                fill = true,
                data = clasificacionesActo.Select(x => (decimal)actosFiltroFecha.Where(y => y.clasificacionID == x.id && y.tipoActo == TipoActoCH.Inseguro).Count()).ToList()
            };
        ...
        {
            var clasificacionesCondicion = _context.tblRH_AC_Clasificacion.Where(x => x.tipoRiesgo == TipoRiesgoCH.Condicion).OrderBy(x => x.descripcion).ToList();
        ...
        public Dictionary<string, object> FillCboCC()
        {
            ...
            try
            {
                ...
                string mensajeError = "Ocurrió un error al obtener el listado de CC.";

                List<tblC_Nom_CatalogoCC> lstCC = _context.Select<tblC_Nom_CatalogoCC>(new DapperDTO
                {
                    baseDatos = (MainContextEnum)vSesiones.sesionEmpresaActual,
                    consulta = @"SELECT cc, ccDescripcion FROM tblC_Nom_CatalogoCC WHERE estatus = @estatus",
                    parametros = new { estatus = true }
                }).ToList();
                ...
                foreach (var item in lstCC)
                {
                    ...
                    {
                        objComboDTO = new ComboDTO();
                        objComboDTO.Value = item.cc.Trim();
        ...
        public Dictionary<string, object> FillCboDepartamentos(string cc)
        {
            ...
            {
                #region VALIDACIONES
                string mensajeError = "Ocurrió un error al obtener los departamentos.";
                if (string.IsNullOrEmpty(cc)) { throw new Exception(mensajeError); }
                #endregion

                #region FILL COMBO DEPARTAMENTOS
                List<tblRH_EK_Departamentos> lstDepartamentos = _context.Select<tblRH_EK_Departamentos>(new DapperDTO
                {
                    baseDatos = (MainContextEnum)vSesiones.sesionEmpresaActual,
                    consulta = @"SELECT id, desc_depto FROM tblRH_EK_Departamentos WHERE cc = @cc AND esActivo = @esActivo",
                    parametros = new { cc = cc, esActivo = true }
                }).ToList();
                ...
                {
                    if (item.id > 0 && !string.IsNullOrEmpty(item.desc_depto))
                    {
                        objComboDTO = new ComboDTO();
                        objComboDTO.Value = item.id.ToString();
                        objComboDTO.Text = item.desc_depto.Trim();
                        lstComboDTO.Add(objComboDTO);
                    }
                }
            ...
            {
                LogError(0, 0, _NOMBRE_CONTROLADOR, "FillCboDepartamentos", e, AccionEnum.FILLCOMBO, 0, new { cc = cc });
                resultado.Add(MESSAGE, e.Message);
                resultado.Add(SUCCESS, false);
            }
            return resultado;
        ...
        private string GetDescripcionCC(string cc)
        {
            ...
            {
                #region VALIDACIONES
                if (string.IsNullOrEmpty(cc)) { throw new Exception("Ocurrió un error al obtener la descripción del CC."); }
                #endregion

                #region SE OBTIENE LA DESCRIPCIÓN DEL CC
                tblC_Nom_CatalogoCC objCC = _context.Select<tblC_Nom_CatalogoCC>(new DapperDTO
                {
                    baseDatos = (MainContextEnum)vSesiones.sesionEmpresaActual,
                    consulta = @"SELECT cc, ccDescripcion FROM tblC_Nom_CatalogoCC WHERE cc = @cc AND estatus = @estatus",
                    parametros = new { cc = cc, estatus = true }
                }).FirstOrDefault();

                if (objCC != null)
                {
                    if (!string.IsNullOrEmpty(objCC.cc) && !string.IsNullOrEmpty(objCC.ccDescripcion))
                        CC_Descripcion = string.Format("[{0}] {1}", objCC.cc.Trim(), objCC.ccDescripcion.Trim());
                }
                #endregion
            }
            catch (Exception e)
        ...
        private string GetProcedimiento(int procedimientoID)
        {
            string procedimiento = string.Empty;
            try
            {
                #region VALIDACIONES
                string mensajeError = "Ocurrió un error al obtener el procedimiento.";
                if (procedimientoID <= 0) { throw new Exception(mensajeError); }
                #endregion

                #region SE OBTIENE EL PROCEDIMIENTO
                tblRH_AC_MatrizAccionesDisciplinarias objMatriz = _context.tblRH_AC_MatrizAccionesDisciplinarias.Where(w => w.id == procedimientoID).FirstOrDefault();
                if (objMatriz == null)
                    throw new Exception(mensajeError);

                if (!string.IsNullOrEmpty(objMatriz.tipoInfraccion))
                    procedimiento = objMatriz.tipoInfraccion.Trim();
                #endregion
            }
            catch (Exception e)
            {
                LogError(0, 0, _NOMBRE_CONTROLADOR, "GetProcedimiento", e, AccionEnum.CONSULTA, procedimientoID, new { procedimientoID = procedimientoID });
        ...
        public string GetDepartamento(int idDepartamento)
        {
            ...
            {
                #region VALIDACIONES
                string mensajeError = "Ocurrió un error al obtener el departamento.";
                if (idDepartamento <= 0) { throw new Exception(mensajeError); }
                #endregion

                #region SE OBTIENE EL DEPARTAMENTO
                tblRH_EK_Departamentos objDepartamento = _context.Select<tblRH_EK_Departamentos>(new DapperDTO
                {
                    baseDatos = (MainContextEnum)vSesiones.sesionEmpresaActual,
                    consulta = @"SELECT * FROM tblRH_EK_Departamentos WHERE id = @idDepartamento",
                    parametros = new { idDepartamento = idDepartamento }
                }).FirstOrDefault();

                if (objDepartamento == null)
                    throw new Exception(mensajeError);

                if (!string.IsNullOrEmpty(objDepartamento.desc_depto))
                    departamento = objDepartamento.desc_depto.Trim();
                #endregion
            }
            catch (Exception e)
            {
                LogError(0, 0, _NOMBRE_CONTROLADOR, "GetDepartamento", e, AccionEnum.CONSULTA, idDepartamento, new { idDepartamento = idDepartamento });
                return departamento;
            }
            return departamento;
        }

        public string GetAccion(int idAccion)
        {
            string accion = string.Empty;
            try
            {
                #region VALIDACIONES
                string mensajeError = "Ocurrió un error al obtener la acción.";
                if (idAccion <= 0) { throw new Exception(mensajeError); }
                #endregion

                #region SE OBTIENE LA ACCIÓN
                tblRH_AC_Accion objAccion = _context.Select<tblRH_AC_Accion>(new DapperDTO
                {
                    baseDatos = (MainContextEnum)vSesiones.sesionEmpresaActual,
                    consulta = @"SELECT * FROM tblRH_AC_Accion WHERE id = @idAccion",
                    parametros = new { idAccion = idAccion }
                }).FirstOrDefault();

                if (objAccion == null)
                    throw new Exception(mensajeError);

                if (!string.IsNullOrEmpty(objAccion.descripcion))
                    accion = objAccion.descripcion.Trim();
                #endregion
            }
            catch (Exception e)
            {
                LogError(0, 0, _NOMBRE_CONTROLADOR, "GetAccion", e, AccionEnum.CONSULTA, idAccion, new { idAccion = idAccion });
                return accion;
            }
            return accion;
        }

        public string GetClasificacion(int idClasificacion)
        {
            string clasificacion = string.Empty;
            try
            {
                #region VALIDACIONES
                string mensajeError = "Ocurrió un error al obtener la clasificación.";
                if (idClasificacion <= 0) { throw new Exception(mensajeError); }
                #endregion

                #region SE OBTIENE LA CLASIFICACIÓN
                tblRH_AC_Clasificacion objClasificacion = _context.Select<tblRH_AC_Clasificacion>(new DapperDTO
                {
                    baseDatos = (MainContextEnum)vSesiones.sesionEmpresaActual,
                    consulta = @"SELECT * FROM tblRH_AC_Clasificacion WHERE id = @idClasificacion",
                    parametros = new { idClasificacion = idClasificacion }
                }).FirstOrDefault();

                if (objClasificacion == null)
                    throw new Exception(mensajeError);

                if (!string.IsNullOrEmpty(objClasificacion.descripcion))
                    clasificacion = objClasificacion.descripcion.Trim();
                #endregion
            }
            catch (Exception e)
            {
                LogError(0, 0, _NOMBRE_CONTROLADOR, "GetClasificacion", e, AccionEnum.CONSULTA, idClasificacion, new { idClasificacion = idClasificacion });
        ...
        public Dictionary<string, object> FillCboTipoReportes()
        {
            ...
            try
            {
                ...
                #region AMONESTACIÓN
                objComboDTO = new ComboDTO();
                idReporte = (int)TipoReporteEnum.AMONESTACION;
                objComboDTO.Value = idReporte.ToString();
                objComboDTO.Text = EnumHelper.GetDescription((TipoReporteEnum)idReporte);
                lstComboDTO.Add(objComboDTO);
                #endregion

                #region SUSPENSIÓN
                objComboDTO = new ComboDTO();
                idReporte = (int)TipoReporteEnum.SUSPENSION;
                objComboDTO.Value = idReporte.ToString();
                objComboDTO.Text = EnumHelper.GetDescription((TipoReporteEnum)idReporte);
                lstComboDTO.Add(objComboDTO);
                #endregion

                #region RESPONSABILIDAD
                objComboDTO = new ComboDTO();
                idReporte = (int)TipoReporteEnum.CARTA_RESPONSABILIDAD;
                objComboDTO.Value = idReporte.ToString();
                objComboDTO.Text = EnumHelper.GetDescription((TipoReporteEnum)idReporte);
                lstComboDTO.Add(objComboDTO);
                #endregion
                ...
                #region ACTA ADMINISTRATIVA
                objComboDTO = new ComboDTO();
                idReporte = (int)TipoReporteEnum.ACTA_ADMINISTRATIVA;
                objComboDTO.Value = idReporte.ToString();
                objComboDTO.Text = EnumHelper.GetDescription((TipoReporteEnum)idReporte);
                lstComboDTO.Add(objComboDTO);
                #endregion
            ...
            {
                LogError(0, 0, _NOMBRE_CONTROLADOR, "FillCboTipoReportes", e, AccionEnum.FILLCOMBO, 0, 0);
                resultado.Add(MESSAGE, e.Message);
                resultado.Add(SUCCESS, false);
            }
            return resultado;
        ...
        private string SetPrimeraLetraMayuscula(string palabra)
        {
            ...
            {
                LogError(0, 0, _NOMBRE_CONTROLADOR, "ConvertirPrimeraLetraMayusculaRestanteMinisculas", e, AccionEnum.METODO_GENERAL, 0, new { palabra = palabra });
        ...
        public Dictionary<string, object> InfEmpleado(int claveEmpleado)
        {
            ...
            try
            {
                ...
                empleadoIncidenteDTO objEmpleado = _context.Select<empleadoIncidenteDTO>(new DapperDTO
                {
                    baseDatos = (MainContextEnum)vSesiones.sesionEmpresaActual,
                    consulta = @"SELECT
                                ...
                                FROM tblRH_EK_Empleados as e
                                    ...
                                    where e.clave_empleado= @claveEmpleado AND e.estatus_empleado = @estatus_empleado
                                    order by e.ape_paterno DESC",
                    parametros = new { claveEmpleado, estatus_empleado = 'A' }
                }).FirstOrDefault();

                if (objEmpleado == null) 
                {
                    objEmpleado = _context.Select<empleadoIncidenteDTO>(new DapperDTO
                    {
                        baseDatos = MainContextEnum.GCPLAN,
                        consulta = @"SELECT
                                    e.clave_empleado AS claveEmpleado, (e.nombre + ' ' + e.ape_paterno + ' ' + e.ape_materno) AS nombreEmpleado, e.fecha_nac AS edadEmpleado, e.fecha_antiguedad AS antiguedadEmpleado, e.fecha_antiguedad AS antiguedadEmpleadoStr, p.descripcion AS puestoEmpleado,
                ...
                #region SE OBTIENE AL RESPONSABLE DEL CC
                List<ComboBoxDTO> lstUsuarios = new List<ComboBoxDTO>();
                List<int> lstPlantillasRequisiciones = new List<int> { 123 };
                tblFA_Paquete objPaquete = _context.tblFA_Paquete.Where(x => x.cc.cc == objEmpleado.ccID).OrderByDescending(x => x.fechaCreacion).FirstOrDefault();

                if (objPaquete != null)
                {
                    foreach (var facultamiento in objPaquete.facultamientos.Where(x => lstPlantillasRequisiciones.Contains(x.plantillaID) && x.aplica))
                    {
                        foreach (var item in facultamiento.empleados.Where(x => x.esActivo && x.aplica))
                        {
                            objEmpleado.responsableCC = item.nombreEmpleado;
                            objEmpleado.FK_responsableCC = (int)item.claveEmpleado;
                        }
                    }
                }
                ...
                resultado.Add(SUCCESS, true);
                resultado.Add("empleado", objEmpleado);
                #endregion
            }
            catch (Exception ex)
            {
                LogError(0, 0, _NOMBRE_CONTROLADOR, "InfEmpleado", ex, AccionEnum.CONSULTA, 0, new { claveEmpleado = claveEmpleado });
                resultado.Add(MESSAGE, ex.Message);
                resultado.Add(SUCCESS, false);
            }
            return resultado;
