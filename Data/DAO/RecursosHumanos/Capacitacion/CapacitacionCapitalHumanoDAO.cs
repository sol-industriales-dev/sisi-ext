...
namespace Data.DAO.RecursosHumanos.Capacitacion
{
    public class CapacitacionCapitalHumanoDAO : GenericDAO<tblS_CapacitacionCursos>, ICapacitacionCapitalHumanoDAO
    {
        ...
        #region Constructor
        public CapacitacionCapitalHumanoDAO()
        {
            resultado.Clear();
        ...
        public Dictionary<string, object> guardarCurso(tblS_CapacitacionCursos curso)
        {
            ...
            try
            {
                ...
                curso.estatus = EstatusCursoEnum.Completo;
        ...
        public Dictionary<string, object> getPuestosEnkontrolMandos(List<int> mandos)
        {
            ...
            try
            {
                ...
                if (mandos != null)
                {
                    ...
                    if (puestos.Count > 0)
                    {
                        var puestosMandos = _context.tblS_CapacitacionCapitalHumanoPuestoMando.Where(x => x.estatus && mandos.Contains((int)x.mando)).Select(x => x.puesto).ToList();
        ...
        public Dictionary<string, object> ObtenerListaControlesAsistencia(string cc, int estado, DateTime fechaInicio, DateTime fechaFin)
        {
            try
            {
                ...
                var listaConstruplan = new List<Core.DTO.Principal.Generales.ComboDTO>();
                var listaArrendadora = new List<Core.DTO.Principal.Generales.ComboDTO>();

                using (var _contextConstruplan = new MainContext(EmpresaEnum.Construplan))
                {
                    listaConstruplan = _contextConstruplan.tblC_Nom_CatalogoCC.Select(x => new Core.DTO.Principal.Generales.ComboDTO{
                        Value = x.cc,
                        Text = x.ccDescripcion
                    }).ToList();
                }

                using (var _contextArrendadora = new MainContext(EmpresaEnum.Arrendadora))
                {
                    listaArrendadora = _contextArrendadora.tblC_Nom_CatalogoCC.Select(x => new Core.DTO.Principal.Generales.ComboDTO
                    {
                        Value = x.cc,
                        Text = x.ccDescripcion
                    }).ToList();
                ...
                var listaControlAsistencia = _context.tblS_CapacitacionControlAsistencia.Where(x =>
                    ...
                    (estado == 0 ? true : x.estatus == (EstatusControlAsistenciaEnum)estado) &&
        ...
        public Dictionary<string, object> CrearControlAsistencia(tblS_CapacitacionControlAsistencia controlAsistencia)
        {
            using (var dbTransaction = _context.Database.BeginTransaction())
            {
                try
                {
                    ...
                    {
                        asis.estatus = EstatusEmpledoControlAsistenciaEnum.Aprobado;
                        ...
                        var empleadoConstruplan = _context.Select<dynamic>(new DapperDTO
                        {
                            ...
                            parametros = new { claveEmpleado = asis.claveEmpleado }
                ...
                try
                {
                    ...
                    foreach (var examenesAsistente in listaExamenesAsistentes)
                    {
                        ...
                        var empleadoConstruplan = _context.Select<dynamic>(new DapperDTO
                        {
                            baseDatos = MainContextEnum.Construplan,
                            consulta = @"SELECT * FROM tblRH_EK_Empleados WHERE estatus_empleado = 'A' AND clave_empleado = @claveEmpleado",
                            parametros = new { claveEmpleado = controlAsistenciaDetalle.claveEmpleado }
...
//                ";
        ...
        public Tuple<Stream, string> DescargarExpedienteEmpleado(int claveEmpleado, byte[] licencia)
        {
            ...
            try
            {
                ...
                var capacitaciones = _context.tblS_CapacitacionControlAsistencia
                    .Where(x =>
        ...
        public Dictionary<string, object> CargarDatosGeneralesDashboard(List<string> ccsCplan, List<string> ccsArr, DateTime fechaInicio, DateTime fechaFin, List<string> clasificaciones)
        {
            ...
            try
            {
                ...
                var capacitacionesPorCC = _context.tblS_CapacitacionControlAsistencia.ToList().Where(c =>
                    ...
                    (clasificaciones != null ? listaClasificacionesEnum.Contains(c.curso.clasificacion) : true) && c.division == divisionActual
                ).ToList();
        ...
        private static decimal ObtenerTotalHorasHombreCapacitacion(List<tblS_CapacitacionControlAsistencia> listaCapacitaciones)
        {
        ...
        private static double ObtenerTotalPersonalCapacitado(List<tblS_CapacitacionControlAsistencia> listaCapacitaciones)
        {
            if (listaCapacitaciones == null || listaCapacitaciones.Count == 0)
            {
                return 0;
        ...
        private List<EmpleadoExpirarDTO> cargarTblEmpleadosExpirar(List<tblS_CapacitacionControlAsistencia> listaCapacitaciones, List<EmpleadoCapacitacionDTO> empleados)
        {
            ...
            var cursosAplicablesPorPuesto = _context.tblS_CapacitacionCursosPuestos
                .Where(x => x.estatus && puestosID.Contains(x.puesto_id) && x.division == divisionActual)
                .Select(x => x.curso_id).Distinct().ToList();

            var cursosAplicablesEmpleado = _context.tblS_CapacitacionCursos
                .Where(x =>
                    x.estatus == EstatusCursoEnum.Completo &&
                    (x.esGeneral || x.aplicaTodosPuestos || cursosAplicablesPorPuesto.Contains(x.id)) && x.division == divisionActual
                    ).ToList();

            var cursosGenerales = _context.tblS_CapacitacionCursos
                .Where(x => x.estatus == EstatusCursoEnum.Completo && x.esGeneral && x.division == divisionActual);
...
//                ";
        ...
        private List<EmpleadoCapacitacionDTO> ObtenerEmpleadosCCPorDepartamento(List<string> ccsCplan, List<string> ccsArr, List<string> departamentos, List<MandoEnum> mandos = null)
        {
            bool existeCCsCplan = ccsCplan != null && ccsCplan.Count > 0;
            bool existeCCsArr = ccsArr != null && ccsArr.Count > 0;
...
//            @" ORDER BY c.cc, nombreEmpleado";
            ...
            var whereMandos = "";
            if (mandos != null && mandos.Count > 0)
            {
                whereMandos = " AND mdo.mando in (";
                foreach (var item in mandos)
                {
                    whereMandos += (int)item;
            ...
            string queryBase = @"SELECT 
                                ...
                                INNER JOIN tblP_CC AS c ON e.cc_contable = c.cc
                                INNER JOIN tblS_CapacitacionCapitalHumanoPuestoMando AS mdo ON mdo.puesto = p.puesto AND mdo.estatus = 1
                                WHERE e.estatus_empleado ='A' AND e.cc_contable IN @ccsCplan " + (departamentos != null ? @" AND e.clave_depto IN @departamentos " : @"") + whereMandos +
                                @" ORDER BY c.cc, (e.nombre + ' ' + e.ape_paterno + ' ' + e.ape_materno)";
            ...
            if (existeCCsCplan)
            {
                ...
                listaEmpleadosCP = _context.Select<dynamic>(new DapperDTO
                {
                    baseDatos = MainContextEnum.Construplan,
                    consulta = queryBase,
                    parametros = new { ccsCplan, departamentos}
                });
        ...
        private List<PorcentajesExpiradosDTO> ObtenerPorcentajeCursosVigentes(List<EmpleadoExpirarDTO> detalleCurso, List<string> listaCCs, List<EmpleadoCapacitacionDTO> empleados)
        {
            ...
            var cursosIDPorPuesto = _context.tblS_CapacitacionCursosPuestos
                .Where(x => x.estatus && puestosID.Contains(x.puesto_id) && x.division == divisionActual)
                .Select(x => x.curso_id).Distinct();

            var cursosAplicables = _context.tblS_CapacitacionCursos.Where(x =>
                x.estatus == EstatusCursoEnum.Completo &&
        ...
        private List<Tuple<int, string>> ObtenerCursosImpartidos(List<tblS_CapacitacionControlAsistencia> listaCapacitaciones, DateTime fechaInicio, DateTime fechaFin)
        {
            ...
            {
                var contador = listaCapacitaciones.Where(x => x.curso.clasificacion == item && x.fechaCapacitacion >= fechaInicio && x.fechaCapacitacion <= fechaFin).Count();
        ...
        private List<Tuple<decimal, string>> ObtenerHH(List<tblS_CapacitacionControlAsistencia> listaCapacitaciones, DateTime fechaInicio, DateTime fechaFin)
        {
            ...
            foreach (var clasificacionEnum in EnumExtensions.ToArray<ClasificacionCursoEnum>())
            {
                var capacitacionesPorClas = listaCapacitaciones
                    .Where(x => x.curso.clasificacion == clasificacionEnum && x.fechaCapacitacion >= fechaInicio && x.fechaCapacitacion <= fechaFin);
...
//                    ORDER BY departamento";
        ...
        public Dictionary<string, object> CargarDatosMatrizNecesidades(List<string> ccsCplan, List<string> ccsArr, List<string> departamentosIDs, List<ClasificacionCursoEnum> clasificaciones, List<MandoEnum> mandos)
        {
            try
            {
                ...
                if (mandos == null)
                {
                    mandos = EnumExtensions.ToArray<MandoEnum>().OrderBy(x => x).ToList();
                }

                mandos = mandos.OrderBy(x => x).ToList();
                ...
                var empleados = ObtenerEmpleadosCCPorDepartamento(ccsCplan, ccsArr, departamentosIDs, mandos);
                var cursosAplicables = CargarCursosAplicables(empleados, clasificaciones, ccsCplan, ccsArr,mandos);
        ...
        private void AgregarGraficasMandoAdministrativoProtocoloFatalidad(ref Dictionary<string, object> resultado, List<IndicadorClasificacionDTO> indicadores)
        {
            ...
            var datosBarras = ObtenerPorcentajeClasificacionPorMando(indicadoresProtocolos, Core.Enum.RecursosHumanos.Capacitacion.MandoEnum.ALTO, false);
            ...
            cantidadProtocolo.datasets.AddRange(ObtenerCantidadClasificacionPorMando(indicadoresProtocolos, Core.Enum.RecursosHumanos.Capacitacion.MandoEnum.ALTO));
        ...
        private void AgregarGraficasMandoMedioProtocoloFatalidad(ref Dictionary<string, object> resultado, List<IndicadorClasificacionDTO> indicadores)
        {
            var indicadoresProtocolos = ObtenerIndicadoresPorClasificacion(indicadores, ClasificacionCursoEnum.ProtocoloFatalidad);
            var porcentajeProtocolo = new { labels = indicadoresProtocolos.GroupBy(x => x.cursoClave).Select(x => x.Key).ToList(), datasets = new List<DatasetDTO>() };
            var datosBarras = ObtenerPorcentajeClasificacionPorMando(indicadoresProtocolos, Core.Enum.RecursosHumanos.Capacitacion.MandoEnum.MEDIO, false);
            List<decimal> listaLinea = new List<decimal>();
            ...
            var cantidadProtocolo = new { labels = indicadoresProtocolos.GroupBy(x => x.cursoClave).Select(x => x.Key).ToList(), datasets = new List<DatasetDTO>() };
            cantidadProtocolo.datasets.AddRange(ObtenerCantidadClasificacionPorMando(indicadoresProtocolos, Core.Enum.RecursosHumanos.Capacitacion.MandoEnum.MEDIO));
        ...
        private void AgregarGraficasMandoOperativoProtocoloFatalidad(ref Dictionary<string, object> resultado, List<IndicadorClasificacionDTO> indicadores)
        {
            var indicadoresProtocolos = ObtenerIndicadoresPorClasificacion(indicadores, ClasificacionCursoEnum.ProtocoloFatalidad);
            var porcentajeProtocolo = new { labels = indicadoresProtocolos.GroupBy(x => x.cursoClave).Select(x => x.Key).ToList(), datasets = new List<DatasetDTO>() };
            var datosBarras = ObtenerPorcentajeClasificacionPorMando(indicadoresProtocolos, Core.Enum.RecursosHumanos.Capacitacion.MandoEnum.TECNICO_ADMON, false);
            List<decimal> listaLinea = new List<decimal>();
            ...
            var cantidadProtocolo = new { labels = indicadoresProtocolos.GroupBy(x => x.cursoClave).Select(x => x.Key).ToList(), datasets = new List<DatasetDTO>() };
            cantidadProtocolo.datasets.AddRange(ObtenerCantidadClasificacionPorMando(indicadoresProtocolos, Core.Enum.RecursosHumanos.Capacitacion.MandoEnum.TECNICO_ADMON));
        ...
        private void AgregarGraficasMandoAdministrativoNormativo(ref Dictionary<string, object> resultado, List<IndicadorClasificacionDTO> indicadores)
        {
            ...
            var datosBarras = ObtenerPorcentajeClasificacionPorMando(indicadoresNormativos, Core.Enum.RecursosHumanos.Capacitacion.MandoEnum.ALTO, false);
            List<decimal> listaLinea = new List<decimal>();
            ...
            cantidadNormativo.datasets.AddRange(ObtenerCantidadClasificacionPorMando(indicadoresNormativos, Core.Enum.RecursosHumanos.Capacitacion.MandoEnum.ALTO));
        ...
        private void AgregarGraficasMandoMedioNormativo(ref Dictionary<string, object> resultado, List<IndicadorClasificacionDTO> indicadores)
        {
            var indicadoresNormativos = ObtenerIndicadoresPorClasificacion(indicadores, ClasificacionCursoEnum.Normativo);
            var porcentajeNormativo = new { labels = indicadoresNormativos.GroupBy(x => x.cursoClave).Select(x => x.Key).ToList(), datasets = new List<DatasetDTO>() };
            var datosBarras = ObtenerPorcentajeClasificacionPorMando(indicadoresNormativos, Core.Enum.RecursosHumanos.Capacitacion.MandoEnum.MEDIO, false);
            List<decimal> listaLinea = new List<decimal>();
            ...
            var cantidadNormativo = new { labels = indicadoresNormativos.GroupBy(x => x.cursoClave).Select(x => x.Key).ToList(), datasets = new List<DatasetDTO>() };
            cantidadNormativo.datasets.AddRange(ObtenerCantidadClasificacionPorMando(indicadoresNormativos, Core.Enum.RecursosHumanos.Capacitacion.MandoEnum.MEDIO));
        ...
        private void AgregarGraficasMandoOperativoNormativo(ref Dictionary<string, object> resultado, List<IndicadorClasificacionDTO> indicadores)
        {
            var indicadoresNormativos = ObtenerIndicadoresPorClasificacion(indicadores, ClasificacionCursoEnum.Normativo);
            var porcentajeNormativo = new { labels = indicadoresNormativos.GroupBy(x => x.cursoClave).Select(x => x.Key).ToList(), datasets = new List<DatasetDTO>() };
            var datosBarras = ObtenerPorcentajeClasificacionPorMando(indicadoresNormativos, Core.Enum.RecursosHumanos.Capacitacion.MandoEnum.TECNICO_ADMON, false);
            List<decimal> listaLinea = new List<decimal>();
            ...
            var cantidadNormativo = new { labels = indicadoresNormativos.GroupBy(x => x.cursoClave).Select(x => x.Key).ToList(), datasets = new List<DatasetDTO>() };
            cantidadNormativo.datasets.AddRange(ObtenerCantidadClasificacionPorMando(indicadoresNormativos, Core.Enum.RecursosHumanos.Capacitacion.MandoEnum.TECNICO_ADMON));
        ...
        private void AgregarGraficasMandoAdministrativoTecnicoOperativo(ref Dictionary<string, object> resultado, List<IndicadorClasificacionDTO> indicadores)
        {
            ...
            var datosBarras = ObtenerPorcentajeClasificacionPorMando(indicadoresTecnico, Core.Enum.RecursosHumanos.Capacitacion.MandoEnum.ALTO, false);
            List<decimal> listaLinea = new List<decimal>();
            ...
            cantidadTecnico.datasets.AddRange(ObtenerCantidadClasificacionPorMando(indicadoresTecnico, Core.Enum.RecursosHumanos.Capacitacion.MandoEnum.ALTO));
        ...
        private void AgregarGraficasMandoMedioTecnicoOperativo(ref Dictionary<string, object> resultado, List<IndicadorClasificacionDTO> indicadores)
        {
            var indicadoresTecnico = ObtenerIndicadoresPorClasificacion(indicadores, ClasificacionCursoEnum.TecnicoOperativo);
            var porcentajeTecnico = new { labels = indicadoresTecnico.GroupBy(x => x.cursoClave).Select(x => x.Key).ToList(), datasets = new List<DatasetDTO>() };
            var datosBarras = ObtenerPorcentajeClasificacionPorMando(indicadoresTecnico, Core.Enum.RecursosHumanos.Capacitacion.MandoEnum.MEDIO, false);
            List<decimal> listaLinea = new List<decimal>();
            ...
            var cantidadTecnico = new { labels = indicadoresTecnico.GroupBy(x => x.cursoClave).Select(x => x.Key).ToList(), datasets = new List<DatasetDTO>() };
            cantidadTecnico.datasets.AddRange(ObtenerCantidadClasificacionPorMando(indicadoresTecnico, Core.Enum.RecursosHumanos.Capacitacion.MandoEnum.MEDIO));
        ...
        private void AgregarGraficasMandoOperativoTecnicoOperativo(ref Dictionary<string, object> resultado, List<IndicadorClasificacionDTO> indicadores)
        {
            var indicadoresTecnico = ObtenerIndicadoresPorClasificacion(indicadores, ClasificacionCursoEnum.TecnicoOperativo);
            var porcentajeTecnico = new { labels = indicadoresTecnico.GroupBy(x => x.cursoClave).Select(x => x.Key).ToList(), datasets = new List<DatasetDTO>() };
            var datosBarras = ObtenerPorcentajeClasificacionPorMando(indicadoresTecnico, Core.Enum.RecursosHumanos.Capacitacion.MandoEnum.TECNICO_ADMON, true);
            List<decimal> listaLinea = new List<decimal>();
            ...
            var cantidadTecnico = new { labels = indicadoresTecnico.GroupBy(x => x.cursoClave).Select(x => x.Key).ToList(), datasets = new List<DatasetDTO>() };
            cantidadTecnico.datasets.AddRange(ObtenerCantidadClasificacionPorMando(indicadoresTecnico, Core.Enum.RecursosHumanos.Capacitacion.MandoEnum.TECNICO_ADMON));
        ...
        private void AgregarGraficasMandoAdministrativoInstructivoOperativo(ref Dictionary<string, object> resultado, List<IndicadorClasificacionDTO> indicadores)
        {
            ...
            var datosBarras = ObtenerPorcentajeClasificacionPorMando(indicadoresInstructivo, Core.Enum.RecursosHumanos.Capacitacion.MandoEnum.ALTO, false);
            List<decimal> listaLinea = new List<decimal>();
            ...
            cantidadInstructivo.datasets.AddRange(ObtenerCantidadClasificacionPorMando(indicadoresInstructivo, Core.Enum.RecursosHumanos.Capacitacion.MandoEnum.TECNICO_ADMON));
        ...
        private void AgregarGraficasMandoMedioInstructivoOperativo(ref Dictionary<string, object> resultado, List<IndicadorClasificacionDTO> indicadores)
        {
            var indicadoresInstructivo = ObtenerIndicadoresPorClasificacion(indicadores, ClasificacionCursoEnum.InstructivoOperativo);
            var porcentajeInstructivo = new { labels = indicadoresInstructivo.GroupBy(x => x.cursoClave).Select(x => x.Key).ToList(), datasets = new List<DatasetDTO>() };
            var datosBarras = ObtenerPorcentajeClasificacionPorMando(indicadoresInstructivo, Core.Enum.RecursosHumanos.Capacitacion.MandoEnum.MEDIO, false);
            List<decimal> listaLinea = new List<decimal>();
            ...
            var cantidadInstructivo = new { labels = indicadoresInstructivo.GroupBy(x => x.cursoClave).Select(x => x.Key).ToList(), datasets = new List<DatasetDTO>() };
            cantidadInstructivo.datasets.AddRange(ObtenerCantidadClasificacionPorMando(indicadoresInstructivo, Core.Enum.RecursosHumanos.Capacitacion.MandoEnum.MEDIO));
        ...
        private void AgregarGraficasMandoOperativoInstructivoOperativo(ref Dictionary<string, object> resultado, List<IndicadorClasificacionDTO> indicadores)
        {
            var indicadoresInstructivo = ObtenerIndicadoresPorClasificacion(indicadores, ClasificacionCursoEnum.InstructivoOperativo);
            var porcentajeInstructivo = new { labels = indicadoresInstructivo.GroupBy(x => x.cursoClave).Select(x => x.Key).ToList(), datasets = new List<DatasetDTO>() };
            var datosBarras = ObtenerPorcentajeClasificacionPorMando(indicadoresInstructivo, Core.Enum.RecursosHumanos.Capacitacion.MandoEnum.TECNICO_ADMON, false);
            List<decimal> listaLinea = new List<decimal>();
        ...
        private void AgregarDatosGraficasDetalles(ref Dictionary<string, object> resultado, List<IndicadorClasificacionDTO> indicadores)
        {
            ...
            #region Instructivo Operativo
            {
                ...
                var cantidadInstructivo = new { labels = indicadoresInstructivo.GroupBy(x => x.cursoClave).Select(x => x.Key).ToList(), datasets = new List<DatasetDTO>() };
                cantidadInstructivo.datasets.AddRange(ObtenerCantidadClasificacionPorMando(indicadoresInstructivo, Core.Enum.RecursosHumanos.Capacitacion.MandoEnum.ALTO));
                resultado.Add("chartCantidadMandoAdministrativoInstructivoOperativo", cantidadInstructivo);
            }
        ...
        private List<DatasetDTO> ObtenerCantidadClasificacionPorMando(List<IndicadorClasificacionDTO> indicadoresPorClasificacion, Core.Enum.RecursosHumanos.Capacitacion.MandoEnum mando)
        {
        ...
        private DatasetDTO ObtenerPorcentajeClasificacionPorMando(List<IndicadorClasificacionDTO> indicadoresPorClasificacion, Core.Enum.RecursosHumanos.Capacitacion.MandoEnum mando, bool reglaPersonalAutorizado)
        {
            var indicadoresPorCurso = indicadoresPorClasificacion.Where(x => x.listaMandos.Contains((int)mando))
                .GroupBy(x => x.cursoClave)
                .OrderBy(x => x.Key)
        ...
        private List<tblS_CapacitacionCursos> CargarCursosAplicables(List<EmpleadoCapacitacionDTO> empleados, List<ClasificacionCursoEnum> clasificaciones, List<string> ccsCplan, List<string> ccsArr, List<MandoEnum> mandos)
        {
            ...
            List<tblS_CapacitacionCursosMando> listaCursosConMando = new List<tblS_CapacitacionCursosMando>();

           
            foreach (var curMando in listaCursos)
            {
                listaCursosConMando = _context.tblS_CapacitacionCursosMando.Where(x => x.estatus && x.division == divisionActual && x.curso_id == curMando.id).ToList().Where(mando => mandos.Contains(mando.filMando)).ToList();
            }

            foreach (var conMando in listaCursosConMando)
            {
                listaCursos = _context.tblS_CapacitacionCursos.Where(x => x.isActivo && x.division == divisionActual).ToList().Where(x =>
                (clasificaciones == null ? true : clasificaciones.Contains(x.clasificacion) && x.id==conMando.curso_id)
            ).ToList();
            }
        ...
        public Dictionary<string, object> getRelacionesCCDepartamentoRazonSocial()
        {
            try
            {
                ...
                var departamentosConstruplan = _context.Select<dynamic>(new DapperDTO
                {
                    baseDatos = MainContextEnum.Construplan,
                    consulta = "SELECT * FROM tblRH_EK_Departamentos",

                });

                var departamentosArrendadora = _context.Select<dynamic>(new DapperDTO
                {
                    baseDatos = MainContextEnum.Arrendadora,
                    consulta = "SELECT * FROM tblRH_EK_Departamentos",
                });
            ...
            using (var dbContextTransaction = _context.Database.BeginTransaction())
            {
                try
                {
                    foreach (var rel in relaciones)
                    {
                        ...
                        //List<dynamic> departamentoConstruplan = _contextEnkontrol.Select<dynamic>(EnkontrolEnum.CplanRh, new OdbcConsultaDTO
                        //{
                        //    consulta = string.Format(@"SELECT * FROM sn_departamentos WHERE clave_depto = '{0}'", rel.departamento)
                        //});
                        //List<dynamic> departamentoArrendadora = _contextEnkontrol.Select<dynamic>(EnkontrolEnum.ArrenRh, new OdbcConsultaDTO
                        //{
                        //    consulta = string.Format(@"SELECT * FROM sn_departamentos WHERE clave_depto = '{0}'", rel.departamento)
                        //});

                        var departamentoConstruplan = _context.Select<DepartamentoDTO>(new DapperDTO
                        {
                            baseDatos = MainContextEnum.Construplan,
                            consulta = "SELECT * FROM tblRH_EK_Departamentos WHERE clave_depto = @clave",
                            parametros = new { clave = rel.departamento }
                        });
                        var departamentoArrendadora = _context.Select<DepartamentoDTO>(new DapperDTO
                        {
                            baseDatos = MainContextEnum.Arrendadora,
                            consulta = "SELECT * FROM tblRH_EK_Departamentos WHERE clave_depto = @clave",
                            parametros = new { clave = rel.departamento }
        ...
        public Dictionary<string, object> editarRelacionCCDepartamentoRazonSocial(RelacionCCDepartamentoRazonSocialDTO relacion)
        {
            using (var dbContextTransaction = _context.Database.BeginTransaction())
            {
                try
                {
                    ...
                    //List<dynamic> departamentoConstruplan = _contextEnkontrol.Select<dynamic>(EnkontrolEnum.CplanRh, new OdbcConsultaDTO
                    //{
                    //    consulta = string.Format(@"SELECT * FROM sn_departamentos WHERE clave_depto = '{0}'", relacion.departamento)
                    //});
                    //List<dynamic> departamentoArrendadora = _contextEnkontrol.Select<dynamic>(EnkontrolEnum.ArrenRh, new OdbcConsultaDTO
                    //{
                    //    consulta = string.Format(@"SELECT * FROM sn_departamentos WHERE clave_depto = '{0}'", relacion.departamento)
                    ...
                    var departamentoConstruplan = _context.Select<DepartamentoDTO>(new DapperDTO
                    {
                        baseDatos = MainContextEnum.Construplan,
                        consulta = "SELECT * FROM tblRH_EK_Departamentos WHERE clave_depto = @clave",
                        parametros = new { clave = relacion.departamento }
                    });
                    var departamentoArrendadora = _context.Select<DepartamentoDTO>(new DapperDTO
                    {
                        baseDatos = MainContextEnum.Arrendadora,
                    ...
                    using (ZipArchive archive = new ZipArchive(controlAsistencia.InputStream))
                    {
                        ...
                        foreach (ZipArchiveEntry archivo in archivos)
                        {
                            ...
                            else
                            {
                                ...
                                var empleadoEnkontrolConsulta = _context.tblRH_EK_Empleados.FirstOrDefault(e => e.clave_empleado == claveEmpleado);

                                if (empleadoEnkontrolConsulta == null) 
                                {
                                    using (var _gcplan = new MainContext(EmpresaEnum.GCPLAN)) 
                                    {
                                        empleadoEnkontrolConsulta = _gcplan.tblRH_EK_Empleados.FirstOrDefault(e => e.clave_empleado == claveEmpleado);
                                    }
                                }
                                ...
                                var empleadoEnkontrol = empleadoEnkontrolConsulta;
...
//            List<dynamic> listaEmpleados = _contextEnkontrol.Select<dynamic>(empresa == 1 ? EnkontrolAmbienteEnum.Rh : EnkontrolAmbienteEnum.RhArre, odbc);
            ...
            try
            {
                ...
                var listaEmpleadosARR = _context.Select<dynamic>(new DapperDTO
                {
                    ...
                    consulta = @"SELECT 
                                ...
                                FROM tblRH_EK_Empleados AS e
                                    INNER JOIN tblRH_EK_Departamentos AS d ON e.clave_depto = d.clave_depto
                                WHERE e.estatus_empleado = 'A' AND e.clave_empleado = @claveo",
                    parametros = new { clave }
                });
        ...
        {
            return _context.tblS_CapacitacionControlAsistencia
                     .Where(x => x.estatus == EstatusControlAsistenciaEnum.Completa && x.division == divisionActual)
                     .ToList()
                     .Where(x => CursoAprobadoEmpleado(x.asistentes, claveEmpleado))
                     ...
                     .Select(x => new
                     {
                         ...
                         x.curso.nombre,
                         clasificacion = x.curso.clasificacion.GetDescription(),
                         x.lugar,
                         instructor = x.esExterno ? x.instructorExterno : GlobalUtils.ObtenerNombreCompletoUsuario(x.instructor),
        ...
        private Tuple<string, string>[] ObtenerCursosLicenciaEmpleado(int claveEmpleado)
        {
            ...
            return _context.tblS_CapacitacionControlAsistencia
                    .Where(x =>
                         x.estatus == EstatusControlAsistenciaEnum.Completa &&
                         (x.curso.clasificacion == ClasificacionCursoEnum.ProtocoloFatalidad || x.curso.clasificacion == ClasificacionCursoEnum.TecnicoOperativo) && x.division == divisionActual)
                .ToList()
                .Where(x => CursoAprobadoEmpleado(x.asistentes, claveEmpleado))
        ...
        private List<PorcentajeCapacitacionDTO> ObtenerListaPorcentajeCapacitacion(int claveEmpleado, int puestoID, out string porcentajeGeneral)
        {
            ...
            foreach (var clasificacion in clasificaciones)
            {
                ...
                var cursosClasificacion = _context.tblS_CapacitacionCursos
                    .Where(x => x.isActivo && x.estatus == EstatusCursoEnum.Completo && x.clasificacion == clasificacion && x.division == divisionActual)
                    .ToList();

                // Se filtran los cursos aprobados de esa clasificación.
                var cursosAprobadosClasificacion = _context.tblS_CapacitacionControlAsistencia
        ...
        {
            var capacitaciones = _context.tblS_CapacitacionControlAsistenciaDetalle
                .Where(x => x.claveEmpleado == claveEmpleado && x.controlAsistencia.estatus != EstatusControlAsistenciaEnum.PendienteCargaAsistencia && x.division == divisionActual)
                .ToList();
        ...
        public Dictionary<string, object> getListasAutorizacion(List<int> listaCursos, List<tblS_CapacitacionListaAutorizacionCC> listaCC)
        {
            try
            {
                ...
                {
                    baseDatos = MainContextEnum.Arrendadora,
                    consulta = "SELECT clave_depto as id, desc_depto as departamento, cc, 2 AS empresa FROM tblRH_EK_Departamentos",
...
//                    ORDER BY e.ape_paterno DESC";
                ...
                string query = @"SELECT 
                    ...
                    FROM tblRH_EK_Empleados AS e
                        INNER JOIN tblRH_EK_Puestos AS p ON e.puesto = p.puesto
                        INNER JOIN tblP_CC AS c on e.cc_contable = c.cc
                    WHERE e.estatus_empleado = 'A' AND (TRIM(e.nombre) + ' ' + TRIM(e.ape_paterno) + ' ' + TRIM(e.ape_materno)) LIKE @termStr
                    ORDER BY e.ape_paterno DESC";
...
//                List<dynamic> listaEmpleadosARR = _contextEnkontrol.Select<dynamic>(EnkontrolEnum.ArrenRh, odbc);
        ...
        public bool enviarCorreoListaAutorizacion(int listaAutorizacionID, List<Byte[]> archivoListaAutorizacion, List<string> listaCorreos)
        {
            ...
            try
            {
                ...
                if (listaAutorizacion != null)
                {
                    ...
                    correoEnviado = GlobalUtils.sendMailWithFiles(asunto, mensaje, listaCorreos, archivosAdjuntos);
                }
                else
                {
                    throw new Exception("No se encuentra la información de la lista de autorización.");
        ...
        public Dictionary<string, object> getListaSeguimiento(List<string> listaCC, TipoSeguimientoEnum tipoSeguimiento, DateTime fechaInicio, DateTime fechaFin)
        {
            try
            {
                ...
                {
                    baseDatos = MainContextEnum.Construplan,
                    consulta = "SELECT clave_empleado, (nombre + ' ' + ape_paterno + ' ' + ape_materno) AS nombre, clave_depto, cc_contable, 1 AS empresa FROM tblRH_EK_Empleados",
                ...
                {
                    baseDatos = MainContextEnum.Arrendadora,
                    consulta = "SELECT clave_empleado, (nombre + ' ' + ape_paterno + ' ' + ape_materno) AS nombre, clave_depto, cc_contable, 2 AS empresa FROM tblRH_EK_Empleados",
        ...
        public Dictionary<string, object> cargarDashboardCiclos(List<string> listaCC, List<AreaDTO> listaAreas, DateTime fechaInicio, DateTime fechaFin)
        {
            try
            {
                ...
                {
                    baseDatos = MainContextEnum.Construplan,
                    consulta = "SELECT clave_empleado, (nombre + ' ' + ape_paterno + ' ' + ape_materno) AS nombre, clave_depto, cc_contable, 1 AS empresa FROM tblRH_EK_Empleados WHERE estatus_empleado = 'A'",
                ...
                {
                    baseDatos = MainContextEnum.Arrendadora,
                    consulta = "SELECT clave_empleado, (nombre + ' ' + ape_paterno + ' ' + ape_materno) AS nombre, clave_depto, cc_contable, 2 AS empresa FROM tblRH_EK_Empleados WHERE estatus_empleado = 'A'",
...
//                    List<dynamic> listaEmpleadosARR = _contextEnkontrol.Select<dynamic>(EnkontrolEnum.ArrenRh, odbc);
                    ...
                    {
                        baseDatos = MainContextEnum.Construplan,
                        consulta = "SELECT clave_empleado AS claveEmpleado , (TRIM(e.nombre) + ' ' + TRIM(e.ape_paterno) + ' ' + TRIM(e.ape_materno)) AS nombreEmpleado FROM tblRH_EK_Empleados WHERE estatus_empleado = 'A'",
            ...
            using (var dbContextTransaction = _context.Database.BeginTransaction())
            {
                try
                {
                    ...
                    if (recorridoSIGOPLAN != null)
                    {
                        ...
                        correoEnviado = GlobalUtils.sendMailWithFiles(asunto, mensaje, correo, archivosAdjuntos);
        ...
        public Dictionary<string, object> cargarHorasAdiestramiento(List<string> listaCC, DateTime fechaInicial, DateTime fechaFinal)
        {
            try
            {
                ...
                {
                    baseDatos = MainContextEnum.Construplan,
                    consulta = "SELECT clave_empleado, (nombre + ' ' + ape_paterno + ' ' + ape_materno) AS nombre, clave_depto, cc_contable, 1 AS empresa, estatus_empleado FROM tblRH_EK_Empleados WHERE estatus_empleado = 'A'",
                ...
                {
                    baseDatos = MainContextEnum.Arrendadora,
                    consulta = "SELECT clave_empleado, (nombre + ' ' + ape_paterno + ' ' + ape_materno) AS nombre, clave_depto, cc_contable, 2 AS empresa, estatus_empleado FROM tblRH_EK_Empleados WHERE estatus_empleado = 'A'",
        ...
        public Dictionary<string, object> guardarNuevoColaboradorCapacitacion(tblS_CapacitacionIHHColaboradorCapacitacion colaboradorCapacitacion)
        {
            using (var dbContextTransaction = _context.Database.BeginTransaction())
            {
                try
                {
                    ...
                    {
                        baseDatos = MainContextEnum.Construplan,
                        consulta = "SELECT clave_empleado, clave_depto, cc_contable, 1 AS empresa FROM tblRH_EK_Empleados WHERE clave_empleado = @clave",
                        parametros = new { clave = colaboradorCapacitacion.colaborador}
                    });
                    empleadoConstruplan.AddRange(_context.Select<EmpleadoDTO>(new DapperDTO
                    {
                        baseDatos = MainContextEnum.GCPLAN,
                        consulta = "SELECT clave_empleado, clave_depto, cc_contable, 1 AS empresa FROM tblRH_EK_Empleados WHERE clave_empleado = @clave",
                        parametros = new { clave = colaboradorCapacitacion.colaborador }
                    ...
                    {
                        baseDatos = MainContextEnum.Arrendadora,
                        consulta = "SELECT clave_empleado, clave_depto, cc_contable, 2 AS empresa FROM tblRH_EK_Empleados WHERE clave_empleado = @clave",
                        parametros = new { clave = colaboradorCapacitacion.colaborador }
                    });
                    ...
                    _context.tblS_CapacitacionIHHColaboradorCapacitacion.Add(colaboradorCapacitacion);
                    _context.SaveChanges();
            ...
            using (var dbContextTransaction = _context.Database.BeginTransaction())
            {
                try
                {
                    ...
                    if (colaboradorCapacitacionSIGOPLAN != null)
                    {
                        ...
                        {
                            baseDatos = MainContextEnum.Construplan,
                            consulta = "SELECT clave_empleado, nombre + ' ' + ape_paterno + ' ' + ape_materno AS nombre, 1 AS empresa FROM tblRH_EK_Empleados WHERE clave_empleado = @clave",
                            parametros = new { clave = colaboradorCapacitacionSIGOPLAN.colaborador }
                        });
                        empleadoConstruplan.AddRange(_context.Select<EmpleadoDTO>(new DapperDTO
                        {
                            baseDatos = MainContextEnum.GCPLAN,
                        ...
                        {
                            baseDatos = MainContextEnum.Arrendadora,
                            consulta = "SELECT clave_empleado, nombre + ' ' + ape_paterno + ' ' + ape_materno AS nombre, 2 AS empresa FROM tblRH_EK_Empleados WHERE clave_empleado = @clave",
                            parametros = new { clave = colaboradorCapacitacionSIGOPLAN.colaborador }
                        });
                        ...
                        //});
                        //CC
                        var centrosCostoConstruplan = _context.Select<dynamic>(new DapperDTO
                        {
                            baseDatos = MainContextEnum.Construplan,
                            consulta = "SELECT * FROM tblP_CC",
                        });
                        var centrosCostoArrendadora = _context.Select<dynamic>(new DapperDTO
                        {
                            baseDatos = MainContextEnum.Arrendadora,
                            consulta = "SELECT * FROM tblP_CC",
                        });
...
//                    });
                    ...
                    var empleadoArrendadora = _context.Select<dynamic>(new DapperDTO
                    {
                        ...
                        consulta = @"SELECT
                                        ...
                                        2 AS empresa 
                                    FROM tblRH_EK_Empleados emp
                                        LEFT JOIN tblRH_EK_Puestos pue ON emp.puesto = pue.puestos",
        ...
        public List<HorasHombreDTO> postObtenerTablaHorasHombre(HorasHombreDTO parametros, bool ActivarHeader)
        {
            try
            {
                ...
                foreach (var s in parametros.comboCC)
                {
                    ...
                    {
                        var lstIdControlAsistencias = _context.tblS_CapacitacionControlAsistencia.Where(r => r.cc == CentroDeCosto && r.fechaCapacitacion >= item.FechaInicio && r.fechaCapacitacion <= item.FechaFin).ToList().Select(y => y.id).ToList();
                        var lstIdCursos = _context.tblS_CapacitacionControlAsistencia.Where(r => r.cc == CentroDeCosto && r.fechaCapacitacion >= item.FechaInicio && r.fechaCapacitacion <= item.FechaFin).ToList().Select(y => y.cursoID).ToList();


                        var sumaCapacitados = _context.tblS_CapacitacionControlAsistenciaDetalle.Where(r => r.cc == CentroDeCosto && lstIdControlAsistencias.Contains(r.controlAsistenciaID)).ToList().Count();
                        var sumaDuracion = _context.tblS_CapacitacionCursos.Where(r => lstIdCursos.Contains(r.id)).ToList().Sum(y => y.duracion);
...
//            var odbc = new OdbcConsultaDTO();
            ...
            string queryBase = @"SELECT 
                                ...
                                INNER JOIN tblRH_EK_Departamentos AS d ON e.clave_depto = d.clave_depto
                                INNER JOIN tblP_CC AS c ON e.cc_contable = c.cc
                                WHERE e.estatus_empleado ='A' AND e.cc_contable = '" + cc + "' ORDER BY c.cc, (e.nombre + ' ' + e.ape_paterno + ' ' + e.ape_materno)";
            ...
            if (cPlanOcArren == 1)
            {
                ...
                numerototal = departamentosCplan.Distinct().Count();
            }
            else
            {
                //odbc.consulta = String.Format(queryBase);
