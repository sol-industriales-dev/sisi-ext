...
{
    public class CapacitacionSeguridadCursosMandoMapping : EntityTypeConfiguration<tblS_CapacitacionSeguridadCursosMando>
    {
        public CapacitacionSeguridadCursosMandoMapping()
        {
            HasKey(x => x.id);
            Property(x => x.id).HasDatabaseGeneratedOption(DatabaseGeneratedOption.Identity).HasColumnName("id");

            Property(x => x.curso_id).HasColumnName("curso_id");
            HasRequired(x => x.Curso).WithMany(x => x.Mandos).HasForeignKey(d => d.curso_id);
            Property(x => x.mando).HasColumnName("mando");
            Property(x => x.division).HasColumnName("division");
            Property(x => x.estatus).HasColumnName("estatus");

            ToTable("tblS_CapacitacionSeguridadCursosMando");
        }
    }
}
