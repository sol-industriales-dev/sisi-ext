(() => {
    ...
    GetReporteEmpleadoCC = function () {
            ...
            btnSetRango.on('click', function () {
                ...
                if (strMensajeError != "") {
                    ...
                    if (cboxCentroCostos.val() != "" && cboxEmpleado.val() == "") {
                        fncGetReporteEmpleadoCC(`${btmDate[2]}-${btmDate[0]}-${btmDate[1]}`, `${topDate[2]}-${topDate[0]}-${topDate[1]}`, cboxCentroCostos.val(), null);
                    } else if (cboxEmpleado.val() != "" && cboxCentroCostos.val() == "") {
                        fncGetReporteEmpleadoCC(`${btmDate[1]}-1-${btmDate[0]}`, `${topDate[1]}-${ultimoDiaDelMes}-${topDate[0]}`, null, cboxEmpleado.val());
                    } else {
                        fncGetReporteEmpleadoCC(`${btmDate[1]}-1-${btmDate[0]}`, `${topDate[1]}-${ultimoDiaDelMes}-${topDate[0]}`, cboxCentroCostos.val(), cboxEmpleado.val());
                    }
                }
            });
        ...
        function initTblReporteAnualNomina(data, headers) {
            ...
            dtReporteEmpleadoCC = tblReporteEmpleadoCC.DataTable({
                ...
                columns: [
                    ...
                    { data: 'sueldoUnidades', title: 'Sueldo Unidades', visible: false },
                    { data: 'sueldoImporte', title: 'Sueldo Importe', visible: false },
                    { data: 'septimoDiaUnidades', title: 'Septimo Dia Unidades', visible: false },
                    { data: 'septimoDiaImporte', title: 'Septimo Día Importe', visible: false },
        ...
        //#region Excel
        function saveData(btm, tp, centroC, idEmpleado) {
            let dtData = [],
                dtTabla = dtReporteEmpleadoCC.data().toArray(),
                dtHeaders = [];

            var numCols = dtReporteEmpleadoCC.columns().nodes().length;

            //Aumentar rango para las columnas de totales y conceptos
            for (let i = 0; i < numCols; i++) {
                dtHeaders[i] = dtReporteEmpleadoCC.column(i).header().textContent;
            }
            ...
            axios.post('creaVariableDeSesionEmpleadoCC', { bottom: btm, top: tp, cc: centroC, empleado: idEmpleado, btm: `1 de ${getMonth(btmDate[0])} ${btmDate[1]}`, tp: `${ultimoDiaDelMes} de ${getMonth(topDate[0])} ${topDate[1]}`, tipoRaya: cboFiltroTipoRaya.val() }).then(response => {
                location.href = '/Administrativo/Nomina/crearReporteEmpleadoCC';
                let { success, items, message } = response.data;
                if (success) {
                }
            }).catch(error => Alert2Error(error.message));
