(() => {
$.namespace('Administrativo.RecursosHumanos.ListaNegra');
ListaNegra = function (){
        let itemsPorcentaje;
        const txtCve = $('#txtCve');
        ...
        function initDataTblBono() {
            dtTabla = tblData.DataTable({
                ...
                language: dtDicEsp,
                dom: 'Bfrtip',
                buttons: parametrosImpresion("Lista negra", "<center><h3>Lista negra para bono desempeño mensual</h3></center>"),
                buttons: [
                    {
                ...
                columns: [
                    { title:'CVE' ,data: 'cve', className: 'text-center', width: '10%',type: "string" },
                ...
                ,initComplete: function (settings, json) {
                    tblData.on(`click` ,`.eliminar` , function () {
                        ...
                        eliminarDeListaNegra(cve, $(this).closest('tr'));
                    });
                }
            });
        }
        ...
        function guardarEmpleado(empleado) {
            $.post('/Administrativo/Bono/GuardarEnListaNegra',
            {
                empleado
            }).then(response => {
                if (response.Success) {
        ...
        function empleadosListaNegra(){
            $.get('/Administrativo/Bono/EmpleadosListaNegra').then(response => {
                if (response.Success) {
        ...
        function eliminarDeListaNegra(claveEmpleado, row) {
            $.post('/Administrativo/Bono/EliminarDeListaNegra',
            {
                claveEmpleado
            }).then(response => {
                if (response.Success) {
    ...
    $(document).ready(() => {
        Administrativo.RecursosHumanos.ListaNegra = new ListaNegra();
    })
