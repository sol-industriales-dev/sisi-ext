(() => {
    ...
    CiclosTrabajo = function () {
                ...
                columns: [
                    ...
                    { data: 'fechaString', title: 'Fecha', sortable: false },
                    {
                        title: '', render: function (data, type, row, meta) {
                            return `<button class="btn btn-xs btn-primary botonVerDetalles"><i class="fa fa-eye"></i></button>`;
                        }
                    }
                ],
                columnDefs: [
        ...
        function guardarRegistroCiclo() {
            ...
            axios.post('GuardarRegistroCiclo', { registroCiclo, listaRevisiones, listaPropuestas, listaAreas })
                .then(response => {
                    let { success, datos, message } = response.data;
        ...
        function initTablaSeguimientoAcciones() {
            dtSeguimientoAcciones = tablaSeguimientoAcciones.DataTable({
                ...
                columns: [
                    ...
                    { data: 'colaboradorDesc', title: 'Colaborador' },
                    {
                        title: 'Aprobó', render: function (data, type, row, meta) {
                            return row.rutaEvidencia != null ? `
                                <div>
                                    <input type="checkbox" id="checkBoxAccion_${meta.row}" class="regular-checkbox" ${row.aprobo ? `checked` : ``} ${row.evaluador > 0 || (_privilegioUsuario != 1 && _privilegioUsuario != 4) ? 'disabled' : ''}>
                                    <label for="checkBoxAccion_${meta.row}"></label>
                                </div>
                            ` : ``;
                        }
                    },
                    {
                        title: 'Comentarios', render: function (data, type, row, meta) {
                            return row.rutaEvidencia != null ? `<input class="form-control inputComentariosEvaluador" value="${row.comentariosEvaluador}" ${row.evaluador > 0 || (_privilegioUsuario != 1 && _privilegioUsuario != 4) ? 'disabled' : ''}>` : ``;
                        }
                    }
                ],
                columnDefs: [
        ...
        function buscarSeguimiento() {
            ...
            axios.post('GetListaSeguimiento', { listaCC, tipoSeguimiento, fechaInicio, fechaFin })
                .then(response => {
                    let { success, datos, message } = response.data;
