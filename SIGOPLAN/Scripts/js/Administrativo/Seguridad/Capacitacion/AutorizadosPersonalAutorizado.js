(() => {
    $.namespace('Adminstrativo.Seguridad.Capacitacion.AutorizadosPersonalAutorizado');
    AutorizadosPersonalAutorizado = function () {
        ...
        const report = $("#report");
        const txtFechaAsistente = $('#txtFechaAsistente');
        ...
        (function init() {
            ...
            inputClaveEmpleado.getAutocompleteValid(setDatosEmpleado, verificarEmpleado, { porClave: true }, '/Administrativo/Capacitacion/GetEmpleadoEnKontrolAutocomplete');
            inputNombreEmpleado.getAutocompleteValid(setDatosEmpleado, verificarEmpleado, { porClave: false }, '/Administrativo/Capacitacion/GetEmpleadoEnKontrolAutocomplete');
        ...
        function initTablaAsistentes() {
            dtAsistentes = tablaAsistentes.DataTable({
                ...
                createdRow: function (row, rowData) {
                    $(row).find('.inputFechaAsistencia').datepicker();
                },
                columns: [
                    ...
                    {
                        data: 'fechaAsistenciaString', title: 'Fecha', render: function (data, type, row, meta) {
                            return `<input class="form-control text-center inputFechaAsistencia" value="${data}">`;
                        }
                    },
                    {
        ...
        function agregarAsistente() {
            ...
            agregarRowAsistente(id, nombreEmpleado, puestoEmpleado, ccID, `${ccID} - ${cc}`, txtFechaAsistente.val());
            inputNombreEmpleado.val('');
            inputClaveEmpleado.val('');
            txtFechaAsistente.val("")
            inputNombreEmpleado.focus();
        }

        function agregarRowAsistente(claveEmpleado, nombreEmpleado, puestoDesc, cc, ccDesc, fecha) {
        ...
        function llenarPantalla(datos) {
            ...
            let departamentosPeru = datos.listaCC.filter((x) => x.empresa == 6);
            ...
            if (departamentosPeru.length > 0) {
                let listaCCUnicos = departamentosPeru.map((x) => x.cc).filter(onlyUnique)

                listaCCUnicos.forEach((x) => {
                    let groupOption = `<optgroup label="PERU - ${x}"></optgroup>`;
                    let listaDepartamentosFiltrados = departamentosPeru.filter((y) => y.cc == x);
            ...
            divInteresados.find('*').not('#inputInteresados').remove();
        ...
        function getInformacionAutorizados() {
            ...
            tablaAsistentes.find('tbody tr').each(function (index, row) {
                let rowData = dtAsistentes.row(row).data();
                let fechaAsistencia = $(row).find('.inputFechaAsistencia').val();
            ...
            let datos = {
                ...
                secretarioCSH: (+inputEmpresa.val() != 6) ? inputSecretarioCSH.data().id : 0,
    ...
    $(document).ready(() => Adminstrativo.Seguridad.Capacitacion.AutorizadosPersonalAutorizado = new AutorizadosPersonalAutorizado())
})();