let GestionControllers = function () {
    ...
    const initTablaCambiosCategoria = function () {
        dtGestionCambios = tblGestionCambios.DataTable({
            ...
            columns: [
                ...
                },
                {
                    ...
                    render: function (data, type, row) {
                        ...
                        if (data == false) {
                            htmlBotones += `<button class='btn btn-danger subirArchivos' data-esActivo="0" data-id="${row.id}">` +
                                `<i class="fas fa-upload"></i></button>`;
                        } else {
                            htmlBotones += `<button class='btn btn-success descargarArchivos' data-esActivo="1" data-id="${row.id}">` +
                                `<i class="fas fa-eye"></i></button>`;
                        }

                        if (row.btnEliminarArchivos) {
                            htmlBotones += `&nbsp;<button class='btn btn-danger eliminarArchivos' data-id="${row.id}"><i class="fas fa-times"></i></button>`;
                        }
            ...
            drawCallback: function (settings) {
                ...
                $('.eliminarArchivos').on("click", function (e) {
                    let row = $(this).closest('tr');
                    let rowData = dtGestionCambios.row(row).data();

                    Alert2AccionConfirmar('Atención', '¿Desea eliminar los archivos cargados del formato de cambio seleccionado?', 'Confirmar', 'Cancelar', () => eliminarArchivos(rowData.id));
                });
            }
    ...
    const postObtenerTablaCambiosCategoria = function () {
        ...
        axios.post('/Capacitacion/TablaFormatosPendientes', { parametros: parametro })
    ...
    function eliminarArchivos(formatoCambio_id) {
        axios.post('EliminarArchivosFormatoCambio', { formatoCambio_id }).then(response => {
            let { success, data, message } = response.data;

            if (success) {
                Alert2Exito('Se han eliminado los archivos.');
                btnBuscar.click();
            } else {
                AlertaGeneral(`Alerta`, message);
            }
    ...
    const postObtenerTablaCambiosCategoriaModalDescargas = function () {
        ...
        axios.post('/Capacitacion/postObtenerTablaCambiosCategoriaModalDescargas', { parametro: parametro })
            .catch(o_O => AlertaGeneral(o_O.message))
            .then(response => {
                let { success, data } = response.data;
                if (success) {
    ...
    const ObtenerArchivos = function (idFormato) {
        axios.post('/Capacitacion/obtenerArchivoCODescargas', { idFormatoCambio: idFormato })
            .catch(o_O => AlertaGeneral(o_O.message))
            .then(response => {
                let { success, data } = response.data;
                if (success) {
                    btnDescargarExamen.attr('data-id', data[0].id)
                    btnDescargarReporteEvaluacion.attr('data-id', data[0].id)
                    btnDescargarSolicitudCambio.attr('data-id', data[1].id)
                    btnDescargarListadoCambio.attr('data-id', data[2].id)
                    $('#pExamen').text(data[0].rutaArchivo.split('\\')[8]);
                    $('#pReporteEvaluacion').text(data[0].rutaArchivo.split('\\')[8]);
                    $('#pSolicitudCambio').text(data[1].rutaArchivo.split('\\')[8]);
                    $('#pLimitadoCambio').text(data[2].rutaArchivo.split('\\')[8]);
                }
            });
    }
