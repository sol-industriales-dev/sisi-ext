(() => {
    $.namespace('Administrativo.Seguridad.Capacitacion.MatrizNecesidades');
    ...
    MatrizNecesidades = function () {
        ...
        const tablaPrioridades = $('#tablaPrioridades');
        let dtTablaPrioridades = null;
        ...
        (function init() {
            ocultarFiltroArrendadoraCC();

            // Lógica de inicialización.
            llenarCombos();
            agregarListeners();
            initTablaEstadisticas();
            initTablaPrioridades();
        ...
        function llenarCombos() {
            ...
            // comboArea.fillCombo('/Administrativo/Capacitacion/GetAreas', null, false, 'Todos');
            convertToMultiselect('#comboArea');
            comboClasificacion.fillCombo('/Administrativo/Capacitacion/GetClasificacionCursos', null, false, 'Todos');
        ...
        function ocultarFiltroArrendadoraCC() {
            let divisionActual = +new URLSearchParams(window.location.search).get('division');
            let ocultar = false;
            ...
            if (ocultar) { //Se oculta el div del filtro de Arrendadora y se reacomoda el HTML de los demás filtros.
                $('#divFiltroArrendadora').css('display', 'none');

                $('#divFiltroConstruplan').removeClass('col-md-5').addClass('col-md-4');
                $('#divFiltroArea').removeClass('col-md-5').addClass('col-md-3');
                $('#divFiltroClasificacion').removeClass('col-md-5').addClass('col-md-3');

                $('#divFiltroArea').appendTo('#divRowFiltros1');
                $('#divFiltroClasificacion').appendTo('#divRowFiltros1');
                $('#divBotonBuscar').appendTo('#divRowFiltros1');

                //Se cambia el id para que no se eliminen los nuevos divs en el siguiente paso.
                $('#divRowFiltros1').find('#divFiltroArea').attr('id', 'divFiltroArea_Nuevo');
                $('#divRowFiltros1').find('#divFiltroClasificacion').attr('id', 'divFiltroClasificacion_Nuevo');
                $('#divRowFiltros1').find('#divBotonBuscar').attr('id', 'divBotonBuscar_Nuevo');

                $('#divFiltroArea').remove();
                $('#divFiltroClasificacion').remove();
                $('#divBotonBuscar').remove();
            }
        }
        ...
        function cargarDatosMatrizNecesidades() {
            ...
            axios.post('/Administrativo/Capacitacion/CargarDatosMatrizNecesidades', { ccsCplan, ccsArr, departamentosIDs, clasificaciones })
                .then(response => {
                    ...
                    if (success) {
                        ...
                        initChartPorcentajeTotalClasificacionHighCharts(response.data.porcentajeTotalClasificacion);
        ...
        function initChartPorcentajeTotalClasificacionHighCharts(datos) {
            Highcharts.chart('chartPorcentajeTotalClasificacion', {
                chart: {
                    zoomType: 'xy'
                },
                lang: highChartsDicEsp,
                title: { text: 'Porcentaje de cumplimiento por clasificación' },
                xAxis: {
                ...
                series: [
                    { name: datos.serie1Descripcion, type: 'column', yAxis: 1, data: datos.serie1, color: 'rgb(237, 125, 49)' },
                    { name: '', type: 'line', data: datos.serie2, color: 'red' }
                ],
        ...
        function cargarDatosSeccion(seccion) {
            ...
            axios.post('/Administrativo/Capacitacion/CargarDatosSeccionMatriz', { ccsCplan, ccsArr, departamentosIDs, clasificaciones, seccion })
                .then(response => {
                    let { success, datos, message } = response.data;
        ...
        var titleIdentificacion = "";
        switch (idEmpresa.val()) {
            case '3':
                titleIdentificacion = 'CUI';
                break;
            case '6':
                titleIdentificacion = 'DNI';
                break;
            default:
                titleIdentificacion = 'CURP';
                break;
        }
        function establecerColumnasCursos(listadoEmpleados, columnasCursos) {
            ...
            const columnas = [
                ...
                { data: 'curp', title: titleIdentificacion },
                { data: 'nombre', title: 'Nombre' },
                { data: 'puesto', title: 'Puesto' },
                { data: 'departamentoEmpleado', title: 'Área' }
            ];
        ...
        function initTablaPrioridades() {
            dtTablaPrioridades = tablaPrioridades.DataTable({
                retrieve: true,
                paging: false,
                searching: false,
                language: dtDicEsp,
                ...
                columns: [
                    ...
                    { data: 'rango', title: 'Rango' }
                ],
                createdRow: function (row, rowData) {
                    switch (rowData.tipoColor) {
                        case 'rojo':
                            $(row).find('td:eq(0)').css('background-color', 'rgb(192, 0, 0)');
                            $(row).find('td:eq(0)').css('color', 'white');
                            break;
                        case 'amarillo':
                            $(row).find('td:eq(0)').css('background-color', 'rgb(255, 192, 0)');
                            $(row).find('td:eq(0)').css('color', 'white');
                            break;
                        case 'verde':
                            $(row).find('td:eq(0)').css('background-color', 'rgb(0, 176, 80)');
                            $(row).find('td:eq(0)').css('color', 'white');
                            break;
                    }
                },
            ...
            AddRows(tablaPrioridades, [
                { prioridad: 'Alta (Urgente)', rango: '0% - 59%', tipoColor: 'rojo' },
                { prioridad: 'Media (Importante)', rango: '60% - 85%', tipoColor: 'amarillo' },
                { prioridad: 'Baja (Monitorear)', rango: '86% - 100%', tipoColor: 'verde' }
            ]);
        }
        ...
        function initTablaIndicadoresGlobales() {
            dtTablaIndicadoresGlobales = tablaIndicadoresGlobales.DataTable({
                ...
                createdRow: function (row, rowData) {
                    if (rowData.porcentajeCapacitacionNumero >= 0 && rowData.porcentajeCapacitacionNumero < 60) { //Rojo
                        $(row).css('background-color', 'rgb(224, 132, 132)');
                    } else if (rowData.porcentajeCapacitacionNumero > 60 && rowData.porcentajeCapacitacionNumero < 85) { //Amarillo
                        $(row).css('background-color', 'rgb(254, 224, 132)');
                    } else if (rowData.porcentajeCapacitacionNumero >= 86 && rowData.porcentajeCapacitacionNumero <= 100) { //Verde
                        $(row).css('background-color', 'rgb(132, 216, 170)');
                    }
                },
                columnDefs: [
                    { className: "dt-center", "targets": "_all" }
    ...
    $(() => Administrativo.Seguridad.Capacitacion.MatrizNecesidades = new MatrizNecesidades())
