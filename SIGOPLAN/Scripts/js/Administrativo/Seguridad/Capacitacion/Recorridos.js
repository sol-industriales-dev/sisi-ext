(() => {
    ...
    Recorridos = function () {
        ...
        const celdaRecorridosRealizados = $('#celdaRecorridosRealizados');
        const celdaRecorridosRequeridos = $('#celdaRecorridosRequeridos');
        const celdaPorcentajeCumplimiento = $('#celdaPorcentajeCumplimiento');
        //#endregion
        ...
        //#region CONST CREAR/ELIMINAR EVIDENCIA DE RECORRIDOS
        const mdlCEEvidencias = $("#mdlCEEvidencias")
        const txtArchivoAdjunto = $("#txtArchivoAdjunto")
        const btnGuardarEvidenciaRecorrido = $("#btnGuardarEvidenciaRecorrido")
        const tblEvidenciasRecorrido = $("#tblEvidenciasRecorrido")
        let dtEvidenciaRecorrido;
        //#endregion

        //#region ESTADISTICAS POR AREA
        //Operacion
        const spanActosDetectadosOp = $('#spanActosDetectadosOp');
        const spanActosSolventadosOp = $('#spanActosSolventadosOp');
        const spanPorcSolventadosOp = $('#spanPorcSolventadosOp');
        const spanCondicionesDetectadosOp = $('#spanCondicionesDetectadosOp');
        const spanCondicionesSolventadoOp = $('#spanCondicionesSolventadoOp');
        const spanPorcCorreccionesOp = $('#spanPorcCorreccionesOp');
        const spanAccionesEficientesYSegurasOp = $('#spanAccionesEficientesYSegurasOp');

        //Mantenimiento/Overhaul
        const spanActosDetectadosMante = $('#spanActosDetectadosMante');
        const spanActosSolventadosMante = $('#spanActosSolventadosMante');
        const spanPorcSolventadosMante = $('#spanPorcSolventadosMante');
        const spanCondicionesDetectadosMante = $('#spanCondicionesDetectadosMante');
        const spanCondicionesSolventadoMante = $('#spanCondicionesSolventadoMante');
        const spanPorcCorreccionesMante = $('#spanPorcCorreccionesMante');
        const spanAccionesEficientesYSegurasMante = $('#spanAccionesEficientesYSegurasMante');
        //#endregion
        ...
        (function init() {
            ...
            inputRealizador.getAutocompleteValid(setDatosRealizador, verificarRealizador, { porClave: false }, '/Administrativo/Capacitacion/GetUsuariosAutocomplete');
            inputRealizadorNuevo.getAutocompleteValid(setDatosRealizadorNuevo, verificarRealizadorNuevo, { porClave: false }, '/Administrativo/Capacitacion/GetUsuariosAutocomplete');
            ...
            //#region FUNCIONES RECORRIDOS EVIDENCIAS
            btnGuardarEvidenciaRecorrido.click(function () {
                fncGuardarEvidenciaRecorrido()
            })
            //#endregion
        })();
        ...
        btnSendMail.click(function () {
            if (slMCorreos.val() == 0) {
                Alert2Warning("No se han seleccionado líderes.");
                return;
            }

            axios.post('VerificarCorreosValidos', { listaUsuariosID: getValoresMultiples("#slMCorreos") }).then(response => {
                let { success, data, message } = response.data;

                if (success) {
                    //#region Continuar con la función de enviar correo (código viejo)
        ...
        function initTablaGenerales() {
            dtGenerales = tablaGenerales.DataTable({
                ...
                initComplete: function (settings, json) {
                    ...
                    tablaGenerales.on('click', '.botonQuitarRecorrido', function () {
                        let rowData = tablaGenerales.DataTable().row($(this).closest('tr')).data();

                        Alert2AccionConfirmar('Atención', '¿Desea eliminar el registro seleccionado?', 'Confirmar', 'Cancelar', () => {
                            axios.post('EliminarRegistroRecorrido', { recorrido_id: rowData.id }).then(response => {
                                let { success, data, message } = response.data;

                                if (success) {
                                    Alert2Exito('Se ha borrado la información.');
                                    buscarRecorridos();
                                } else {
                                    AlertaGeneral(`Alerta`, message);
                                }
                    ...
                    { data: 'realizadorDesc', title: 'REALIZADOR' },
                    {
                        title: 'CONSULTAR', render: function (data, type, row, meta) {
                            return `
                                ...
                                ${_privilegioUsuario == 1 ? `<button class="btn btn-sm btn-danger botonQuitarRecorrido"><i class="fa fa-times"></i></button>` : ``}
                            `;
                        }
                    }
                ],
        ...
        function initTablaSeguimiento() {
            dtSeguimiento = tablaSeguimiento.DataTable({
                ...
                columns: [
                    ...
                    {
                        data: "solventada", title: 'Solventada', className: 'dt-body-center',
                            ...
                            if (row.solventada == 0) {
                                html = `
                                    <div class="radioBtn btn-group">
                                        ...
                                        <a class="btn btn-info btnEvidencia"><i class="far fa-folder-open"></i>&nbsp;Cargar soporte</a>
                                    </div>`;
                            } else {
                                html += `<button class="btn btn-primary btnVisualizarEvidencia" title="Visualizar soporte."><i class="fas fa-file-pdf"></i>&nbsp;Visualizar soporte</button>`
                            }
                ...
                initComplete: function (settings, json) {
                    ...
                    tablaSeguimiento.on('click', '.btnEvidencia', function () {
                        let rowData = dtSeguimiento.row($(this).closest('tr')).data();
                        btnGuardarEvidenciaRecorrido.data().idRecorrido = rowData.id
                        fncGetEvidenciasRecorrido()
                        mdlCEEvidencias.modal("show")
                    });

                    tablaSeguimiento.on('click', '.btnVisualizarEvidencia', function () {
                        let rowData = dtSeguimiento.row($(this).closest('tr')).data();
                        fncVisualizarEvidenciaRecorrido(rowData.id)
                    });
                },
        ...
        function buscarRecorridos() {
            ...
            axios.post('GetRegistrosRecorridos', { listaCC, listaAreas, fechaInicio, fechaFin, realizador })
                .then(response => {
                    let { success, datos, message } = response.data;

                    if (success) {
                        $('#divTab').find('.tab-pane').removeClass('hide');
                        celdaRecorridosRealizados.text(response.data.realizados);
                        celdaRecorridosRequeridos.text(response.data.requeridos);
                        celdaPorcentajeCumplimiento.text(response.data.porcentajeCumplimiento + '%');
            ...
            axios.post('CargarDashboardRecorridos', { listaCC, listaAreas, fechaInicio, fechaFin, realizador })
                .then(response => {
                    let { success, datos, message } = response.data;
                    ...
                    if (success) {
                        ...
                        //POR AREA
                        //OPERACION
                        spanActosDetectadosOp.text(response.data.listaActosOp.detectados);
                        spanActosSolventadosOp.text(response.data.listaActosOp.solventados);
                        spanPorcSolventadosOp.text(response.data.listaActosOp.porcentajeCumplimiento);
                        spanCondicionesDetectadosOp.text(response.data.listaCondicionesOp.detectados);
                        spanCondicionesSolventadoOp.text(response.data.listaCondicionesOp.solventados);
                        spanPorcCorreccionesOp.text(response.data.listaCondicionesOp.porcentajeCumplimiento + "%");
                        spanAccionesEficientesYSegurasOp.text(response.data.listaAccionesOp.acciones);

                        //MANTENIMIENTO
                        spanActosDetectadosMante.text(response.data.listaActosMante.detectados);
                        spanActosSolventadosMante.text(response.data.listaActosMante.solventados);
                        spanPorcSolventadosMante.text(response.data.listaActosMante.porcentajeCumplimiento);
                        spanCondicionesDetectadosMante.text(response.data.listaCondicionesMante.detectados);
                        spanCondicionesSolventadoMante.text(response.data.listaCondicionesMante.solventados);
                        spanPorcCorreccionesMante.text(response.data.listaCondicionesMante.porcentajeCumplimiento + "%");
                        spanAccionesEficientesYSegurasMante.text(response.data.listaAccionesMante.acciones);
        ...
        function initTblEvidenciasRecorrido() {
            dtEvidenciaRecorrido = tblEvidenciasRecorrido.DataTable({
                language: dtDicEsp,
                destroy: false,
                paging: true,
                ordering: true,
                ...
                initComplete: function (settings, json) {
                    tblEvidenciasRecorrido.on('click', '.eliminarRegistro', function () {
                        let rowData = dtEvidenciaRecorrido.row($(this).closest('tr')).data();
                        Alert2AccionConfirmar('¡Cuidado!', '¿Desea eliminar el registro seleccionado?', 'Confirmar', 'Cancelar', () => fncEliminarEvidenciaRecorrido(rowData.id));
                    });
                },
                columnDefs: [
                    { className: 'dt-center', 'targets': '_all' },
        ...
        function fncVisualizarEvidenciaRecorrido(FK_Recorrido) {
            if (FK_Recorrido > 0) {
                let obj = new Object();
                obj.FK_Recorrido = FK_Recorrido
                axios.post("VisualizarEvidenciaRecorrido", obj).then(response => {
                    let { success, items, message } = response.data;
                    if (success) {
        ...
        function fncEliminarEvidenciaRecorrido(idArchivo) {
            if (idArchivo > 0) {
                let obj = {};
                obj.idArchivo = idArchivo;
                axios.post('EliminarEvidenciaRecorrido', obj).then(response => {
                    let { success, items, message } = response.data;
                    if (success) {
                        Alert2Exito(message)
                        fncGetEvidenciasRecorrido()
        ...
        const fncGuardarEvidenciaRecorrido = function () {
            var data = fncGetEvidenciaParaGuardar();
            let obj = new Object();
            obj.FK_Recorrido = btnGuardarEvidenciaRecorrido.data().idRecorrido
            axios.post('GuardarEvidenciaRecorrido', data, { params: FK_Recorrido = obj }, { headers: { 'Content-Type': 'multipart/form-data' } }).then(response => {
                let { success, datos, message } = response.data;
                if (success) {
                    Alert2Exito(message)
                    fncGetEvidenciasRecorrido()
        ...
        function fncGetEvidenciasRecorrido() {
            if (btnGuardarEvidenciaRecorrido.data().idRecorrido > 0) {
                let obj = {};
                obj.FK_Recorrido = btnGuardarEvidenciaRecorrido.data().idRecorrido
                axios.post('GetEvidenciasRecorrido', obj).then(response => {
                    let { success, items, message } = response.data;
                    if (success) {
                        //#region FILL DATATABLE
                        dtEvidenciaRecorrido.clear();
                        dtEvidenciaRecorrido.rows.add(response.data.lstArchivos);
                        dtEvidenciaRecorrido.draw();
