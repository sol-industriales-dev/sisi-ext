(function () {
    ...
    CapturaCursos = function () {
        ...
        //#region PETICIONES
        const getCursos = (clasificaciones, puestos, estatus) => { return $.post('/Administrativo/CapacitacionSeguridad/GetCursos', { clasificaciones, puestos, estatus }) };
        const getTipoExamen = (curso_id) => { return $.post('/Administrativo/CapacitacionSeguridad/GetTipoExamen', { curso_id }) };
        const GuardarCurso = (curso) => { return $.post('/Administrativo/CapacitacionSeguridad/GuardarCurso', { curso }) };
        const ActualizarCurso = (curso, mandos, puestosNuevos, puestosAutorizacionNuevos, centrosCosto) => {
            return $.post('/Administrativo/CapacitacionSeguridad/ActualizarCurso', { curso, mandos, puestosNuevos, puestosAutorizacionNuevos, centrosCosto })
        };
        const GuardarExamenes = (examenes) => { return $.post('/Administrativo/CapacitacionSeguridad/GuardarExamenes', { examenes }) };
        const DescargarArchivo = (examen_id) => { return $.post('/Administrativo/CapacitacionSeguridad/DescargarArchivo', { examen_id }) };
        const EliminarExamen = (examen_id) => { return $.post('/Administrativo/CapacitacionSeguridad/EliminarExamen', { examen_id }) };
        //const getClasificacion = () => { return $.post('/Administrativo/CapacitacionSeguridad/GetClasificacionCursos') };
        //#endregion
        ...
        function agregarListeners() {
            ...
            $('#btnImprimir').on('click', function () {
                ...
                $(this).download = '/Administrativo/CapacitacionSeguridad/CrearExcelRelacionCursosPuestos';
                $(this).href = '/Administrativo/CapacitacionSeguridad/CrearExcelRelacionCursosPuestos';

                location.href = '/Administrativo/CapacitacionSeguridad/CrearExcelRelacionCursosPuestos';
                    ...
                    { data: 'clasificacionDesc', title: 'Clasificación' },
                    {
                        ...
                        render: function (data, type, row, meta) {
                            return _privilegioUsuario != 3 ? `
                                ...
                                ${(row.clasificacion != 3 && row.clasificacion != 4 && row.clasificacion != 11) ? `<button title="Gestionar exámenes" class="btn-alta-examen btn btn-xs btn-info" value="${row.id}"><i class="far fa-file-word"></i></button>&nbsp;` : ``}
        ...
        function eliminarCurso(cursoID) {
            ...
            $.post('/Administrativo/CapacitacionSeguridad/EliminarCurso', { cursoID })
                .then(response => {
                    if (response.success) {
                        // Operación exitosa.
        ...
        function guardarCargaMasiva() {
            ...
            request.open("POST", "/Administrativo/CapacitacionSeguridad/GuardarCargaMasivaRelacionCursosPuestosAutorizacion");
