(() => {
    $.namespace('Adminstrativo.Seguridad.CapacitacionSeguridad.ControlAsistencia');

    ControlAsistencia = function () {

        // Variables.
        //// Url autocompletes
        const getCursosAutocompleteURL = '/Administrativo/CapacitacionSeguridad/GetCursosAutocomplete';
        const getUsuariosAutocompleteURL = '/Administrativo/CapacitacionSeguridad/GetUsuariosAutocomplete';
        const getGetLugarCursoAutocompleteURL = '/Administrativo/CapacitacionSeguridad/GetLugarCursoAutocomplete';
        const getEmpleadoEnKontrolAutocompleteURL = '/Administrativo/CapacitacionSeguridad/GetEmpleadoEnKontrolAutocomplete';
        ...
        function guardarExamenesAsistentes() {
            ...
            if (esCursoGeneral) {
                ...
                const asistentes = tablaExamenes.find('tbody tr[role="row"]').toArray();
                ...
                for (let index = 0; index < asistentes.length; index++) {
                    ...
                    $.ajax({
                        url: '/Administrativo/CapacitacionSeguridad/CargarExamenesAsistente',
                        data,
                        async: false,
                        cache: false,
                        contentType: false,
                ...
                $.post('/Administrativo/CapacitacionSeguridad/GuardarExamenesAsistentes', { jefeID, coordinadorID, secretarioID, gerenteID, rfc, razonSocial })
                    .always($.unblockUI)
                    .then(response => {
                        if (response.success) {
                            // Operación exitosa.
            ...
            for (let index = 0; index < asistentes.length; index++) {
                ...
                if (inputFile != undefined) {
                    ...
                    if (archivoDC3 != undefined) {
                        ...
                        $.ajax({
                            url: '/Administrativo/CapacitacionSeguridad/GuardarArchivosDC3',
                            data,
                            async: false,
                            cache: false,
                            contentType: false,
        ...
        function guardarEvaluacionesAsistentes() {
            ...
            $.post('/Administrativo/CapacitacionSeguridad/GuardarEvaluacionAsistentes', { listaAsistentes })
                .always($.unblockUI)
                .then(response => {
                    if (response.success) {
                        // Operación exitosa.
        ...
        function crearListaAsistencia() {
            ...
            $.post('/Administrativo/CapacitacionSeguridad/CrearControlAsistencia', { controlAsistencia })
                .always($.unblockUI)
                .then(response => {
                    if (response.success) {
                        // Operación exitosa.
        ...
        function cargarListaControlAsistencia() {
            ...
            $.get('/Administrativo/CapacitacionSeguridad/ObtenerListaControlesAsistencia', { cc, estado, fechaInicio, fechaFin })
                .always($.unblockUI)
                .then(response => {
                    if (response.success) {
                        // Operación exitosa.
        ...
        function llenarCombos() {
            selectRazonSocial.fillCombo('/Administrativo/CapacitacionSeguridad/GetRazonSocialCombo', null, false, null);
            ...
            axios.get('ObtenerComboCCAmbasEmpresas')
                .then(response => {
                    ...
                    if (success) {
                        ...
                        items.forEach(x => {
                            let groupOption = `<option value="Todos">Todos</option><optgroup label="${x.label}"></optgroup>`;
                            if (idEmpresa == 6) {
                                x.options.forEach(y => {
                                    groupOption += `<option value="${y.Value}" empresa="${x.label == 'PERÚ' ? 6 : 0}">${y.Text}</option>`;
                                });
                                ...
                                let groupOption2 = `<option value="-1">Seleccionar</option><optgroup label="${x.label}"></optgroup>`
                                x.options.forEach(y => {
            ...
            // $.get('/Administrativo/CapacitacionSeguridad/ObtenerComboCCAmbasEmpresas')
            //     .then(response => {
            //         if (response.success) {
            //             // Operación exitosa.
            ...
            //             comboAltaCC.append(response.items.map(item => `<option value=${item.Value} >${item.Text}</option>`).join(''));
            //         } else {
            //             // Operación no completada.
            //             AlertaGeneral(`Operación fallida`, `No se pudo completar la operación: ${response.message}`);
            //         }
            ...
            $.get('/Administrativo/CapacitacionSeguridad/ObtenerEstatusControlAsistencia')
                .always($.unblockUI)
                .then(response => {
                    if (response) {
                        // Operación exitosa.
        ...
        function cargarArchivoControlAsistencia() {
            ...
            $.ajax({
                url: '/Administrativo/CapacitacionSeguridad/SubirArchivoControlAsistencia',
                data,
                cache: false,
                contentType: false,
                processData: false,
        ...
        function initTablaControlAsistencia() {
            ...
            dtTablaControlAsistencia = tablaControlAsistencia.DataTable({
                ...
                columns: [
                    ...
                    {
                        data: 'id', title: 'DC-3', render: (data, type, row) =>
        ...
        function eliminarControlAsistencia(controlAsistenciaID) {
            ...
            $.post('/Administrativo/CapacitacionSeguridad/EliminarControlAsistencia', { controlAsistenciaID })
                .then(response => {
                    if (response.success) {
                        // Operación exitosa.
        ...
        function cargarAsistentes(controlAsistencia) {
            ...
            $.get('/Administrativo/CapacitacionSeguridad/CargarAsistentesCapacitacion', { controlAsistenciaID: controlAsistencia.id })
                .always($.unblockUI)
                .then(response => {
                    if (response.success) {
                        // Operación exitosa.
        ...
        function cargarDatosControlAsistencia(controlAsistenciaID) {
            ...
            $.get('/Administrativo/CapacitacionSeguridad/CargarDatosControlAsistencia', { controlAsistenciaID })
                .always($.unblockUI)
                .then(response => {
                    if (response.success) {
                        // Operación exitosa.
            ...
            let columnas = [
                ...
                { data: 'estatusAutorizacionDesc', title: 'Estatus Autorización' },
                {
                    data: 'tipoExamen', title: 'Examen Base', render: (data, type, row) => {
                        if (row.clasificacion != 3 && row.clasificacion != 4 && row.clasificacion != 11) {
                            return `
                            <p>${row.tipoExamen}</p>
                ...
                columnas.push(
                    {
                        data: 'id', title: 'Examen Diagnóstico', render: (data, type, row) => {
                            ...
                            } else if (row.clasificacion != 3 && row.clasificacion != 4 && row.clasificacion != 11) {
        ...
        function initTablaDC3() {
            dtTablaDC3 = tablaDC3.DataTable({
                ...
                columns: [
                    ...
                    { data: 'estatusDesc', title: 'Estatus' },
                    {
                        data: 'id', title: 'Archivo DC-3', render: (data, type, row) => {
                            if (row.rutaDC3 == null) {
        ...
        function guardarCargaMasiva() {
            ...
            request.open("POST", "/Administrativo/CapacitacionSeguridad/GuardarCargaMasivaControlAsistencia");
    ...
    $(() => Adminstrativo.Seguridad.CapacitacionSeguridad.ControlAsistencia = new ControlAsistencia());
})();