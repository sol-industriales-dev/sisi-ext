(() => {
    ...
    $.namespace('HorasHombreCapacitacion.Capacitacion');
...
const report = $('#report')
    ...
    function events() {
        btnBuscar.click(function () {
            obtenerInputPromedios();
            postObtenerTablaHorasHombre();
        });
        btnDescargar.click(function(){
            $.blockUI({ message: 'Procesando...' });
        let parametros = getComboCCPos();

            axios.post('creaExcelito', {parametros:parametros})
    ...
    function postObtenerTablaHorasHombre() {
        let parametros = getComboCCPos();
        axios.post('postObtenerTablaHorasHombre', { parametros: parametros })
            .catch(o_O => AlertaGeneral(o_O.message))
            .then(response => {
                let { success, items } = response.data;
                if (success) {
                    console.log(items)
                    AddRows(tblHorasHombreCapacitacion, items)
                    // $('.table-bordered stripe order-column').parent().css('padding',0);
    ...
    function obtenerInputPromedios() {
        let parametros = getComboCCPos();
        axios.post('obtenerInputPromedios', {parametros:parametros})
            .catch(o_O => AlertaGeneral(o_O.message))
            .then(response => {
                let { success, items} = response.data;
                if (success) {
                    console.log(items)
                    $('#HrsCap').val(parseFloat(items.promedioHRSCapacitaciones).toFixed(2));
                    $('#HrsTrab').val(parseFloat(items.promedioHRSTrabajadas).toFixed(2));
                }
            });
    }

    function imprimirColaborador() {
        $.blockUI({ message: 'Procesando...', baseZ: 2000 });
        report.attr("src", `/Reportes/Vista.aspx?idReporte=214`);
    ...
    function verReporte() {
        ...
        let parametros = getComboCCPos();
        axios.post('CrearReporteHorasHombres', {parametros:parametros})
            .catch(o_O => AlertaGeneral(o_O.message))
            .then(response => {
    ...
    $(document).ready(() => {
        HorasHombreCapacitacion.Capacitacion = new Capacitacion();
    })
