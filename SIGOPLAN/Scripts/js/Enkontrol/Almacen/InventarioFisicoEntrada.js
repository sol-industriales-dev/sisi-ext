(() => {
    $.namespace('Enkontrol.Almacen.Almacen.InventarioFisicoEntrada');
    InventarioFisicoEntrada = function () {
        //#region Selectores
        ...
        (function init() {
            ...
            btnGuardar.click(guardarEntradaInventarioFisico);
        ...
        btnImprimir.on('click', function () {
            ...
            if (almacen == "" || numero == "") {
                AlertaGeneral(`Aviso`, `Debe capturar un almacén y un número.`);
                return;
            }

            $.blockUI({ message: 'Procesando...' });
            $.post('/Enkontrol/Almacen/GetReporteEntradaFisico', { almacen, numero })
                .always($.unblockUI)
                .then(response => {
                    if (response.success) {
                        verReporte();
        ...
        function cargarInformacion() {
            ...
            if ((almacenID != '' && !isNaN(almacenID)) && (numero != '' && !isNaN(numero))) {
                $.blockUI({ message: 'Procesando...' });
                $.post('/Enkontrol/Almacen/GetEntradaInventarioFisico', { almacenID, numero })
                    .always($.unblockUI)
                    .then(response => {
                        if (response.success) {
    ...
    $(document).ready(() => Enkontrol.Almacen.Almacen.InventarioFisicoEntrada = new InventarioFisicoEntrada())
