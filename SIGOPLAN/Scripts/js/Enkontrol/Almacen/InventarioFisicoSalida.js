(() => {
    $.namespace('Enkontrol.Almacen.Almacen.InventarioFisicoSalida');
    InventarioFisicoSalida = function () {
        //#region Selectores
        ...
        (function init() {
            ...
            btnGuardar.click(guardarSalidaInventarioFisico);
            btnImprimir.click(imprimirMovimientoSalidaInventarioFisico);
                    ...
                    .then(response => {
                        if (response.success) {
                            if (response.data != null) {
                                ...
                                if (!flagTraspaso) {
                                    $.post('/Enkontrol/Almacen/GetNuevaSalidaInventarioFisico', { almacenID })
                                        .then(response => {
                                            if (response.success) {
        ...
        tblPartidas.on('change', '.inputInsumo', function () {
            ...
            $.post('/Enkontrol/Requisicion/GetInsumoInformacionByAlmacen', { insumo, almacen }).always($.unblockUI).then(response => {
                if (response.success) {
                    ...
                    row.find('td:eq(5)').text(ins.costoPromedio);
                    ...
                    let costo_promedio = unmaskNumero($(row).find('td:eq(5)').text());
                    ...
                    // rowData.importe = nuevoImporte; //CHECAR IMPORTE
        ...
        btnGuardarExcel.on('click', function () {
            try {
                ...
                request.open("POST", "/Enkontrol/Almacen/CargarExcelSalidaFisico");
        ...
        function initTablePartidas() {
            tblPartidas.DataTable({
                ...
                initComplete: function (settings, json) {
                    ...
                    // tblPartidas.on('change', 'input', function () {
                    //     let row = $(this).closest('tr');
                    //     let rowData = tblPartidas.DataTable().row(row).data();
                ...
                initComplete: function (settings, json) {
                    tblExistencias.on('click', '.btnSeleccionarUbicacion', function () {
                        ...
                        if (_filaInsumo != null) {
                            ...
                            $(_filaInsumo).find('.inputArea').change();
        ...
        function cargarInformacion() {
            ...
            if ((almacenID != '' && !isNaN(almacenID)) && (numero != '' && !isNaN(numero))) {
                $.blockUI({ message: 'Procesando...' });
                $.post('/Enkontrol/Almacen/GetSalidaInventarioFisico', { almacenID, numero })
                    .always($.unblockUI)
                    .then(response => {
                        if (response.success) {
        ...
        function guardarSalidaInventarioFisico() {
            ...
            if (movimiento.detalle.length > 0) {
                ...
                $.blockUI({ message: 'Procesando...' });
                $.post('/Enkontrol/Almacen/GuardarSalidaInventarioFisico', { movimiento })
                    .always($.unblockUI)
                    .then(response => {
                        if (response.success) {
        ...
        function imprimirMovimientoSalidaInventarioFisico() {
            let almacen = inputAlmacenNum.val().trim();
            let numero = inputNumero.val().trim();

            if (datosImprimirInvalidos(+almacen, +numero)) {
                AlertaGeneral(`Alerta`, `Seleccione un almacén y un número de movimiento válido.`);
                return;
            }

            $.blockUI({ message: 'Consultando datos reporte...' });
            $.post('/Enkontrol/Almacen/ImprimirMovimientoSalidaInventarioFisico', { almacen, numero })
                .always($.unblockUI)
                .then(response => {
                    if (response.success) {
                        verReporte();
        ...
        function datosImprimirInvalidos(almacen, numero) {
            return (almacen <= 0 || numero <= 0);
        }
        ...
        function verReporte() {
            report.attr("src", `/Reportes/Vista.aspx?idReporte=123`);
    ...
    $(document).ready(() => Enkontrol.Almacen.Almacen.InventarioFisicoSalida = new InventarioFisicoSalida())
