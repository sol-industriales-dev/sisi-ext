(() => {
    ...
    Generar = function () {
        ...
        const textAreaCorreoProveedor = $('#textAreaCorreoProveedor');
        ...
        function fnEnviarOC() {
            ...
            if (textAreaCorreoProveedor.val() != '') {
                ...
                report.on('load', function () {
                    ...
                    formData.append("correo", textAreaCorreoProveedor.val());
        ...
        if (_empresaActual == 6) {
            $('#selectMoneda, #selectTipoCompraVenta').on('change', function () {
                ...
                if (moneda == 2) {
                    ...
                    axios.post('GetTipoCambioPeru').then(response => {
                        ...
                        if (success) {
                            if (+selectTipoCompraVenta.val() == 1) {
                                inputTipoCambio.val(maskNumero6DCompras(response.data.data.TIPOCAMB_COMPRA));
                            } else if (+selectTipoCompraVenta.val() == 4) {
                                inputTipoCambio.val(maskNumero6DCompras(response.data.data.TIPOCAMB_VENTA));
                            }
                        } else {
                    ...
                    inputTipoCambio.val(maskNumero6DCompras(1));
                }
            });
        ...
        function initTablePartidas() {
            tblPartidas.DataTable({
                ...
                createdRow: function (row, rowData) {
                    ...
                    // else {
                    //     $(row).find('.inputColocada').val(rowData.cantidad);
                    // }
                },
                columns: [
            ...
            getCompra().done(response => {
                if (response.success) {
                    if (response.info.cc != null) {
                        ...
                        _Colocada = response.info.colocada;
                        // btnOpenEnviar.prop("disabled", response.info.colocada); // HABILITAR BOTON SOLO SI ESTA AUTORIZADA LA OC

                        textAreaCorreoProveedor.val(response.info.correoProveedor);
                    } else {
                        AlertaGeneral('Alerta', 'No se encontró información.');
        ...
        function verReporteCompra(cc, numero) {
            ...
            report.on('load', function () {
                ...
                openCRModal();
                // if (_Colocada == true) {
                //     btnOpenEnviar.prop("disabled", true); // HABILITAR BOTON SOLO SI ESTA AUTORIZADA LA OC
                // }
                // else {
                //     btnOpenEnviar.prop("disabled", false); // HABILITAR BOTON SOLO SI ESTA AUTORIZADA LA OC
                // }
