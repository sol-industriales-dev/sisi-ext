(() => {
    ...
    Generar = function () {
        ...
        __flagValidacionInsumoSubcontratista = false;
        ...
        function usuarioExiste() {
            axios.post('/OrdenCompra/usuarioCompradorExiste').then(response => {
                ...
                if (success) {
                    // console.log("Se escondio por que no eres comprador");
                    $('#btnOpenEnviar').hide();

                }
                else {
                    // console.log("Se muestra por que eres comprador");
                    $('#btnOpenEnviar').show();
                }
        ...
        const guardarNuevaCompra = (compra) => $.post('/Enkontrol/OrdenCompra/GuardarNuevaCompraInterna', { compra: compra });
        ...
        function fnEnviarOC() {
            ...
            if (file != undefined && inputCorreoProveedor.val() != '') {
                ...
                $.unblockUI();
                AlertaGeneral("Alerta", "¡Ambos datos son requeridos!");
            }

        }

        //Peticiones para Editar Compra
        const getCompra = () => $.post('/Enkontrol/OrdenCompra/GetCompra_Interna', { cc: selectCCEditar.val(), num: inputNumeroEditar.val() != '' ? inputNumeroEditar.val() : 0 });
        const guardarCambios = (compra) => $.post('/Enkontrol/OrdenCompra/UpdateCompraInterna', { compra: compra });
                                ...
                                if (compra.anticipoBool) {
                                    if (compra.totalAnticipo > 0 && compra.totalAnticipo <= compra.total_rec) {
                                        ...
                                        guardarNuevaCompra(compra).done(response => {
                                            if (response.success) {
                                                ...
                                                AlertaGeneral('Alerta', 'Se ha guardado la información. Números de Compra: ' + response.numerosComprasString + '.');
                                    ...
                                    guardarNuevaCompra(compra).done(response => {
                                        if (response.success) {
                                            ...
                                            selectCCEditar[0].selectedIndex = 0;
        ...
        inputIVAPorcentaje.on('change', e => {
            ...
            if (!isNaN(valor)) {
                ...
                inputIVANumero.val(maskNumero6DCompras((subTotal * iva) / 100));
                inputTotal.val(maskNumero6DCompras(subTotal + (unmaskNumero6DCompras(inputIVANumero.val()))));
            } else {
        ...
        textAreaDescPartida.on('change', function () {
            ...
            let renglonActivoEditar = tblPartidas.find('tbody .active');
            let rowData = tblPartidas.DataTable().row(renglonActivo).data();
            let rowDataEditar = tblPartidas.DataTable().row(renglonActivoEditar).data();
        ...
        checkboxAnticipo.on('click', function () {
            ...
            inputTotalAnticipo.val(checked ? maskNumero6DCompras(0) : '');
        ...
        inputTotalAnticipo.on('change', function () {
            ...
            inputTotalAnticipo.val(maskNumero6DCompras(valor));
        });
        ...
        function initTablePartidas() {
            tblPartidas.DataTable({
                ...
                initComplete: function (settings, json) {
                    ...
                    tblPartidas.on('change', 'input, select', function () {
                        ...
                        if (!$(this).hasClass('inputProveedorDistinto')) {
                            ...
                            $.post('/Enkontrol/OrdenCompra/GetProveedorInfo', { num: proveedorDistinto != '' ? proveedorDistinto : 0 }).then(response => {
                                let rowData = tblPartidas.DataTable().row(row).data();
                                let insumo = unmaskNumero(row.find('.inputInsumo').val());
                                let insumoDesc = row.find('.inputInsumoDesc').val();
                                let fecha_entrega = row.find('.inputRequerido').val();
                    ...
                    tblPartidas.on("change", ".selectAreaCuenta", function () {
                        //OMAROMAR
                        // console.log("Entre");
                        let textSelect = $('.selectAreaCuenta option:selected').text();
                        let option = $('.selectAreaCuenta').find(`option[text="${textSelect}"]`);
                        let dataPrefijo = option.attr("data-prefijo");
                ...
                columns: [
                    ...
                    // {
                    //     data: 'costoPromedio', title: 'Costo Promedio', render: (data, type, row, meta) => {
                    //         let input = document.createElement('input');

                    //         input.classList.add('form-control', 'text-right', 'inputCostoPromedio');
                    //         $(input).attr('value', maskNumero(data == null ? 0 : data));
                    //         $(input).attr('disabled', true);
            ...
            tblPartidas.DataTable({
                ...
                initComplete: function (settings, json) {
                    ...
                    tblPartidas.on('change', 'input, select', function () {
                        ...
                        let precio = unmaskNumero6DCompras(row.find('.inputPrecio').val());
                        // let costoPromedio = unmaskNumero(row.find('.inputCostoPromedio').val());
                        ...
                        rowData.precio = precio;
                        // rowData.costoPromedio = costoPromedio;
                    ...
                    tblPartidas.on("change", ".selectAreaCuenta", function () {
                        //OMAROMAR
                        let textSelect = $('.selectAreaCuenta option:selected').text();
                        let area = $(`.selectAreaCuenta option:contains(${textSelect})`).val();
                        let cuenta = $(`.selectAreaCuenta option:contains(${textSelect})`).attr("data-prefijo");
                        $('.selectAreaCuenta option:selected').attr("area", area);
                        $('.selectAreaCuenta option:selected').attr("cuenta", cuenta);
                ...
                columns: [
                    ...
                    },
                    {
                        data: 'precio', title: 'Precio', render: (data, type, row, meta) => {
                            ...
                            $(input).attr('value', maskNumero6DCompras(data));
                            // $(input).attr('disabled', true);
                    ...
                    //         $(input).attr('value', maskNumero6DCompras(data == null ? 0 : data));
                    //         $(input).attr('disabled', true);
                ...
                columns: [
                    ...
                    { data: 'descRet', title: 'Desc Ret' },
                    {
                        data: 'cantidad', render: function (data, type, row, meta) {
                            let html = '<input id="inputRet_' + meta.row + '" class="form-control cantidad" style="text-align: right; height: 24px;" value="' + maskNumero6DCompras(data).replace('$', '') + '" data-orden="' + row.orden + '" data-id_cpto="' + row.id_cpto + '" data-cantidad="' + row.cantidad + '" />'
                    ...
                    },
                    {
                        data: 'porc_ret', render: function (data, type, row, meta) {
                            let html = '<input class="form-control text-right inputPorcentaje" value="' + maskNumero6DCompras(data).replace('$', '') + '" style="height: 24px;" />'
        ...
        function llenarInformacion(data) {
            ...
            inputTipoCambio.val(maskNumero6DCompras(data.tipo_cambio != 0 ? data.tipo_cambio : 1));
            inputSubTotal.val(maskNumero6DCompras(data.sub_total));
            inputIVAPorcentaje.val(data.porcent_iva != 0 ? data.porcent_iva : '');
            inputIVANumero.val(maskNumero6DCompras(data.iva));
            inputRetencion.val(maskNumero6DCompras(data.rentencion_despues_iva));
            inputTotal.val(maskNumero6DCompras(data.total));
            inputTotalFinal.val(maskNumero6DCompras(data.sub_total + data.iva - data.rentencion_despues_iva));
            ...
            inputTotalAnticipo.val(data.anticipoBool ? maskNumero6DCompras(data.totalAnticipo) : '');
        ...
        let esColocada = false;
        function getInfoCompra(flagEditar) {
            let compra = {
                //#region Panel Izquierdo
                cc: selectCCEditar.val(),
                numero: inputNumeroEditar.val() == '' ? 0 : inputNumeroEditar.val(),
            ...
            if (!flagEditar) {
                tblPartidas.find('tbody tr').each(function (idx, row) {
                    let rowData = tblPartidas/*Editar*/.DataTable().row($(row)).data();
                    ...
                    let partida = {
                        ...
                        cantidad: colocada > 0 ? colocada : cantidad,
                        ...
                        porcent_iva: unmaskNumero6DCompras(inputIVAPorcentaje.val()),
                        iva: unmaskNumero6DCompras(inputIVANumero.val()),
                        partidaDescripcion: rowData.partidaDescripcion,
                        proveedorDistinto: proveedorDistinto
                    };
            ...
            getProveedorInfo(inputProvNum.val()).done(function (response) {
                if (response != null) {
                    if (response.cancelado != 'C') {
                        ...
                        inputTipoCambio.val(maskNumero6DCompras(response.monedaTipoCambio));
        ...
        function calcularImportePorRenglon(row) {
            ...
            row.find('.inputImporte').val(maskNumero6DCompras(importe));
        }
        ...
        function calcularResultados() {
            ...
            inputSubTotal.val(maskNumero6DCompras(subTotal));
            inputIVAPorcentaje.change();
        }
        ...
        function calcularRetenciones() {
            ...
            inputRetencion.val(maskNumero6DCompras(totalRetenciones));
            inputTotalFinal.val(maskNumero6DCompras(total - totalRetenciones));
        }
        ...
        function setInsumoDesc(e, ui) {
            ...
            // row.find('.inputCostoPromedio').val(maskNumero6DCompras(ui.item.costoPromedio));
        }
        ...
        function validarInsumo(e, ul) {
            if (ul.item == null) {
                ...
                // row.find('.inputCostoPromedio').val(maskNumero6DCompras(0));
            }
        }
            ...
            getCompra().done(response => {
                if (response.success) {
                    if (response.info.cc != null) {
                        ...
                        tblPartidas.find('tbody tr:eq(0) td:eq(0)').click();
                        selectCategoria.val(response.info.categoria);
                        if (response.info.tieneCuadro) {
                            btnVerCuadro.css('display', 'inline-block');
                            btnNoTieneCuadro.css('display', 'none');
                            selectFolioCuadro.fillCombo('/Enkontrol/OrdenCompra/FillComboFolioCuadro', { cuadrosExistentes: response.info.cuadrosExistentes }, false);
                        ...
                        if (response.info.estatus == 'C') {
                            ...
                            if (response.info.ST_OC == 'A' && response.info.st_impresa == 'I') {
        ...
        function reInicializarCamposRenglon(row, rowData, flagEditar) {
            ...
            //selectAreaCuenta.fillCombo('/Enkontrol/Requisicion/FillComboAreaCuentaOCInterna', { cc: !flagEditar ? selectCC.val() : selectCCEditar.val() }, false, "000-000");

            let optionExistente = selectAreaCuenta.find(`option[value=${rowData.area}][data-prefijo=${rowData.cuenta}]`)[0];

            if (rowData.area != 0 && rowData.cuenta != 0) {
                if (optionExistente == undefined) {
                    selectAreaCuenta.append(`
                    <option value="${rowData.area}" name="undefined" data-prefijo="${rowData.cuenta}" data-comboid="undefined">${rowData.areaCuentaDesc}</option>
                `);
                }
            }
        ...
        function verReporteCompra(cc, numero) {
            ...
            report.attr("src", '/Reportes/Vista.aspx?idReporte=241' + '&cc=' + cc + '&numero=' + numero);
            report.on('load', function () {
                $.unblockUI();
                openCRModal();
                if (_Colocada == true) {
