(() => {
    ...
    Seguimiento = function () {
        ...
        btnBuscar.on('click', function () {
            ...
            let listaTipoInsumo = getValoresMultiples('#multiSelectTipoInsumo');
            let fechaInicial = inputFechaInicial.val();
            let fechaFinal = inputFechaFinal.val();
            let estatus = comboFiltroCompra.val();
            let compradorSugeridoEnReq = selectComprador.val() == "" ? 0 : selectComprador.val();
            let requisitor = selRequisitores.val() == "" ? 0 : selRequisitores.val();
            let consigna = +selectConsigna.val();

            $.post('/Enkontrol/Requisicion/GetRequisicionesSeguimiento', { listaCC, listaTipoInsumo, fechaInicial, fechaFinal, estatus, requisitor, consigna, compradorSugeridoEnReq }).then(response => {
                if (response.success) {
                ...
                columns: [
                    ...
                    { data: 'nivelAutOCEnt', title: 'Aut OC - Ent' },
                    {
                        render: function (data, type, row, meta) {
                            if (row.validadoAlmacen) {
                                return `<button class="btn btn-xs btn-default botonSurtido"><i class="fa fa-align-justify"></i></button>`;
                            } else {
                                return ``;
                            }
                        }, title: 'Surtido'
        ...
        function verReporte() {
            ...
            report.attr("src", '/Reportes/Vista.aspx?idReporte=125');
