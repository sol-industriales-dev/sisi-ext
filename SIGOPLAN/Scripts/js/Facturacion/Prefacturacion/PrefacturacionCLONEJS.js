/// <reference path="PrefacturacionCLONEJS.js" />
(function () {
    ...
    index = function () {
        ...
        const txtMetodoPagoSAT = $('#txtMetodoPagoSAT');
        const txtRegimenFiscal = $('#txtRegimenFiscal');
        const txtSelectTM = $('#txtSelectTM');
        const txtCondEntrega = $('#txtCondEntrega');
        const txtTipoFlete = $('#txtTipoFlete');
        const txtTipoPedido = $('#txtTipoPedido');
        const txtFormaPagoSAT = $('#txtFormaPagoSAT');
        const txtTipoFactura = $('#txtTipoFactura');
        const txtSerie = $('#txtSerie');
        const txtReqOC = $('#txtReqOC');
        const txtCondPago = $('#txtCondPago');
        const txtComentarios = $('#txtComentarios');
        const txtEntregado = $('#txtEntregado');
        const txtCuenta = $('#txtCuenta');
        const txtIVAValue = $('#txtIVAValue');
        ...
        function init() {
            ...
            txtMetodoPagoSAT.fillCombo('/Facturacion/Prefacturacion/FillComboMetodoPagoSat', null, false, null);
            txtRegimenFiscal.fillCombo('/Facturacion/Prefacturacion/FillComboRegimenFiscal', null, false, null);
            txtSelectTM.fillCombo('/Facturacion/Prefacturacion/FillComboTM', null, false, null);
            txtCondEntrega.fillCombo('/Facturacion/Prefacturacion/FillComboCondEntrega', null, false, null);
            txtTipoFlete.fillCombo('/Facturacion/Prefacturacion/FillComboTipoFlete', null, false, null);
            txtTipoPedido.fillCombo('/Facturacion/Prefacturacion/FillComboTipoPedido', null, false, "P");
            txtTipoFactura.fillCombo('/Facturacion/Prefacturacion/FillComboTipoFactura', null, false, null);
            txtSerie.fillCombo('/Facturacion/Prefacturacion/FillComboSerie', null, false, "A");

            // txtMetodoPago.on("change", function () {
            //     if ($(this).val() != "") {
            //         fncGetFormaPagoSAT();
            ...
            txtIVAValue.on("change", function () {
                let textCboIVAActual = $("#txtIVAValue option:selected").text();

                $("#labelIVA1").val(textCboIVAActual);

                let option = $("#lstConceptoImporte option[value=\"16% I.V.A.\"]").val();
                if (option == undefined) {

                    let option = $("#lstConceptoImporte option[value=\"8% I.V.A.\"]").val();
                    if (option == undefined) {

                        let option = $("#lstConceptoImporte option[value=\"0% I.V.A.\"]").val();
                        if (option == undefined) {

                        } else {

                            $("#lstConceptoImporte option[value=\"0% I.V.A.\"]").val(textCboIVAActual);
                        }
                    } else {
                        $("#lstConceptoImporte option[value=\"8% I.V.A.\"]").val(textCboIVAActual);

                    }
                } else {
                    $("#lstConceptoImporte option[value=\"16% I.V.A.\"]").val(textCboIVAActual);
            ...
            txtIVAValue.val(0.16);
            txtIVAValue.trigger("change");
        }
            ...
            tblUnidades.bootgrid({
                ...
                formatters: {
                    ...
                    "cc": function (column, row) {
                        let tempContCC = contTipo;
                        tempContCC--;
                        return "<select class=\"form-control ccPartida\" id=\"ccPartida" + tempContCC + "\" data-cc=\"" + row.cc + "\"/></select>";
                        // return "<select class=\"form-control selTipo\" id=\"tipo" + contTipo++ + "\"><option value=\"1\" " + insumo + " >Insumo</option><option value=\"3\" " + concepto + ">Concepto</option><option value=\"4\" " + total + ">Total</option></select><input type=\"hidden\" class=\"form-control\" id=\"id" + contId++ + "\" value =" + id + " />";
                    },
                    "TipoInsumo": function (column, row) {
                        let tempContTipo = contTipo;
                        tempContTipo--;

                        let numInusmoVal = row.TipoInsumo ?? "";
                        return "<input type=\"text\" class=\"form-control insumo\" id=\"TipoInsumo" + tempContTipo + "\" value=\"" + numInusmoVal + "\"/>";
                        // return "<select class=\"form-control selTipo\" id=\"tipo" + contTipo++ + "\"><option value=\"1\" " + insumo + " >Insumo</option><option value=\"3\" " + concepto + ">Concepto</option><option value=\"4\" " + total + ">Total</option></select><input type=\"hidden\" class=\"form-control\" id=\"id" + contId++ + "\" value =" + id + " />";
                    },
                    ...
                    "Concepto": function (column, row) {
                        ...
                        return "<div class=\"editar insumoDesc\" id=\"unidad" + contUnidad++ + "\">" + unidad + "</div>";
                    },

                    "Unidad": function (column, row) {
                        var concepto = row.Concepto == "undefined" || row.Concepto == undefined ? "&nbsp" : row.Concepto;
                        return "<input type=\"text\" class=\"form-control unidad\" id=\"concepto" + contConcepto++ + "\" value =" + concepto + " />"
                    },
                ...
                tblUnidades.find(".dinero").on("change", function (e) {
                    tblUnidades.find('tr').each(function () {
                        ...
                        if ($(this).find('td:eq(7) input').val() != undefined && tipo == "1") {
                            var numero1 = unmaskDinero($(this).find('td:eq(6) input').val());
                            $(this).find('td:eq(6) input[type="text"]').val(maskDinero(numero1));
                            var numero2 = unmaskDinero($(this).find('td:eq(7) input').val());
                            $(this).find('td:eq(7) input[type="text"]').val(maskDinero(numero2));
                        }
                    });
                });
                ...
                $(".ccPartida").fillCombo('/Administrativo/Facultamiento/getComboCCEnkontrol', null, false, null);

                let lstElemDesc = document.getElementsByClassName("insumoDesc");
                let lstElemCC = document.getElementsByClassName("ccPartida");

                for (var i = lstElemDesc.length - 1; i >= 0; i--) {
                    let item = $(`#${lstElemDesc[i].id}`);
                    let itemCC = $(`#${lstElemCC[i].id}`);

                    item.getAutocomplete(setInsumoBusqPorDesc, { cc: selCCFiltro.val() }, '/Facturacion/Prefacturacion/GetAutoCompleteInsumosDesc');

                    if (itemCC.data("cc") != undefined && itemCC.data("cc") != null) {
                        itemCC.val(itemCC.data("cc"));
                        itemCC.trigger("change");
                    }

                    //V1
                    // item.getAutocomplete(setInsumoBusqPorDesc, { cc: selCCFiltro.val() }, '/Enkontrol/Requisicion/getInsumosDesc');
                }

                let lstElemInsumos = document.getElementsByClassName("insumo");


                for (var i = lstElemInsumos.length - 1; i >= 0; i--) {

                    let item = $(`#${lstElemInsumos[i].id}`);
                    item.getAutocomplete(setInsumoDesc, { cc: selCCFiltro.val() }, '/Facturacion/Prefacturacion/GetAutoCompleteInsumos');

                    //
                    // item.getAutocomplete(setInsumoDesc, { cc: selCCFiltro.val() }, '/Enkontrol/Requisicion/getInsumos');
                    //
                }
        ...
        function AgregarRenglon() {
            ...
            tblUnidades.find('tbody')
                .append($('<tr data-row-id="' + getUltimoDataRow() + '">')
                    ...
                    .append($('<td>')
                        .append($("<select class=\"form-control ccPartida\" id=\"ccPartida" + tempContTipo + "\" /></select>")
                        )
                    )
                    .append($('<td>')
                        .append($("<input type=\"text\" class=\"form-control insumo\" id=\"TipoInsumo" + tempContTipo + "\" />")
                        )
                    )
                    .append($('<td>')
                    ...
                    .append($('<td>')
                        .append($("<input type=\"text\" id=\"cantidad" + contConcepto + "\" class=\"form-control importe unidad\" value =\"&nbsp\" />")
                        )
                    )
                    .append($('<td>')
                        .append($("<div class=\"editar insumoDesc\" id=\"unidad" + contUnidad + "\"  value =\"\" \"></div>")
                        )
                    )
                    .append($('<td>')
            ...
            $(`#ccPartida${tempContTipo}`).fillCombo('/Administrativo/Facultamiento/getComboCCEnkontrol', null, false, null);

            let tempContUnidad = contUnidad;
            tempContUnidad = tempContUnidad - 1;
            ...
            for (var i = lstElemDesc.length - 1; i >= 0; i--) {
                ...
                item.getAutocomplete(setInsumoBusqPorDesc, { cc: selCCFiltro.val() }, '/Facturacion/Prefacturacion/GetAutoCompleteInsumosDesc');
                // itemCC.fillCombo('/Administrativo/Facultamiento/getComboCCEnkontrol', null, false, null);
        ...
        function setInsumoBusqPorDesc(e, ui) {
            ...
            row.find('.insumo').val(ui.item.clave);
            row.find('.insumoDesc').html(ui.item.descripcion);
        ...
        function saveRepPrefactura(id) {
            $.ajax({
                datatype: "json",
                type: "POST",
                url: "/Facturacion/Prefacturacion/savePrefacturaCLONE",
        ...
        function getObjPrefactura() {
            var obj = {
                ...
                MetodoPagoSAT: txtMetodoPagoSAT.val(),
                RegimenFiscal: txtRegimenFiscal.val(),
                TM: txtSelectTM.val(),
                CondEntrega: txtCondEntrega.val(),
                TipoFlete: txtTipoFlete.val(),
                ReqOC: txtReqOC.val(),
                CondicionesPago: txtCondPago.val(),
                TipoPedido: txtTipoPedido.val(),
                Observaciones: txtComentarios.val(),
                Serie: txtSerie.val(),
                Entregado: txtEntregado.val(),
                Cuenta: txtCuenta.val(),
                TipoFactura: txtTipoFactura.val(),
                numcte: $("#btnVerReporte").data("numcte") ?? 0,
                IVA: txtIVAValue.val(),
            }
            if (txtEstado.val() == '0') {
                obj.Estado = '1';
        ...
        function getLstPrefactura() {
            ...
            tblUnidades.find("tr").each(function (idx, row) {
                if (idx > 0 && $(":nth-child(2)", row).html() != undefined) {
                    ...
                    JsonData.cc = $(this).find('td:eq(1) select').val();
                    JsonData.TipoInsumo = $(this).find('td:eq(2) input[type="text"]').val();
                    JsonData.Cantidad = $(this).find('td:eq(3) input[type="text"]').val();
                    JsonData.Concepto = $(this).find('td:eq(4) input[type="text"]').val();
                    var unidad = $(this).find('td:eq(5) div.editar').html().replace("&nbsp;", "");
                    JsonData.Unidad = encodeURIComponent(unidad);
                    JsonData.Precio = unmaskDinero($(this).find('td:eq(6) input[type="text"]').val());
                    JsonData.Importe = unmaskDinero($(this).find('td:eq(7) input[type="text"]').val());
...
function actualizacionConfirmada() {
    ...
    $.blockUI({ message: mensajes.PROCESANDO });
    $.ajax({
        url: '/Facturacion/Prefacturacion/ActualizaEstatusCLONE',
        type: "POST",
        datatype: "json",
    ...
    if (numCliente.val() > 0) {
        $.ajax({
            ...
            success: function (response) {
                ...
                $("#txtCondPago").val(response.condpago);
                modalBusqCliente.unblock();

                $("#btnVerReporte").data("numcte", response.numcte);
...
function setPrefactura(data) {
    ...
    $('#txtMetodoPagoSAT').val(data.MetodoPagoSAT);
    $('#txtMetodoPagoSAT').trigger("change");
    ...
    $("#txtRegimenFiscal").val(data.RegimenFiscal);
    $("#txtSelectTM").val(data.TM);
    $("#txtCondEntrega").val(data.CondEntrega);
    $("#txtTipoFlete").val(data.TipoFlete);
    $("#txtReqOC").val(data.ReqOC);
    $("#txtCondPago").val(data.CondicionesPago);
    $("#txtTipoPedido").val(data.TipoPedido);
    $("#txtComentarios").val(data.Observaciones);
    $("#txtSerie").val(data.Serie);
    $("#txtEntregado").val(data.Entregado);
    $("#txtCuenta").val(data.Cuenta);
    $("#txtTipoFactura").val(data.TipoFactura);
    $("#txtIVAValue").val(data.IVA);
    $("#txtIVAValue").trigger("change");
}
    ...
    if (response != null) {
        ...
        for (var j = 0; j <= largo; j++) {
            if (response[j] != undefined) {
                ...
                JsonData.cc = response[j].cc;
                JsonData.TipoInsumo = response[j].TipoInsumo;
...
function setInsumosDefault() {
    ...
    $('#txtMetodoPagoSAT').val("");
    $('#txtMetodoPagoSAT').trigger("change");
    $("#txtRegimenFiscal").val("");
    $("#txtSelectTM").val("");
    $("#txtCondEntrega").val("");
    $("#txtTipoFlete").val("");
    $("#txtCondPago").val("");
    ...
    let textCboIVA = $("#txtIVAValue option:selected").text();

    var lst = [{ Label: "Subtotal", Valor: "$0.00" }, { Label: textCboIVA, Valor: "$0.00" }]
    ...
    for (var i in lst) {
        if (0 <= +(i) && typeof +(i) == "number") {
            tblImpuesto.find('tbody')
                .append($('<tr>')
                    .append($('<td>')
                        .append(lst[i].Label == undefined ? "" : '<div class="input-group"><input type="text" id="labelIVA' + i + '" class="form-control importe lblImpuesto" list="lstConceptoImporte"/ value="' + lst[i].Label + '"><span class="input-group-addon" style="cursor:n-resize">■</span><input type="text" class="form-control importe valImpuesto" value="' + lst[i].Valor + '"/></div>'
                        )
                    )
                );
        }
    ...
    tblUnidades.find('tr').each(function (idx, row) {
        if (idx > 0) {
            if ($(this).find("td:eq(0) select", row).val() == '1') {
                ...
                $(this).find("td:eq(3)", row).removeClass('hidden');
                $(this).find("td:eq(4)", row).removeClass('hidden');
                $(this).find("td:eq(5)", row).removeClass('hidden');
                $(this).find("td:eq(6)", row).removeClass('hidden');

                $(this).find("td:eq(5)", row).removeAttr('colspan');

                $(this).find("td:eq(3) input", row).removeClass('hidden');
                $(this).find("td:eq(4) input", row).removeClass('hidden');
                $(this).find("td:eq(6) input", row).removeClass('hidden');
                $(this).find("td:eq(7) input", row).removeClass('hidden');

                if (validaVacio($(this).find("td:eq(3) input", row).val()))
                    $(this).find("td:eq(3) input", row).val(1);
                $(this).find("td:eq(7) input", row).prop("readonly", true);
            }
            if ($(this).find("td:eq(0) select", row).val() == '2') {

                $(this).find("td:eq(3)", row).addClass('hidden');
                $(this).find("td:eq(4)", row).addClass('hidden');
                $(this).find("td:eq(6)", row).addClass('hidden');
                $(this).find("td:eq(7)", row).addClass('hidden');

                $(this).find("td:eq(5)", row).attr('colspan', 5);
            ...
            if ($(this).find("td:eq(0) select", row).val() == '3') {
                ...
                $(this).find("td:eq(4)", row).removeClass('hidden');
                $(this).find("td:eq(6)", row).removeClass('hidden');
                $(this).find("td:eq(7)", row).removeClass('hidden');
                ...
                $(this).find("td:eq(7) input", row).prop("readonly", false);
            }
            if ($(this).find("td:eq(0) select", row).val() == '4') {
                ...
                $(this).find("td:eq(3) input", row).addClass('hidden');
                $(this).find("td:eq(4) input", row).addClass('hidden');
                $(this).find("td:eq(6) input", row).addClass('hidden');
                $(this).find("td:eq(7) input", row).removeClass('hidden');
                $(this).find("td:eq(7) input", row).prop("readonly", false);
            }
        }
...
function setImporte() {
    if (getRadioValue("radCalAuto")) {
            ...
            if (idx > 0) {
                ...
                if (tipo == '1') {
                    ...
                    if (validaVacio(cantidad)) {
                        $(this).find("td:eq(3) input").val(1);
                        cantidad = 1;
                    }
                    var precio = unmaskDinero($(this).find('td:eq(6) input').val());
                    var importe = cantidad * precio;
                    $(this).find('td:eq(6) input[type="text"]').val((maskNumDinero(precio)));
                    $(this).find('td:eq(7) input[type="text"]').val((maskNumDinero(importe)));
                    subTotal += importe;
                    totalCuerpo += importe;
                }
                if (tipo == '4') {
                    $(this).find('td:eq(7) input[type="text"]').val((maskNumDinero(totalCuerpo)));
                    totalCuerpo = 0;
                }
            }
        });
        var sumaParcial = 0;
        var sumarTodo = 0;
        var ivaPercent = Number($("#txtIVAValue").val());
...
function printAjustarUnidad(lstUnidad, lstImpuesto) {
    ...
    copy.splice(copy.findIndex(i => i.Label === "16% I.V.A." || i.Label === "8% I.V.A." || i.Label === "0% I.V.A."), 1);
...
function printAjustarImporte(lstImpuesto) {
    ...
    arr.push(lstImpuesto.find(o => o.Label === '16% I.V.A.' || o.Label === '8% I.V.A.' || o.Label === '0% I.V.A.'));
