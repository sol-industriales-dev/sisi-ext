(() => {
    ...
    FileManager = function () {
        ...
        //#region Elementos que pueden aparecer en el menú contextual
        const MENU_BASE = [
            { // 0
                id: "create",
                method: "createFolder",
        ...
        //#region Modal crear subdivisión
        ...
        //#region Modal subir varios archivos
        ...
        const cboFiltroObraCerrada = $('#cboFiltroObraCerrada');
        const btnFiltroCerrarObra = $('#btnFiltroCerrarObra');
        //#endregion

        //#region CONST DUPLICAR CARPETA
        const mdlDuplicarCarpetas = $('#mdlDuplicarCarpetas');
        const btnFiltroDuplicarCarpeta = $('#btnFiltroDuplicarCarpeta');
        const txtDuplicar_NombreCarpeta = $('#txtDuplicar_NombreCarpeta');
        const cboDuplicar_CarpetaBaseDestino = $('#cboDuplicar_CarpetaBaseDestino');
        const btnCrearCarpetaBase = $('#btnCrearCarpetaBase');
        //#endregion

        //#region CONST CERRAR OBRA
        const mdlCerrarObra = $('#mdlCerrarObra');
        const cboCerrarObra = $('#cboCerrarObra');
        const btnCerrarObra = $('#btnCerrarObra');
        //#endregion
        ...
        (function init() {
            ...
            fncGetPermisoAcciones();
        })();

        function agregarListeners() {
            //#region GESTOR DOCUMENTAL
            // Modal aviso
            ...
            btnFiltroBuscar.click(function () {
                document.location.href = `/FileManager/FileManager?obraCerrada=${cboFiltroObraCerrada.val()}`;
            });

            btnFiltroDuplicarCarpeta.click(function () {
                fncFillCboCarpetaBaseDestino();
                mdlDuplicarCarpetas.modal("show");
            });

            btnFiltroCerrarObra.click(function () {
                fncFillCboCarpetasNivelObra();
                mdlCerrarObra.modal("show");
            });
            //#endregion

            //#region DUPLICAR CARPETA
            btnCrearCarpetaBase.click(function () {
                fncGenerarCarpetaDuplicado();
            });
            //#endregion

            //#region CERRAR OBRAS
            btnCerrarObra.click(function () {
                Alert2AccionConfirmar('¡Cuidado!', '¿Desea cerrar la obra seleccionada?', 'Confirmar', 'Cancelar', () => fncCerrarObra());
            });
            //#endregion
        }
        ...
        function obtenerUrlPArams() {
            ...
            if (variables != undefined && variables != "") {
                cboFiltroObraCerrada.val(variables.obraCerrada);
            } else {
                cboFiltroObraCerrada.val(0);
            }
            cboFiltroObraCerrada.trigger('change');
        }
        ...
        //#region FUNCIONES DUPLICAR CARPETA
        function fncFillCboCarpetaBaseDestino() {
            cboDuplicar_CarpetaBaseDestino.fillCombo('FileManager/FillCboCarpetasBases', null, false, '--Seleccione una obra--');
            cboDuplicar_CarpetaBaseDestino.select2();
        }

        function fncGenerarCarpetaDuplicado() {
            let objParamsDTO = fncGetObjGenerarCarpetaDuplicado();
            axios.post('FileManager/GenerarCarpetaDuplicado', objParamsDTO).then(response => {
                let { success, items, message } = response.data;
                if (success) {
                    Alert2Exito(message);
                    mdlDuplicarCarpetas.modal("hide");
                } else {
                    Alert2Error(message);
                }
        ...
        function fncGetObjGenerarCarpetaDuplicado() {
            let objParamsDTO = {};
            objParamsDTO.padreID = cboDuplicar_CarpetaBaseDestino.val();
            objParamsDTO.nombreCarpetaBase = txtDuplicar_NombreCarpeta.val();
            return objParamsDTO;
        }
        //#endregion

        //#region FUNCIONES CERRAR OBRA
        function fncFillCboCarpetasNivelObra() {
            cboCerrarObra.fillCombo("FileManager/FillCboCarpetasObras", null, false, "--Seleccione una obra--");
            cboCerrarObra.select2();
        }

        function fncCerrarObra() {
            let obj = fncObjCerrarObra();
            if (obj != null) {
                axios.post('FileManager/CerrarObra', obj).then(response => {
                    let { success, items, message } = response.data;
                    if (success) {
                        Alert2Exito(message);
                        mdlCerrarObra.modal("hide");
                    } else {
                        Alert2Error(message);
                    }
                }).catch(error => Alert2Error(error.message));
        ...
        function fncObjCerrarObra() {
            fncDefaultCtrls("cboCerrarObra", true);

            if (cboCerrarObra.val() == "--Seleccione una obra--") { fncValidacionCtrl("cboCerrarObra", true, "Es necesario seleccionar una obra."); return null; }

            let obj = {};
            obj.idArchivo = cboCerrarObra.val();
            return obj;
        }
        //#endregion
        ...
        function initWebixTree() {
                    ...
                    path: {
                        ...
                        oninit: function () {
                            this.$$("path") && (this.attachEvent("onFolderSelect", webix.bind(function (padreID) {
                                ...
                                if (archivo && archivo.$count < 1 && !archivo.open) {
                                    archivo.open = true;
            ...
            webix.protoUI({
                ...
                createFolder: function (archivoID) {
                    ...
                    comboTipoArchivoFolder.fillCombo('FileManager/ObtenerTodosTiposArchivos', null, false, "Todos");
                ...
                createContractor: padreID => {
                    ...
                    $.blockUI({ message: 'Creando carpeta de contratista. Esta operación puede tardar un poco.' });
                    $.post('FileManager/CrearCarpetaContratista', { padreID, divisionID })
                        .always($.unblockUI)
                        .then(response => {
                            if (response.success) {
                ...
                createEstimate: padreID => {
                    ...
                    $.post(`FileManager/CrearCarpetaEstimacion`, { padreID, divisionID })
                        .always($.unblockUI)
                        .then(response => {
                            if (response.success) {
                ...
                createSubdivision: archivoID => {
                    ...
                    comboSubdivision.fillCombo('FileManager/ObtenerSubdivisiones', { divisionID: archivo.divisionID }, false);
                },
                ...
                createProject: archivoID => {
                    ...
                    comboObra.fillCombo('FileManager/ObtenerObras', null, false);
                },
                ...
                multipleFileUpload: archivoID => {
                    ...
                    $.post('FileManager/ObtenerTiposArchivos', { archivoID })
                        .always($.unblockUI)
                        .then(outerResponse => {
                            if (outerResponse.success) {
                                ...
                                $.blockUI({ message: 'Consultando información de la carpeta...' });
                                $.get('FileManager/ObtenerArchivosActualizables', { padreID: archivoID })
                                    .always($.unblockUI)
                                    .then(innerResponse => {
                    ...
                    this.Ox(),
                        ...
                        webix.ajax().post(endpoint, webix.copy(json), {
                            success: webix.bind(function (t, s) {
                                ...
                                if (endpoint === "FileManager/EliminarArchivo") {
                ...
                yx: function (archivoID) {
                    var archivoID = archivoID.toString();
                    archivo = this.getItem(archivoID);
                    if (archivo !== null) {
                        if ("folder" === archivo.type) {
                            fileManagerWebix.setCursor(archivoID);
                            fileManagerWebix.refreshCursor();
                            fileManagerWebix.refresh();
                        } else {
            ...
            webix.ready(() => {
                ...
                $.get('FileManager/VerificarAccesoGestorHierarchy')
                    .then(response => {
                        if (response.success) {
                            if (response.acceso) {
                                cargarDatos();
        ...
        function cargarDatos() {
            fileManagerWebix = new webix.ui({
                ...
                id: "files",
                handlers: {
                    "download": "FileManager/DescargarArchivo",
                    "downloadFolder": "FileManager/DescargarCarpeta",
                    "remove": "FileManager/EliminarArchivo",
                    "rename": "FileManager/RenombrarArchivo"
                }
            });

            $.blockUI({ message: 'Cargando datos' });
            $$("files").load(`FileManager/ObtenerEstructuraDirectoriosHierarchy`).then($.unblockUI).then(agregarEventoMenu);
        }
        ...
        function loadFileManagerFolder(folderID, callback) {
            fncDefaultCtrls("cboFiltroObraCerrada");
            if (cboFiltroObraCerrada.val() != "--Seleccione una opción--") {
                // Reset branch cache
                fileManagerWebix.$e = {};

                $.blockUI({ message: 'Cargando archivos folder...' });
                fileManagerWebix.loadBranch(folderID, callback, `FileManager/ObtenerEstructuraCarpetaHierarchy?obraCerrada=${cboFiltroObraCerrada.val()}`);
            } else {
                fncValidacionCtrl("cboFiltroObraCerrada", true, "Es necesario seleccionar el estatus de la obra.");
            }
        }
        ...
        function subirVariosArchivos() {
            ...
            $.blockUI({ message: 'Subiendo archivos...' });
            $.ajax({
                url: 'FileManager/SubirVariosArchivos',
                data,
                cache: false,
                contentType: false,
                processData: false,
                ...
                .then(response => {
                    if (response.success) {
                        ...
                        $.post('FileManager/SubirDatosVariosArchivos', { listaDatosArchivos, padreID })
                            .always($.unblockUI)
                            .then(response => {
                                if (response.success) {
                                    // Operación exitosa.

                                    $.blockUI({ message: 'Cargando nuevos archivos...' });
                                    loadFileManagerFolder(padreID, () => {
                                        $.unblockUI();
                                        fileManagerWebix.refreshCursor();
        ...
        function crearCarpeta(nombreCarpeta, padreID, listaTiposArchivosID, considerarse, abreviacion) {
            ...
            $.post('FileManager/CrearCarpeta', { nombreCarpeta, padreID, listaTiposArchivosID, considerarse, abreviacion })
                .always($.unblockUI)
                .then(response => {
        ...
        function crearSubdivision(subdivisionID, padreID) {
            ...
            $.post('FileManager/CrearSubdivision', { subdivisionID, padreID })
                .always($.unblockUI)
                .then(response => {
        ...
        function crearObraPorCC(subdivisionID, ccID, padreID) {
            ...
            $.post('FileManager/CrearEstructuraObraSubdivision', { subdivisionID, ccID })
                .always($.unblockUI)
                .then(response => {
                    if (response.success) {
        ...
        function crearObraPorNombre(subdivisionID, nombre, abreviacion, padreID) {
            ...
            $.post('FileManager/CrearEstructuraObraSubdivisionPorNombre', { subdivisionID, nombre, abreviacion })
                .always($.unblockUI)
                .then(response => {
                    if (response.success) {
        ...
        function cargarUsuariosAsignados(archivo) {
            ...
            $.get('FileManager/CargarUsuariosAsignados', { archivoID })
                .then(response => {
                    if (response.success) {
        ...
        function guardarCambiosPermisos() {
            ...
            $.post('FileManager/GuardarCambiosPermisos', { listaUsuarios, archivoID })
                .then(response => {
                    if (response.success) {
                        // mostrarModal(1, `Cambios guardados con éxito.`);
                        Alert2Exito("Se ha registrado con éxito los permisos.");
                    } else {
        ...
        function mostrarModalSubirArchivo() {
            comboTipoArchivo.fillCombo('FileManager/ObtenerTiposArchivos', { archivoID: getActiveParentItemID() }, false, "Seleccione un tipo de archivo");
        ...
        function mostrarModalActualizarArchivo(tipoArchivoID, archivoID) {
            comboTipoArchivo.fillCombo('FileManager/ObtenerTiposArchivos', { archivoID }, false, "Seleccione un tipo de archivo");
            modalCargaArchivo.modal({
                backdrop: 'static',
                keyboard: false
            });
        ...
        function subirArchivo() {
            ...
            $.each(document.getElementById("inputArchivo").files, function (i, file) {
                data.append("lstArchivos", file);
            });
            data.append("tipoArchivoID", comboTipoArchivo.val());
            data.append("padreID", getActiveParentItemID());
            $.blockUI({ message: 'Subiendo archivo. Esta operación puede tardar un poco.' });
            $.ajax({
                url: 'FileManager/SubirArchivo',
                data,
                cache: false,
                contentType: false,
                processData: false,
        ...
        function actualizarArchivo() {
            ...
            $.ajax({
                url: 'FileManager/ActualizarArchivo',
                data,
                cache: false,
                contentType: false,
                processData: false,
        ...
        function obtenerHistorialVersiones(archivoID) {
            ...
            $.get('FileManager/ObtenerHistorialVersiones', { archivoID })
                .then(response => {
                    if (response.success) {
        ...
        function verArchivo(archivoID) {
            ...
            $.post('FileManager/CargarDatosArchivoVisor', { archivoID })
                .always($.unblockUI)
                .then(response => {
                    if (response.success) {
                        // Operación exitosa.
        ...
        function fncDescargarCarpeta(archivoID) {
            if (archivoID > 0) {
                location.href = `FileManager/DescargarCarpeta?archivoID=${archivoID}`;
                // $("#mdlCargando").modal("show");
                // let objParamsDTO = {};
                // objParamsDTO.action = "";
                // objParamsDTO.source = archivoID;
                // axios.post('FileManager/DescargarCarpeta', objParamsDTO).then(response => {
                //     let { success, items, message } = response.data;
                //     if (true) {
                //         Alert2Exito("Se ha descargado con éxito");
                ...
                Alert2Error("Ocurrió un error al descargar la carpeta.");
            }
        }

        //#region GENERALES
        function fncGetPermisoAcciones() {
            axios.post('FileManager/GetPermisoAcciones').then(response => {
                let { success, items, message } = response.data;
                if (success) {
                    if (response.data.puedeGenerarCarpetaBase) {
                        btnFiltroDuplicarCarpeta.css("display", "inline");
                    } else {
                        btnFiltroDuplicarCarpeta.css("display", "none");
                    }

                    if (response.data.puedeCerrarObra) {
                        btnFiltroCerrarObra.css("display", "inline");
                    } else {
                        btnFiltroCerrarObra.css("display", "none");
                    }
                } else {
                    Alert2Error(message);
                }
    ...
    $(document).ready(() => FileManager.FileManager = new FileManager());
})();