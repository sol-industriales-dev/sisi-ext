...
namespace Doconut.Mvc.Core.Controllers
{
    public class HomeController : Controller
    {
        ...
        [HttpPost]
        public async Task<IActionResult> OpenDocument(string fileName)
        {
            ...
            try
            {
                ...
                // Load initial Document Sample.doc's search file Sample.doc.srh
                // Use such code only when you know that the SRH file already exists
                var srhFilePath = Path.Combine(Path.Combine(_hostingEnvironment.WebRootPath, "files"), $"{fileName}.srh");

                if (System.IO.File.Exists(srhFilePath))
                {
                    docViewer.LoadSearchData(srhFilePath);
                ...
                {
                    // Export to DCN
                    // Search Server .EXE will work on this
                    // DCN file

                    var dcnbytes = docViewer.SaveDocument();

                    if (null != dcnbytes && dcnbytes.Length > 0)
                    {
                        // Create a folder for this DCN first
                        var dcnPath = Path.Combine(Path.Combine(_hostingEnvironment.WebRootPath, "files"), "dcn");
                        dcnPath = Path.Combine(dcnPath, token);
                        Directory.CreateDirectory(dcnPath);

                        // Create DCN file inside this folder
                        using (var fs = new FileStream(Path.Combine(dcnPath, $"{token}.dcn"), FileMode.Create))
                        {
                ...
                // This is required to later load the search data back to the viewer
        ...
        {
            var srhPath = Path.Combine(Path.Combine(_hostingEnvironment.WebRootPath, "files"), "srh");

            var srhFilePath = Path.Combine(srhPath, $"{token}.dcn.srh");

            if (System.IO.File.Exists(srhFilePath))
            {
                // get the previously stored instance
                _cache.TryGetValue("docViewer-" + token, out DocViewer documentViewer);

                if (null == documentViewer)
                {
                    throw new Exception("error: DocumentViewer is null");
                }

                // load search file
                documentViewer.LoadSearchData(srhFilePath);

                // free up the cache object
                _cache.Remove("docViewer-" + token);
